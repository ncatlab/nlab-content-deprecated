...
We discuss now the notion of Hamiltonian vector fields in the full generality internal to a [[cohesive (âˆž,1)-topos]] $\mathbf{H}$. We write out the discussion for the case $\mathbf{H} = $ [[SmoothâˆžGrpd]] for convenience, but any other choice of cohesive $(\infty,1)$-topos works as well.
...
For any $X \in \mathbf{H}$,  a morphism $\omega \colon X \to \Omega^{n+1}_{cl}$ is a [[n-plectic geometry|pre-n-plectic structure]] on $X$. For instance $(X,\omega)$ might be a [[symplectic âˆž-groupoid]].
...
1. $\mathbf{Aut}(\nabla)$ is the [[automorphism âˆž-group]] of $\nabla$ formed in the [[slice (âˆž,1)-topos]] $\mathbf{H}_{/\mathbf{B}^n U(1)_{conn}}$

1. $\prod_{\mathbf{B}^n U(1)_{conn}} \colon \mathbf{H}_{/\mathbf{B}^n U(1)_{conn}} \to \mathbf{H}$ is the [[dependent product]] [[(âˆž,1)-functor]]. 

There is a canonical homomorphism of [[âˆž-groups]]

$$
  p \colon \mathbf{QuantMorph}(X,\omega) \to \mathbf{Aut}(X)
$$

to the [[automorphism âˆž-group]] of $X$ (the [[diffeomorphism group]] of $X$), given as the restriction to invertible endomorphisms of the canonical morphism
...
The [[Hamiltonian symplectomorphism n-group]] $\mathbf{HamSymp}(X,\omega)$ of $(X,\omega)$ is the [[âˆž-image]] of this morphism $p$, hence the factorization
...
of $p$ by an [[effective epimorphism in an (âˆž,1)-category|effective epimorphism]] followed by a [[monomorphism in an (âˆž,1)-category|monomorphism]].

The corresponding [[âˆž-Lie algebra]] 
...
on $(X, \omega)$ is an element $\phi \colon X \stackrel{\simeq}{\to} X$ in the [[automorphism âˆž-group]] $\phi \in \mathbf{Aut}(X)$ such that it fits into a [[diagram]] of the form
...
* [[Jean-Luc Brylinski]], _Loop spaces, characteristic classes and geometric quantization_, BirkhÃ¤user.
 {#Brylinski}
