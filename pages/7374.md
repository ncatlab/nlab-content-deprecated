...
A _reduced Segal space_ is a [[category object in an (âˆž,1)-category|category object]] in [[âˆžGrpd]] equipped with an (essentially) surjective functor from the terminal such. So it is the [[delooping]] of a [[monoid in an (âˆž,1)-category]].

Accordingly, if the reduced Segal space happens to be actually a [[groupoid object in an (âˆž,1)-category|groupoid object]], then it is the [[delooping]] of a [[group object in an (âˆž,1)-category|group object]] in [[âˆžGrpd]]: an [[âˆž-group]].
