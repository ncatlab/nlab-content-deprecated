...
It turns out  that such [[differential cohomology diagrams]] are exactly what all [[stable homotopy types]] in [[cohesive (âˆž,1)-toposes]] $\mathbf{H}$ naturally sit in ([Bunke-Nikolaus-VÃ¶lkl 13](#BunkeNikolausVoelkl13)). All the traditional differential cohomology theories and their joint generalization due to ([Hopkins-Singer 02](#HopkinsSinger02)) are special cases of this for the case that $\mathbf{H} = T Smooth \infty Grpd$ is the [[tangent cohesive (âˆž,1)-topos]] of that of [[smooth âˆž-groupoids]] ([[âˆž-stacks]] over the [[site]] of [[smooth manifolds]]).

It is therefore natural to _define_ differential cohomology to be the intrinsic [[cohomology]] of [[cohesive (âˆž,1)-toposes]]. (Notice that while the [[differential cohomology diagram]] itself only involves the [[shape modality]] and the [[flat modality]] of [[cohesion]], the [[sharp modality]] is needed to produce, via [[differential concretification]], the correct [[moduli stacks]] of differential cocycles over a given base object from the [[mapping stack]] of that into the representing [[stable homotopy type]]).
...
* MU-bordisms (unitary bordisms)  
  (Bunke--SchrÃ¶der--Schick--Wiethaupts; and from there Landweber exact cohomology theories)
=--
...
* [[Ulrich Bunke]], [[Thomas Schick]], Ingo SchrÃ¶der,  Moritz Wiethaup,  _Landweber exact formal group laws and smooth cohomology theories_ ([arXiv:0711.1134](http://arxiv.org/abs/0711.1134))
...
The suggestion that differential cohomology is naturally the intrinsic [[cohomology]] of those [[(âˆž,1)-toposes]] which are [[cohesive (âˆž,1)-topos|cohesive]] appeared in
...
and the observation that indeed every [[stable homotopy type]] in a [[cohesive (âˆž,1)-topos]] canonically sits inside a [[differential cohomology diagram]] is due to 
