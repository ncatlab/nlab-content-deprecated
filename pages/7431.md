...
For $\kappa$ some [[cardinal]], say a morphism $f : x \to y$ in $C$ is **relatively $\kappa$-compact** if for all [[(âˆž,1)-pullbacks]] along $h : y' \to y$ to $\kappa$-[[compact object in an (âˆž,1)-category|compact object]]s, $y'$, the pulled back object $h^* x'$ is itself a $\kappa$-compact object.
...
A [[presentable (âˆž,1)-category]] $C$  is an [[(âˆž,1)-topos]] precisely if
...
In [[âˆžGrpd]] the relatively $\kappa$-compact morphisms, $X \to Y$, def. \ref{RelativelyKappaCompact} are precisely those all whose [[homotopy fibers]]
...
We may write $Y$ as an [[(âˆž,1)-colimit]] over itself (see there)

$$
  Y \simeq {\lim_{\to}}_{y \in Y} \{y\} 
$$

and then use the fact that [[âˆžGrpd]] -- being an [[(âˆž,1)-topos]] -- has 
[[universal colimits]], to obtain the [[(âˆž,1)-pullback]] diagram
