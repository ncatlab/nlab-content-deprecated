...
where $[F,-]$ and $T \wedge (-) \colon Spectr \to Spectra$ are [[idempotent (âˆž,1)-monads]] and $[T,-]$, $[F,-]$ are idempotent $\infty$-comonad, the diagonals are [[homotopy fiber sequences]].
...
An $A$-[[âˆž-module]] $N$ is an _$\mathfrak{a}$-local module_ if for every $\mathfrak{a}$-[[torsion module]] $T$ (def. \ref{TorsionInfinityModule}), the [[derived hom space]]
...
For $\mathfrak{a} = (a)$ generated from a single element, then the [localization of an (âˆž,1)-ring](localization+of+a+commutative+ring#ForEInfinityRings)-map $A \to A[a^{-1}]$ is given by the [[(âˆž,1)-colimit]] over the sequence of right-multiplication with $a$
...
of [[âˆž-modules]] [[localization of a module|local]] away from $\mathfrak{a}$ is [[reflective sub-(âˆž,1)-category|reflective]]. The reflector
...
* [[Z. Å koda]], _Noncommutative localization in noncommutative geometry_,  London Math. Society Lecture Note Series 330 ([pdf](http://www.maths.ed.ac.uk/~aar/books/nlat.pdf)), ed.  A. Ranicki; pp. 220--313, [math.QA/0403276](http://arxiv.org/abs/math.QA/0403276)
