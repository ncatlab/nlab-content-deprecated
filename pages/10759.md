...
Each [[filtered object|filtering]] on an [[object]] $X$ in a suitable [[stable (âˆž,1)-category]] $\mathcal{C}$ (a _[[stable homotopy type]]_ $X$ such as a [[spectrum object]], in particular possibly a [[chain complex]]) induces a [[spectral sequence]] whose first page consists of the [[homotopy groups]] of the [[homotopy cofibers]] of the filtering and which under suitable conditions converges to the [[homotopy groups]] of the total object $X$.

This is a generalization of the traditional [[spectral sequence of a filtered complex]] to which it reduces for $\mathcal{C} = Ch_\bullet(\mathcal{A})$ an [[(âˆž,1)-category of chain complexes]] [[presentable (infinity,1)-category|presented]] in the projective [[model structure on chain complexes]]. Therefore the whole zoo of traditional spectral sequences that in turn are special cases of that of a filtered complex (see at _[spectral sequence -- Examples](spectral+sequence#Examples)_) is all subsumed by the concept of spectral sequence of a filtered stable homotopy type.

Moreover, by applying general [[(âˆž,1)-category theory|(âˆž,1)-categorical]] notion to naturally arising towers (such as the [[Whitehead tower]], the [[chromatic tower]]) it naturally produces more specialized spectral sequences (such as the [[Atiyah-Hirzebruch spectral sequence]], the [[chromatic spectral sequence]], etc.). Specifically, applied to a [[coskeleton]] tower of a dual [[Cech nerve]] of an [[E-âˆž algebra]] $E$ it naturally produces the $E$-[[Adams spectral sequence]]. See the discussion of the _[Examples](#Examples)_ below.

Therefore in [[(âˆž,1)-category theory]] one finds a lucky coincidence of historical terminology: _[[spectral sequences]] are essentially [[sequential diagram|sequences]] of [[spectra]]_, when considered on homotopy groups.
...
from a [[stable (âˆž,1)-category]] to an [[abelian category]] induces a functor

$$ Filt(\mathcal{C}) \to SpSeq(\mathcal{A}) $$


from the stable (âˆž,1)-category of [[filtered object in an (âˆž,1)-category|filtered objects]] in $\mathcal{C}$ to the abelian category of bigraded [[spectral sequence|spectral sequences]] in $\mathcal{A}$. 
=--

## Definition


Let throughout $\mathcal{C}$ be a [[stable (âˆž,1)-category]], $\mathcal{A}$ an [[abelian category]], and $\pi \;\colon\; \mathcal{C}\longrightarrow \mathcal{A}$ a [[homological functor]] on $\mathcal{C}$, i.e., a functor that transforms every [[cofiber sequence]]
...
* $\mathcal{C}$ is equipped with a [[t-structure]], $\mathcal{A}$ is the [[heart of a stable (âˆž,1)-category|heart]] of the t-structure, and $\pi$ is the canonical functor.
...
A _[[filtered object in an (âˆž,1)-category]]_ in $\mathcal{C}$ is
...
between $X$ and the [[(âˆž,1)-colimit]] of this sequence.
(The sequence itself is _the filtering on $X$_.)

Dually, a _[[filtered object in an (âˆž,1)-category|co-filtered object in an (âˆž,1)-category]]_ in $\mathcal{C}$ is
an [[object]] $X \in \mathcal{C}$ together with 
a [[sequential diagram]] $X \colon (\mathbb{Z}, \lt) \to \mathcal{C}$
...
between $X$ and the [[(âˆž,1)-limit]] of this sequence.
...
sufficiently nice [[model category]] $C$ (for instance a [[combinatorial  model category]]), then [[(âˆž,1)-colimits]] in $\mathcal{C}$ are computed by [[homotopy colimits]] in $C$. These in turn are computed as ordinary [[colimits]] in $C$ over a cofibrant [[diagram]] in the [[projective model structure on functors]]. 
...
Let $I$ be a [[linearly ordered set]]. An _$I$-chain complex_ in a [[stable (âˆž,1)-category]] $\mathcal{C}$ is an [[(âˆž,1)-functor]]
...
for the [[full sub-(âˆž,1)-category]] of diagrams satisfying these conditions.
...
restricts to an [[equivalence of (âˆž,1)-categories|equivalence]] between the (âˆž,1)-category $Gap(\mathbb{Z},\mathcal{C})$ of $\mathbb{Z}\cup \{\infty\}$-chain complexes in $\mathcal{C}$ (def. \ref{ChainComplexInStableInfinityCategory}) and that of filtered objects in $\mathcal{C}$ (def. \ref{GeneralizedFilteredObject}). The equivalence is given by left and right [[(âˆž,1)-Kan extension]].
...
The spectral sequence above itself only actually depends to the [[triangulated category|triangulated]] [[homotopy category of an (infinity,1)-category|homotopy category]] $Ho(\mathcal{C})$. But its $\infty$-functorial dependence on the filtered object needs the full structure of the [[(âˆž,1)-category]] $\mathcal{C}$
...
restricts to an [[equivalence of (âˆž,1)-categories|equivalence]] between the (âˆž,1)-category of $\mathbb{Z}\cup \{\infty\}$-chain complexes in $\mathcal{C}$ (def. \ref{ChainComplexInStableInfinityCategory}) and that of generalized filtered objects in $\mathcal{C}$ (def. \ref{GeneralizedFilteredObject}).
...
Let $\mathcal{C}$ be a [[stable (âˆž,1)-category]] and let $\pi:\mathcal{C}\to\mathcal{A}$ be a homological functor where $\mathcal{A}$ is an [[abelian category]] which admits [[sequential limits]]. Let $X_\bullet$ be a filtered object in $\mathcal{C}$ such that $\underset{\leftarrow}{\lim} X_\bullet$ exists. Suppose further that:
...
For $\mathcal{A}$ a good [[abelian category]] and $\mathcal{C} = Ch_\bullet(\mathcal{A})$ the [[(âˆž,1)-category of chain complexes]] in $\mathcal{A}$, we recover, by \ref{FilteredObjectInModelCategory},
...
For $\mathcal{C}$ a [[stable (âˆž,1)-category]] and $X_\bullet$ a [[simplicial object in an (âˆž,1)-category]] in $\mathcal{C}$, then the [[simplicial skeleta]] of $X$ give it the structure of a [[filtered object in an (âˆž,1)-category]]. The corresponding [[spectral sequence of a filtered stable homotopy type]] has as its first page the [[Moore complexes]] of the corresponding [[simplicial objects]] of [[homotopy groups]].
...
induced from [[E-âˆž algebras]] (dual [[Cech nerves]]/[[Sweedler corings]]/[[Amitsur complexes]]).
...
Given an [[cosimplicial object in an (âˆž,1)-category]]  with [[(âˆž,1)-colimits]]
...
By the discussion at _[[âˆž-Dold-Kan correspondence]]_ and _[[spectral sequence of a filtered stable homotopy type]]_. This appears as ([[Higher Algebra|Higher Algebra, remark 1.2.4.4]]). Review is around  ([Wilson 13, theorem 1.2.4](#Wilson13)).
...
Let $S$ be an [[E-âˆž ring]] and let $E$ be an [[E-âˆž algebra]] over $S$, hence an [[E-âˆž ring]] equipped with a [[homomorphism]]
...
More generally, for $X$ an $S$-[[âˆž-module]], the canonical [[cosimplicial object]] is
...
Let $E$ be a [[connective spectrum|connective]] [[E-âˆž ring]] such that the core or $\pi_0(E)$, def. \ref{CoreOfARing} is either of
...
[[!redirects spectral sequence of a filtered object in a stable (âˆž,1)-category]]
