...
This is a sub-entry of [[SmoothâˆžGrpd]]. See there for background.
...
<a href="http://nlab.mathforge.org/nlab/show/cohesive+(infinity%2C1)-topos#Structures">structures in a cohesive (âˆž,1)-topos</a> realized in [[SmoothâˆžGrpd]].
...
We discuss the <a href="http://ncatlab.org/nlab/show/cohesive+(infinity%2C1)-topos#Homotopy">intrinsic fundamental âˆž-groupoid</a> construction realized in $Smooth \infty Grpd$.
...
$X_\bullet \in SmoothMfd^{\Delta^{op}} \hookrightarrow [CartSp_{smooth}^{op}, sSet]$, then its image $i_!(X) \in $ [[ETopâˆžGrpd]] under the [relative topological cohesion morphism](#RelativeTopologicalCohesion) is presented by the underlying [[simplicial topological space]] 
...
The claim then follows for general simplicial spaces by observing that $X_\bullet = \int^{[k] \in \Delta} \Delta[k] \cdot X_k \in [CartSp_{smooth}^{op}, sSet]_{proj,loc}$ presents the  [[(âˆž,1)-colimit]] over $X_\bullet : \Delta^{op} \to SmoothMfd \hookrightarrow Smooth \infty Grpd$ and the [[left adjoint|left]] [[adjoint (âˆž,1)-functor]] $i_!$ preserves these [[(âˆž,1)-colimit]]s. 
...
then the image of $X$ under the [[fundamental âˆž-groupoid in a locally âˆž-connected (âˆž,1)-topos]]-functor
...
is equivalent to the standard [[fundamental âˆž-groupoid]] of $X$.
...
[above proposition](#UnderlyingSimplicialTopologicalSpace) this is $\Pi_{Etop}$ applied to the underlying [[simplicial topological space]]. The claim then follows with the [corresponding proposition](#http://ncatlab.org/nlab/show/Euclidean-topological+infinity-groupoid#GeometricHomotopy) discussed at [[ETopâˆžGrpd]].
...
We discuss the notion of _[geometric path  âˆž-groupoids](http://ncatlab.org/nlab/show/cohesive+%28infinity,1%29-topos+--+structures#Paths)_ realized in $Smooth\infty Grpd$.
...
The object $\mathbf{Sing} X \in [CartSp^{op}, sSet]$ presents the abstractly defined [[fundamental âˆž-groupoid in a locally âˆž-connected (âˆž,1)-topos]] $\mathbf{\Pi}(X)$.
...
Using the [[Steenrod-Wockel approximation theorem]] this comes down to the same argument as for [[Euclidean topological âˆž-groupoid]]s. See _[Presentation of the path âˆž-groupoid](http://ncatlab.org/nlab/show/Euclidean-topological+infinity-groupoid#PresentationOfPathGroupoid)_ there.
...
We discuss <a href="http://nlab.mathforge.org/nlab/show/cohesive%20(infinity,1)-topos#InfinGroups">cohesive âˆž-groups</a> in $Smooth \infty Grpd$: smooth $\infty$-groups.
...
$SmoothMfd \hookrightarrow Smooth \infty Grpd$ this is canonically identifed as a  [[0-truncated]] [[âˆž-group]] object in $Smooth \infty Grpd$.
...
as discussed at [[universal principal âˆž-bundle]].
...
_[[circle group]]_ or _1-dimensional [[unitary group]]_ , regarded as a [[0-truncated]] [[âˆž-group]] object in $Smooth\infty Grpd$ under the [embedding](#SmoothManifoldEmbeds) $SmoothMfd \hookrightarrow Smooth \infty Grpd$.
...
Every [[connected]] object $X \in \infty Lie Grpd$ is -- by definition -- the [[delooping]] $X = \mathbf{B}G$ of a Lie [[âˆž-group]] $G = \Omega X$, its [[loop space object]] formed in $\infty LieGrpd$. Since the discussion of [[group object in an (infinity,1)-category|group objects]], [[loop space  object]]s etc. involves only finite [[limit in a quasi-category|(âˆž,1)-limits]] and [[âˆž-stackification]] preserves these, this may be discussed in the [[(âˆž,1)-category of (âˆž,1)-presheaves]] on [[CartSp]]. Since there $(\infty,1)$-limits are computed objectwise, an [[âˆž-group]] object $G$ in $\infty LieGrpd$ is modeled by a [[(âˆž,1)-presheaf]] with values in [[âˆž-group]]s in [[âˆžGrpd]].

By standard results on <a href="http://ncatlab.org/nlab/show/groupoid+object+in+an+(infinity,1)-category#ModelsInInfGrpd">Models for group objects in âˆžGrpd</a> the latter may equivalently be modeled by [[simplicial group]]s. A simplicial group is possibly weak [[âˆž-groupoid]] equipped with a _strict_ group object structure. While [[strict omega-groupoid|strict âˆž-groupoids]] with weak group object structure do not model all [[âˆž-group]]s, weak $\infty$-groupoids with strict group structure do.
...
We discuss the <a href="http://nlab.mathforge.org/nlab/show/cohesive+(infinity%2C1)-topos#Cohomology">intrinsic cohomology and pricipal âˆž-bundles</a> in $Smooth \infty Grpd$.
...
Let $A \in $ [[âˆžGrpd]] be any [[discrete âˆž-groupoid]]. Write $|A| \in $ [[Top]] for its [[geometric realization]]. For $X$ any [[topological space]], the [[nonabelian cohomology]] of $X$ with coefficients in $A$ is the set of [[homotopy]] classes of maps $X \to |A|$
...
We say $Top(X,|A|)$ itself is the [[cocycle]] [[âˆž-groupoid]] for $A$-valued [[nonabelian cohomology]] on $X$.
...
Let $A \in $ [[âˆžGrpd]], write $Disc A \in Smooth \infty Grpd$ for the corresponding [[discrete âˆž-groupoid|discrete smooth âˆž-groupoid]]. Let $X \in SmoothMfd \stackrel{i}{\hookrightarrow} Smooth \infty Grpd$ be a [[paracompact topological space]] regarded as a 0-[[truncated]] 
...
and in fact an [[equivalence in an (âˆž,1)-category|equivalence]] of [[cocycle]] [[âˆž-groupoid]]s
...
The [[universal principal âˆž-bundle|universal G-principal bundle]] is a replacement of the point inclusion $* \to \mathbf{B}G$ by a fibration $\mathbf{E}G \to \mathbf{B}G$.
...
One noteworthy aspect of this object is that it is itself groupal, in fact itself a Lie [[strict 2-group]] in a way that is compatible with the canonical inclusion $G \to \mathbf{E}G$: it is an example of a [[groupal model for universal principal âˆž-bundles]].
...
The following proposition asserts that the general definition of [[principal âˆž-bundle]]s in an [[(âˆž,1)-topos]] $\mathbf{H}$ applied to the coefficient object $\mathbf{B}G$ in $\mathbf{H} = \infty LieGrpd$ for $G$ a [[Lie group]] does reprpduce the ordinary notion of $G$-[[principal bundle]]s.
...
The second statement is a special case of the more general statement that is proven at [[SynthDiffâˆžGrpd]].
...
<a href="http://nlab.mathforge.org/nlab/show/cohesive+%28infinity%2C1%29-topos#TwistedCohomology">twisted cohomology in a cohesive (âˆž,1)-topos</a>.
...
Given a [[Lie group]] $G$, we may regard it either as a [[topological group]] and form the [[classifying space]] $\mathcal{B}G \in $ [[Top]] $\simeq$ [[âˆžGrpd]], or we may form its [[delooping]] [[Lie groupoid]] $\mathbf{B}G \in \infty LieGrpd$, as discussed in the [section on Lie groups](#LieGroups) above.
...
Classes of examples of such lifts have been described in the section on [integration of âˆž-Lie algebra cocycles](#IntegrationOfCocycles). 

By general reasoning, the extensions/[[principal âˆž-bundle]]s classified by such cocycles are their [[homotopy fiber]]
...
This establishes the [[(âˆž,1)-pullback]] diagram
...
$SmoothMfd \hookrightarrow Smooth \infty Grpd$. Let $A \in $ [[âˆžGrpd]] be any [[âˆž-groupoid]] and $Disc A \in Smooth \infty Grpd$ the coresponding [[discrete âˆž-groupoid]].  

We have an equivalence of [[cocycle]] [[âˆž-groupoid]]s
...
Same proof as of the analogous statement at [[ETopâˆžGrpd]].
...
Let $G$ be a [[Lie group]] regarded as a [[0-truncated]] [[âˆž-group]]
...
By the discussion at [[âˆž-cohesive site]] we have that 
...
Since both domain and codomain are abelian, it will be sufficient to demonstrate the statement for $n = 1$, $k$ arbitrary, and for $k = 1$, $n$ arbitrary. All other combinations of $n$ and $k$ are then implied by repeatedly using the [[delooping]]/[[loop space object|looping]] [[adjoint (âˆž,1)-functor|(âˆž,1)-adjunction]] $(\Sigma \dashv \Omega)$  on abelian objects in
...
we may get a presentation for the defining [[(âˆž,1)-pullback]]
...
Let $\Xi : CrsdCplx \to KanCplx$ now denote the inclusion of [[crossed complex]]es into all [[Kan complex]]es/[[âˆž-groupoid]]s.
...
We discuss here the  <a href="http://nlab.mathforge.org/nlab/show/cohesive+(infinity%2C1)-topos#LieAlgebras">intrinsic exponentiated âˆž-Lie algebras</a> in $Smooth \infty Grpd$.
...
for the [[full subcategory]] of the [[opposite category]] on the graded-commutative [[semi-free dga]]s of [[finite type]] on generators in positive degree. This is the category of [[L-âˆž algebra]]s identified by their dual [[Chevalley-Eilenberg algebra]]s.

=--

We describe a presentation of the exponentiation an [[L-âˆž algebra]] to a smooth $\infty$-group. The following somewhat technical definition serves to control the smooth structure on these exponentiated objects.
...
By the <a href="http://ncatlab.org/nlab/show/cohesive+(infinity%2C1)-topos#Homotopy">propositon about geometric homotopy</a> in a [[cohesive (âˆž,1)-topos]] it follows that 
...
**[[n-connected|âˆž-simply connected]] [[Lie integration]]**
of $\mathfrak{g}$.
...
With $G$ regarded as a [smooth âˆž-group](#LieGroups) write $\mathbf{B}G \in Smooth\infty Grpd$ for its [[delooping]]. Recall from [above](#LieGroups) the standard presentation of this by a simplicial presheaf $\mathbf{B}G_c \in [CartSp_{smooth}^{op}, sSet]$.
...
for the [[L-âˆž-algebra]] whose [[Chevalley-Eilenberg algebra]] is given by a single generator in degree $n$ and vanishing differential. We call this the 
**[[line Lie n-algebra]]**
...
The [[discrete âˆž-groupoid]] underlying $\exp(b^{n-1} \mathbb{R})$ is given by the [[Kan complex]] that in degree $k$ has the set of closed differential $n$-forms (with sitting instants) on the $k$-[[simplex]]
...
$\mathbf{\flat} \exp(\mathfrak{g})$ of [exponentiated âˆž-Lie algebras](#StrucLieAlg) $\exp(\mathfrak{g})$.
...
$\mathbf{\flat}_{dR} \exp(\mathfrak{g})$ of [exponentiated L-âˆž algebras](#StrucLieAlg) $\exp(\mathfrak{g})$.
...
we may present the defining [[(âˆž,1)-pullback]] 
$\mathbf{\flat}_{dR} \mathbf{B}^n \mathbb{R} := 
...
We compute the defining double [[(âˆž,1)-pullback]]
...
We need to present the defining [[(âˆž,1)-pullback]]
...
by a [[homotopy pullback]] in $[CartSp^{op}, sSet]_{proj}$ (since [[(âˆž,1)-sheafification]] preserves finite [[(âˆž,1)-pullback]]s it is sufficient to present the $(\infty,1)$-pullback in [[(âˆž,1)-presheaves]]).
...
Therefore this is a [[homotopy pullback]] in $[CartSp^{op}, sSet]_{proj}$ that realizes the  [[(âˆž,1)-pullback]] in question. 
...
is a presentation for the defining double [[(âˆž,1)-pullback]] 
...
and for the [[nLab:universal principal âˆž-bundle|universal principal 2-bundle]]
...
In the [next section](#AbGerbesConnection) we give the proof of this (simple) claim. Here in the warmup phase we instead want to discuss the geometric interpretation of this resolution, along the lines of the section <a href="http://ncatlab.org/schreiber/show/differential+cohomology+in+an+(%E2%88%9E%2C1)-topos+--+survey#CurvatureCharacteristicsI">curvature characteristics of 1-bundles</a> in the [[schreiber:differential cohomology in an (âˆž,1)-topos -- survey|survey-part]].
...
* $INN(U(1))$ is the [[nLab:2-group]] $\Xi(U(1) \to U(1))$, which is a 
  [[nLab:groupal model for universal principal âˆž-bundles|groupal model for the universal U(1)-principal bundle]] $\mathbf{E}U(1)$;
...
The general version of this story is discussed in detail at <a href="#Connections">differential cohomology in an (âˆž,1)-topos -- Local (pseudo-)connections.</a>
...
Recall the definition of the <a href="http://nlab.mathforge.org/nlab/show/cohesive+(infinity%2C1)-topos#DifferentialCohomology">intrinsic differential cohomology</a> on $X \in Smooth \infty Grpd$ with coefficients in $U(1)$ as the [[(âˆž,1)-pullback]]
...
in [[âˆžGrpd]], where the morphism on the right picks one base point in each connected component.
...
[above](#StrucDeRham) we may replace the right morphism in this pullback by $\Omega^{n+1}_{cl}(X) \to \mathbf{H}_{dR}(X,\mathbf{B}^{n+1}U(1))$ and consider the [[(âˆž,1)-pullback]]
...
For the first sequence, the characteristic class exact sequence, use <a href="http://nlab.mathforge.org/nlab/show/cohesive+(infinity%2C1)-topos#CharacteristicClassExactSequence">this general proposition</a> from the discussion at _[[cohesive (âˆž,1)-topos]]_ . This says that over the vanishing curvature form $0 \in \Omega_{cl}^{n+1}(X)$ we have the short exact sequence
...
For the second sequence, the curvature exact sequence, <a href="http://nlab.mathforge.org/nlab/show/cohesive+(infinity%2C1)-topos#CurvatureExactSequence">this general proposition</a> from the discussion at _[[cohesive (âˆž,1)-topos]]_ , which implies that we have a short exact sequence
...
In summary this gives us the following alternative perspective on connections on $\mathbf{B}^{n-1}U(1)$-[[principal âˆž-bundle]]s: such a connection is a cocycle with values in the $\mathbf{B}^n \mathbb{Z}$-quotient of the  $(n+1)$-coskeleton of the simplicial presheaf which over $(U,[k])$ is the set of diagrams of [[dg-algebra]]s
...
The generalization to such diagram cocycles from $b^{n-1}\mathbb{R}$ to general [[âˆž-Lie algebra]]s $\mathfrak{g}$ we discuss below in [âˆž-Lie algebra valued connections](#InfinityLieAlgebraConnection).
...
We discuss the <a href="http://nlab.mathforge.org/nlab/show/cohesive+(infinity%2C1)-topos#ChernWeilTheory">general abstract notion of Chern-Weil homomorphism and âˆž-connections</a> realized in $Smooth \infty Grpd$.

Recall that for $A \in Smooth \infty Grpd$ a smooth $\infty$-groupoid regarded as a coefficient object for [[cohomology]], for instance the [[delooping]] $A = \mathbf{B}G$ of an [[âˆž-group]] $G$ we have abstractly that
...
* the [[connection on an âˆž-bundle|âˆž-connections]] with coefficients in $A$ are the [[cocycle]]s in the [[âˆž-groupoid]] $Smooth \infty Grpd(X,A)_{conn}$ that universally lifts these differential classs in de Rham cohomology to full [differential cohomology](#StrucDifferentialCohomology), in that it universally fills the diagrams
...
We now insert this in the above general abstract definition of the $\infty$-Chern-Weil homomorphism to deduce a presentation of that in terms of smooth families of [[âˆž-Lie algebroid valued differential forms]].

The main step is the construction of a well-suited composite of two spans of morphisms of simplicial presheaves (of two [[âˆž-anafunctor]]s): we consider presentations of [[characteristic class]]es $\mathbf{c} : \mathbf{B}G \to \mathbf{B}^n U(1)$ in the image of the [âˆž-Lie integration](#LieIntegration) map and presented by trunactions and quotients of morphisms of simplicial presheaves of the form
...
-- preferably naturally in the [[L-âˆž algebra]] $\mathfrak{g}$ -- a simplicial presheaf $\exp(\mathfrak{g})_{diff}$ that fills this diagram as follows:
...
This object $\exp(\mathfrak{g})_{diff}$ we shall see may be interpreted as the coefficient for _pseudo_ [[connection on an âˆž-bundle|âˆž-connections]] with values in $\mathfrak{g}$. 

There is however still room to adjust this presentation such as to yield in each cohomology class special nice cocycle representatives. This we will achieve by finding naturally a [[subobject]] $\exp(\mathfrak{g})_{conn} \hookrightarrow \exp(\mathfrak{g})_{diff}$ whose inclusion is an isomorphism on connected components and restricted to which the morphism $curv_{smp} \circ \exp(\mu,cs)$ yields nice representatives in the [[de Rham cohomology|de Rham]] [[hypercohomology]] encoded by $\mathbf{\flat}_{dR} \mathbf{B}^{n+1} \mathbb{R}_{smp}$, namely globally defined differential forms. On this object the differential characteristic classes we will show factors naturally through the refinements to differential cohomology, and hence $\exp(\mathfrak{g})_{conn}$ is finally identified as a presentation for the the coefficient object for [[connection on an âˆž-bundle|âˆž-connections]] with values in $\mathfrak{g}$.


#### $\infty$-Pseudo-connections

We discuss presentations for the differential characteristic classes in the image of [âˆž-Lie integration](#LieIntegration).


Let $\mathfrak{g} \in L_\infty \stackrel{CE}{\hookrightarrow} dgAlg^{op}$ be an [[L-âˆž algebra]].


+-- {: .num_defn}
###### Definition

A [[L-âˆž algebra cohomology|L-âˆž algebra cocycle]] on $\mathfrak{g}$ in degree $n$ is a morphism
...
[exponentiated L-âˆž-algebras](#StrucLieAlg)
...
Let $G \in Smooth \infty Grpd$ be an [[âˆž-group|n-group]] given by [Lie integration](#LieIntegration) of an [[L-âˆž algebra]] $\mathfrak{g}$, in that the [[delooping]] object $\mathbf{B}G$ is presented by the $(n+1)$-[[simplicial skeleton|coskeleton]] [[simplicial presheaf]] 
...
is the [[Kan complex]] of **[[connection on an âˆž-bundle|pseudo n-connections]]** on $G$-[[principal âˆž-bundle]]s. 
...
the <a href="http://ncatlab.org/nlab/show/cohesive+(infinity%2C1)-topos#ChernWeilTheory">intrinsic notion of âˆž-connections</a> in $Smooth \infty Grpd$.


Let still $\mathfrak{g} \in _\infty \stackrel{CE(-)}{\hookrightarrow} dgAlg^{op}$ be an [[L-âˆž algebra]].
...
We say an invariant polynomial $\langle - \rangle$ on $\mathfrak{g}$ is **in transgression** with an [[âˆž-Lie algebra cohomology|L-âˆž algebra cocycle]] $\mu : \mathfrak{g} \to b^{n-1} \mathbb{R}$ if 
...
transgressive [[âˆž-Lie algebra cohomology|L-âˆž algebra cocycles]].
...
For more see [[connection on a smooth principal âˆž-bundle]].
...
* [[scheiber:âˆž-Chern-Simons theory]]
...
For references on  [[Chern-Weil theory in SmoothâˆžGrpd]] and [[connection on a smooth principal âˆž-bundle]], see there.
...
[[!redirects smooth âˆž-groupoid -- structures]]