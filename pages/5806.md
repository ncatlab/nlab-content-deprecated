...
The generalization of the [[bicategory]] _[[Span]]_ to [[(âˆž,n)-categories]]:

An _$(\infty,n)$-category of correspondences_ in [[âˆž-groupoid]] is an [[(âˆž,n)-category]] whose

* [[objects]] are [[âˆž-groupoids]];
...
* [[2-morphisms]] are correspondences of correspondences
  ...
  (where the triangular sub-[[diagram]]s are filled with [[2-morphism]]s in [[âˆžGrpd]] which we do not display here)

* and so on up to [[k-morphism|n-morphism]]s

* $k \gt n$-morphisms are equivalences of order $(k-n)$ of higher correspondences.

Using the symmetric monoidal structure on [[âˆžGrpd]] this becomes a [[symmetric monoidal (âˆž,n)-category]].

More generally, for $C$ some [[symmetric monoidal (âˆž,n)-category]], there is a symmetric monoidal $(\infty,n)$-category of correspondences over $C$, whose

* [[object]]s are [[âˆž-groupoid]]s $X$ equipped with an [[(âˆž,n)-functor]] $X \to C$;

* [[morphism]]s $X \to Y$ are [[correspondences]] in [[(âˆž,1)Cat]] over $C$
...
Even more generally one can allow the [[âˆž-groupoid]]s $X, Y, \cdots$ to be [[(âˆž,n)-categories]] themselves.
...
The [[(âˆž,2)-category]] of correspondences in [[âˆžGrpd]] is discussed in some detail in ([Dyckerhoff-Kapranov 12, section 10](#DyckerhoffKapranov12)).
...
is equivalently the [[(âˆž,n)-category]] of [[En-algebras]] and [[(âˆž,1)-bimodules]] between them in the [[opposite (âˆž,1)-category]] of $\mathbf{H}$ (since every object in a cartesian category is uniquely a [[coalgebra]] by its [[diagonal]] map).
...
For $\mathbf{H}$ an [[(âˆž,1)-topos]] and $\mathcal{C} \in Cat_{(\infty,n)}(\mathbf{H})$ a [[symmetric monoidal (âˆž,n)-category|symmetric monoidal]] [[internal (âˆž,n)-category]] then there is a [[symmetric monoidal (âˆž,n)-category]]
...
$Corr_n(\infty Grpd)$ is a [[symmetric monoidal (âˆž,n)-category|symmetric monoidal]] [[(âˆž,n)-category with duals]]. 
...
The canonical $O(n)$-[[âˆž-action]] on $Corr_n(\infty Grpd)$ induced via prop. \ref{CorrnGrpdHasDuals} by the [[cobordism hypothesis]] (see there at _[the canonical O(n)-action](cobordism+hypothesis#TheCanonicalOnAction)_) is trivial.
...
For $\mathbf{H}$ an [[(âˆž,1)-topos]], then $Corr_n(\mathbf{H})$
is an [[(âˆž,n)-category with duals]].

And generally, for $\mathcal{C} \in SymmMon (\infty,n)Cat(\mathbf{H})$ a [[symmetric monoidal (âˆž,n)-category]] [[internal (âˆž,n)-category|internal]] to $\mathbf{C}$, then $Corr_n(\mathbf{H}_{/\mathbf{C}})$ equipped with the phased tensor product of prop. \ref{InSliceWithPhasedTensorProduct} is an [[(âˆž,n)-category with duals]]

=--

([Haugseng 14, cor. 7.8](#Haugseng14))

Let $Bord_n$ be the [[(âˆž,n)-category of cobordisms]].
...
An explicit definition of the [[(âˆž,2)-category]] of spans in [[âˆžGrpd]] is in section 10 of 
...
An inductive definition of the [[symmetric monoidal (âˆž,n)-category]] $Span_n(\infty Grpd)/C$ of spans of [[âˆž-groupoid]] over a symmetric monoidal $(\infty,n)$-category $C$ is sketched in section 3.2 of 
...
The generalization to an $(\infty,n)$-category $Span_n((\infty,1)Cat^Adj)$ of spans between [[cobordism hypothesis|(âˆž,n)-categories with duals]] is discussed on p. 107 and 108.

The extension to the case when the ambient $\infty$-topos is varied is in

* {#LiBland15} [[David Li-Bland]], _The stack of higher internal categories and stacks of iterated spans_, ([arXiv:1506.08870](http://arxiv.org/abs/1506.08870))
...
[[!redirects (âˆž,n)-category of spans]]
[[!redirects (âˆž,n)-categories of spans]]

[[!redirects (infinity,n)-categories of spans]]

[[!redirects (âˆž,n)-category of correspondences]]
[[!redirects (âˆž,n)-categories of corespondences]]

[[!redirects (infinity,n)-category of correspondences]]
[[!redirects (infinity,n)-categories of correspondences]]

[[!redirects (âˆž,1)-category of correspondences]]
[[!redirects (âˆž,1)-categories of corespondences]]
...
[[!redirects (âˆž,n)-category of n-fold correspondences]]
