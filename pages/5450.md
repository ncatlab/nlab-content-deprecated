...
In the context of the [[geometry]] of [[schemes]] there is a traditional notion of [[Ã©tale morphism of schemes]] and an _Ã©tale topos_ is a [[category of sheaves]] on the [[Ã©tale site]] of a [[scheme]], consisting of [[covers]] by such [[Ã©tale morphism of schemes|Ã©tale morphisms]]. This traditional notion we discuss in 

* [Ã‰tale topos of a scheme](#EtaleToposOfAScheme).

More abstractly, given that [[Ã©tale morphisms of schemes]] may be characterized as modal morphisms with respect to an [[infinitesimal shape modality]], one can consider Ã©tale toposes in every context of [[differential cohesion]]. This we discuss in 

* [Ã‰tale topos of a differentially cohesive object](#GeneralAbstract)

### Ã‰tale topos of a scheme
 {#EtaleToposOfAScheme}

An **Ã©tale topos** is the [[sheaf topos]] over an [[Ã©tale site]], hence over a site whose "open subsets" are [[Ã©tale morphisms]] into the base [[space]]. The intrinsic [[cohomology]] of an Ã©tale [[(âˆž,1)-topos]] is _[[Ã©tale cohomology]]_.

More generally there is the pro-Ã©tale topos over a [[pro-Ã©tale site]], which is a bit better behaved. In particular the intrinsic [[cohomology]] of a pro-Ã©tale [[(âˆž,1)-topos]] includes the [[Weil cohomology theory]] [[â„“-adic cohomology]].

Generally, given that an [[Ã©tale morphism of schemes]] is a [[formally Ã©tale morphism]] subject to a size constraint on its [[fibers]] -- for an actual [[Ã©tale morphism of schemes]] the fibers are [[finite sets]] in the suitable sense (formal duals to [[Ã©tale algebras]]) while for a [[pro-Ã©tale morphism of schemes]] they are [[pro-objects]] of such fibers --
in a suitable ambient context ("[[differential cohesion]]") one can drop all finiteness conditions and consider just opens given by [[formally Ã©tale morphisms]] as encoded by an [[infinitesimal shape modality]]. This we discuss [below](#GeneralAbstract).

### Ã‰tale topos of a differentially cohesive object
 {#GeneralAbstract}

We discuss how in [[differential cohesion]] $\mathbf{H}_{th}$ every object $X$ canonically induces its Ã©tale topos $Sh_{\mathbf{H}_{th}}(X)$.
...
The resulting structure is essentially that discussed ([Lurie, Structured Spaces](#Lurie)) if we regard $\mathbf{H}_{th}$ equipped with its formally Ã©tale morphisms, ([def.](differential+cohesive+%28infinity%2C1%29-topos#FormallyEtaleInHTh)), as a ([[large category|large]]) [[geometry for structured (âˆž,1)-toposes]]. 
...
for the full [[sub-(âˆž,1)-category]] of the [[slice (âˆž,1)-topos]] over $X$ on those maps into $X$ which are formally &eacute;tale, (see [def.](differential+cohesive+%28infinity%2C1%29-topos#FormallyEtaleInHTh)).
...
is an [[(âˆž,1)-pullback]] square. But since $\mathbf{\Pi}_{inf}$ is a [[left adjoint]] it commutes with the $(\infty,1)$-colimit on objects and hence this diagram is equivalent to 
...
This diagram is now indeed an [[(âˆž,1)-pullback]] by the fact that we have [[universal colimits]] in the [[(âˆž,1)-topos]] $\mathbf{H}_{th}$, hence that on the left the component $Y_i$ for each $i \in I$ is the [[(âˆž,1)-pullback]] of $\mathbf{\Pi}_{inf}(Y_i) \to \mathbf{\Pi}_{inf}(X)$, by assumption that we are taking an $(\infty,1)$-colimit over formally &eacute;tale morphisms.
...
By prop. \ref{EtalificationIsCoreflection} the inclusion $(\mathbf{H}_{th})_{/X}^{fet} \hookrightarrow (\mathbf{H}_{th})_{/X}$ is [[reflective sub-(infinity,1)-category|reflective]] with reflector given by the $(\mathbf{\Pi}_{inf}-equivalences , \mathbf{\Pi}_{inf}-closed)$ factorization system. Since $\mathbf{\Pi}_{inf}$ is a [[right adjoint]] and hence in particular preserves [[(âˆž,1)-pullbacks]], the $\mathbf{\Pi}_{inf}$-equivalences are stable under pullbacks. By the discussion at _[[stable factorization system]]_ this is the case precisely if the corresponding reflector preserves [[finite (âˆž,1)-limits]]. Hence the embedding is a [[geometric embedding]] which exhibits a [[sub-(âˆž,1)-topos]] inclusion.
...
also as the ([[petit (âˆž,1)-topos|petit]]) [[(âˆž,1)-topos]] of $X$, or the **Ã©tale topos** of $X$.
...
it preserves [[(âˆž,1)-limits]]. Therefore this is a [[structure sheaf]] which exhibits $Sh_{\mathbf{H}_{th}}(X)$ as a [[structured (âˆž,1)-topos]] over $\mathbf{H}_{th}$ regarded as a (large) [[geometry (for structured (âˆž,1)-toposes)]], with the formally Ã©tale morphisms being the "admissible morphisms".
...
### Sheaf condition and examples of Ã©tale sheaves
...
1. single surjective/[[Ã©tale morphism of schemes|Ã©tale]] morphisms between [[affine schemes]]
...
For $Z \to X$ any [[scheme]] over a [[scheme]] $X$, the induced [[presheaf]] on the [[Ã©tale site]]
...
Unique representatives are given by [[Ã©tale morphism of schemes|Ã©tale schemess]] over $X$.
...
For $f \colon X \longrightarrow Y$ a [[homomorphism]] of [[schemes]], there is induced a [[functor]] on the [[categories]] underlying the [[Ã©tale site]]
...
and hence induces a [[geometric morphism]] between the Ã©tale toposes
...
For $X_{et}$ an [[Ã©tale site]], write $\mathcal{D}(X_{et})$ for the [[derived category]] of the [[abelian category]] $Ab(Sh(X_{et}))$ of [[abelian sheaves]] on $X$.
...
where on the right we have the degree $q$ [[abelian sheaf cohomology]] [[cohomology group|group]] with [[coefficients]] in the given $\mathcal{F}$ ([[Ã©tale cohomology]]).
...
For the special case that $S_Z = \ast$ and $g^{-1}$ includes an [[Ã©tale morphism of schemes|Ã©tale morphism]] $U_Y \to Y$ this yields
...
For $X$ a [[scheme]] and $N$ a [[quasicoherent module]] over its [[structure sheaf]] $\mathcal{O}_X$, then this induces an [[abelian sheaf]] on the [[Ã©tale site]] by
...
A [[cover]] in the [[Zariski topology]] on [[schemes]] is an [[open immersion of schemes]] and hence is in particular an [[Ã©tale morphism of schemes]]. Hence the [[Ã©tale site]] is finer than the [[Zariski site]] and so every Ã©tale [[sheaf]] is a Zariski sheaf, but not necessarily conversely. Expressed in a different way, the Ã©tale topos is a [[subtopos]] of the Zariski topos.

=--


For more see at _[Ã©tale cohomology -- Properties -- Relation to Zariski cohomology](etale+cohomology#RelationZariskiEtaleCohomology)_.

### As a classifying topos

The Ã©tale topos over the big Ã©tale site of [[commutative rings]] is the [[classifying topos]] for [[strict local rings]].
...
* [[GÃ¼nter Tamme]], section II 1 of _[[Introduction to Ã‰tale Cohomology]]_
 {#Tamme}
...
[[!redirects Ã©tale topos]]
[[!redirects Ã©tale toposes]]
[[!redirects Ã©tale topoi]]

[[!redirects Ã©tale (âˆž,1)-topos]]
[[!redirects etale (âˆž,1)-topos]]
[[!redirects Ã©tale (âˆž,1)-toposes]]
[[!redirects etale (âˆž,1)-toposes]]

[[!redirects Ã©tale (infinity,1)-topos]]

[[!redirects etale (infinity,1)-topos]]
[[!redirects Ã©tale (infinity,1)-toposes]]
