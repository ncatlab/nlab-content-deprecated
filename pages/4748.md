...
Generally, a [[connection on an âˆž-bundle]] induces a notion of parallel transport in arbitrary dimension.


## Definition

The higher notions of [[differential cohomology]] and [[Chern-Weil theory]] make sense in any [[cohesive (âˆž,1)-topos]] 
...
In every such there is a notion of [[connection on an âˆž-bundle]] and of its higher parallel transport.

A typical context considered (more or less explicitly) in the literature is $\mathbf{H} = $ [[âˆžLieGrpd]], the cohesive $(\infty,1)$-topos of [[âˆž-Lie groupoid|smooth âˆž-groupoids]]. But other choices are possible. (See also the [Examples](#Examples).)


### Higher parallel transport

Let $A$ be an [[âˆž-Lie groupoid]] such that morphisms $X \to A$ in [[âˆžLieGrpd]] classify the $A$-[[principal âˆž-bundle]]s under consideration. Write $A_{conn}$ for the differential refinement described at [[âˆž-Lie algebra valued form]], such that lifts
...
describe [[connections on âˆž-bundles]].
...
Here $\mathbf{\flat}A = [\mathbf{\Pi}(-),A]$ is the coefficient for [[schreiber:path âˆž-groupoid|flat differential A-cohomology]].
...
* the $n$-form takes values not in $\mathbb{R}$ but more generally is an [[âˆž-Lie algebroid valued differential form]].
...
in [[nLab:âˆžGrpd]] we call the **$\infty$-Chern-Simons action** on $\Sigma$.
...
Even though it is a degenerate case, it can be useful to regard the [[(âˆž,1)-topos]] [[Top]] explicitly a [[cohesive (âˆž,1)-topos]]. For a discussion of this see [[discrete âˆž-groupoid]].



For $\mathbf{H} = $ [[Top]] lots of structure of cohesive $(\infty,1)$-topos theory  degenerates, since by the [[homotopy hypothesis]]-theorem here the [[global section]] [[(âˆž,1)-geometric morphism]]
...
an [[equivalence in an (âˆž,1)-category|equivalence]]. The abstract [[fundamental âˆž-groupoid in a locally âˆž-connected (âˆž,1)-topos]] $\Pi$ is here the ordinary [[fundamental âˆž-groupoid]] 
...
If both [[(âˆž,1)-topos]]es here are [[presentable (âˆž,1)-category|presented]] by their standard [[model category]] models, the standard [[model structure on simplicial sets]] and the standard [[model structure on topological spaces]], then $\Pi$ is presented by the [[singular simplicial complex]] functor in a [[Quillen equivalence]]
...
For instance: for $F \in Top$ and $Aut(F) \in Top$ its [[automorphism âˆž-group]], $F$-fibrations over a base space $X \in Top$ are classfied by morphisms
...
in [[Top]], as described at [[principal âˆž-bundle]].

Using the [[fundamental âˆž-groupoid]] functor we may send this equivalently to a [[fiber sequence]] in [[âˆžGrpd]]
...
We may embed this example into the smooth context by regarding $Aut(F)$ as a [[discrete space|discrete]] [[âˆž-Lie groupoid]] as discussed in the section
[Flat âˆž-Parallel transport in âˆžLieGrpd](#InLieGrpd).
...
be the [[global section]] [[(âˆž,1)-geometric morphism]] of the  [[cohesive (âˆž,1)-topos]] [[âˆžLieGrpd]].

We may reflect the [[âˆž-group]] $Aut(F)$ into this using the [[constant âˆž-stack]]-functor $Disc$ to get the discrete [[âˆž-Lie group]] $Disc Aut(F)$. Let then $X$ be a [[paracompact topological space|paracompact]] [[smooth manifold]], regarded naturally as an object of [[âˆžLieGrpd]]. Then we can consider [[cocycle]]s/classifying morphisms
...
The [[âˆž-groupoid]] of $F$-fibrations in [[Top]] is equivalent to the $\infty$-groupoid of $Disc Aut(F)$-[[principal âˆž-bundle]]s in [[âˆžLieGrpd]]:
...
Moreover, all the [[principal âˆž-bundle]]s classified by the morphisms on the left have canonical extensions to <a href="http://nlab.mathforge.org/nlab/show/cohesive+(infinity%2C1)-topos#FlatDifferentialCohomology">Flat differential cohomology</a> in $\infty LieGrpd$, in that the flat parallel $\infty$-transport $\nabla_{flat}$ in
...
The first statement is a special case of that spelled out at [[âˆžLieGrpd]] and [[nonabelian cohomology]]. The second follows using that in a [[âˆž-connected (âˆž,1)-topos|connected]] [[locally âˆž-connected (âˆž,1)-topos]] the functor $Disc$ is a [[full and faithful (âˆž,1)-functor]].
...
A typical choice for an [[(âˆž,1)-category]] of "$\infty$-vector spaces" is that [[presentable (âˆž,1)-category|presented]] by the a [[model structure on chain complexes]] of modules. In a geometric context this may be replaced by some stack of complexes of vector bundles over some site. 
...
A discussion of how to integrate flat differential forms with values in chain complexes -- a representation of the [[tangent Lie algebroid]] as discussed at [[representations of âˆž-Lie algebroids]] --
...
* [[JoÃ£o Faria Martins]], [[Roger Picken]], _On 2-dimensional holonomy_ ([arXiv](http://arxiv.org/abs/0710.4310))
{#MartinsPickenI}

* [[JoÃ£o Faria Martins]], [[Roger Picken]], _The fundamental Gray 3-groupoid of a smooth manifold and local 3-dimensional holonomy based on a 2-crossed module_ ([arXiv](http://arxiv.org/abs/0907.2566))
{#MartinsPickenII}
...
* [[Kiyonori Gomi]] and Yuji Terashima, _Higher dimensional parallel transport_ Mathematical Research Letters 8, 25â€“33 (2001) ([pdf](http://mrlonline.org/mrl/2001-008-001/2001-008-001-004.pdf))
...
* [[NersÃ©s Aramyan]], Research statement ([pdf](http://math.illinois.edu/~aramyan2/research.pdf))
