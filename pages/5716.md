Welcome on my personal webpage at the nlab.

# Position {#position}

I am currently mątre de conf̩rence (assistant professor) of
[University Paris 6](http://www.upmc.fr/) at
[Jussieu's mathematical institute](http://www.institut.math.jussieu.fr/)
in the
[Algebraic analysis group](http://www.institut.math.jussieu.fr/projets/aa/).


# Research {#myresearch}

I have two main domains of interest:

Mathematics of quantum field theory: i wrote a book whose aim was to give a coordinate free (i.e., categorical) presentation of many tools (and theories) used in the classical and quantum aspects of field theory:

* [Towards the mathematics of quantum field theory](http://people.math.jussieu.fr/~fpaugam/documents/enseignement/master-mathematical-physics.pdf).

I didn't succeed in fulfilling my curiosity in this domain.

Arithmetic geometry: i did my Phd thesis on Abelian varieties, Galois representations and Shimura varieties. My recent interests are related to [[global analytic geometry]], [[mixed geometry]] and [[global Hodge theory]], with views towards applications to
the geometric study of (analytic) number theory. You can find a presentation of some motivations in the master course notes
(preliminary form):

* [Global analytic geometry and the functional equation](http://people.imj-prg.fr/~frederic.paugam/documents/enseignement/master-global-analytic-geometry.pdf)

and at [[global Analytic geometry]].

I think that the combination of the three above tools may be quite useful to better understand the geometric meaning of functional symmetries.

# Webpage {#contact} 

You can find further informations on me (publications, preprints, etc...) on my
[academic webpage](http://www.imj-prg.fr/~frederic.paugam/parts-en/index.html).


[[!redirects Frederic Paugam]]

category: people

