...
Integral Morva K-theory can be obtained as a localization of a quotient $MU/I$ of [[complex cobordism cohomology theory]] $MU$ ([BuhnÃ© 11](#Buhne11)).
...
1. There is a [[KÃ¼nneth isomorphism]]: $K(n)_\ast(X\times Y)\cong K(n)_\ast(X)\otimes_{K(n)_\ast(pt.)}K(n)_\ast(Y).$
...
Due to the third point one may regard $K(n)$ as a [[âˆž-field]] among the [[A-infinity rings]]. See [below](#AsInfinityFields).
...
For each [[prime number]] $p$ and each $n \in \mathbb{N}$, the Morava K-theory $K(n)$ is, up to [[equivalence in an (âˆž,1)-category|equivalence]], the unique [[spectrum]] underlying an [[H-space|homotopy associative]] spectrum which is
...
$K(n)$ admits the structure of an [[A-âˆž algebra]], in fact of an $MU_{(p)}$-[[A-âˆž algebra]]. 
...
This means that the Morava $A_\infty$-rings $K(n)$ are essentially the only [[âˆž-fields]] in the [[stable homotopy category]].
...
The Morava K-theories label the [[prime spectrum of a symmetric monoidal stable (âˆž,1)-category]] of the [[(âˆž,1)-category of spectra]] for [[p-localization|p-local]] and [[finite spectra]] . This is the content of the _[[thick subcategory theorem]]_.
...
Write $gl_1(K(n))$ for the [[âˆž-group of units]] of the (a) Morava K-theory spectrum
...
* the [[mapping space]] from the [[classifying space]] for [[circle n-bundle|circle (n+1)-bundles]] to the [[delooping]] of the [[âˆž-group of units]] of $K(n)$
...
* David Copeland Johnson, ; W. Stephen Wilson, _BP operations and Morava's extraordinary K-theories._, Math. Z. 144 (1): 55âˆ’75,  (1975)
...
* Urs WÃ¼rgler, _Morava K-theories: a survey_, Algebraic topology Poznan 1989, Lecture Notes in Math. 1474, Berlin: Springer, pp. 111 138 (1991)
...
Some [[twisted cohomology|twists]] of Morava K-theory/maps into its [[âˆž-group of units]] as well as the [[Atiyah-Hirzebruch spectral sequence]] for Morava $K$ and Morava $E$ are discussed in 
