...
For $T$ any abelian [[Lawvere theory]], here we discuss -- in a variation of the theme of [[Isbell conjugation]], in generalization of ([ToÃ«n](#Toen)) and following ([Stel](#Stel)) --  a [[simplicial Quillen adjunction]] between [[model category]] structures on cosimplicial $T$-algebras and on [[simplicial presheaves]] over duals of $T$-algebras. We find mild general conditions under which this descends to the local model structure that models [[âˆž-stacks]] over duals of $T$-algebras. In these cases the left adjoint of the Quillen adjunction is given by sending $\infty$-stacks to their cosimplicial $T$-algebras of functions with values in the canonical $T$-[[line object]], and the adjunction models small objects relative to a choice of a small full subcategory $T\subset C \subset T Alg^{op}$  of the localization 
...
For the special case where $T$ is the theory of ordinary commutative algebras this reproduces the situation of ([ToÃ«n](#Toen)) and many statements are straightforward generalizations from that situation. For the case that $T$ is the theory of _[[smooth algebra]]s_ ($C^\infty$-rings) we obtain a refinement of this to the context of synthetic differential geometry. In these cases, in as far as objects in $\mathbf{H}$ may be understood as [[âˆž-Lie groupoids]], the objects in $\mathbf{L}$ may be understood as [[âˆž-Lie algebroids]]. 
...
A good general notion of [[function algebras]] on generalized [[spaces]] are $T$-algebras, for $T$ a [[Lawvere theory]]. A good general notion of [[function algebras]] on [[internal âˆž-groupoid]]s in such spaces are [[cosimplicial object|cosimplicial]] $T$-algebras.
...
+-- {: .num_theorem #ModelTransferToTAlg}
###### Theorem
...
A good notion of a generalized [[space]] modeled on objects in a category $C$ is a [[nLab:sheaf]] on $C$. A good notion of an [[nLab:âˆž-groupoid]] in such generalized spaces is an [[(âˆž,1)-sheaf]] on $C$. Such objects are modeled by the [[model structure on simplicial presheaves]] on $C$.
...
Using this we can follow the remainder of the argument of [ToÃ«n, prop. 2.2.2](#Toen):
...
With the above results, this follows verbatim as the proof of the analogous  ([ToÃ«n, corollary 2.2.3](#Toen)).
...
In this section we discuss that in terms of the [[(âˆž,1)-category theory]] 
that is [[presentable (âˆž,1)-category|presented]] by the model category theoretic structures [above](#Models), these serve to establish the following intrinsic statement.

+-- {: .num_theorem}
###### Theorem

The Quillen adjunction $(\mathcal{O} \dashv j)$ is a [[nLab:presentable (âˆž,1)-category|presentation]] that models $C$-small objects (...) in the [[reflective sub-(âˆž,1)-category]]
...
of the [[nLab:(âˆž,1)-category of (âˆž,1)-sheaves]] $Sh_{(\infty,1)}(C)$, where $\mathbb{L}_C$ is  the [[nLab:localization of an (âˆž,1)-category|localization]] at those morphisms that induce isomorphisms in intrinsic $R$-[[cohomology]], for $R$ the [canonical T-line object](#Line).
...
Since $T$ is assumed to be an abelian Lawvere theory, the [T-line object](#Line) $R \in [C^{op}, sSet]$ canonically has the structure of an abelian [[group object]] in $[C^{op}, sSet]$. As such it presents a 0-[[truncated]] [[âˆž-group]] in the $Sh_{(\infty,1)}(C)$, and so we may consider its [[Eilenberg-MacLane object]]s $\mathbf{B}^n R$ for $n \in \mathbb{N}$.
...
Every [[(âˆž,1)-topos]] such as $\mathbf{H} = Sh_{(\infty,1)}(C)$ comes with its [[cohomology|intrinsic notion of abelian cohomology]]: for $X \in \mathbf{H}$ any object and for $A \in \mathbf{H}$ a [[âˆž-group]] object with arbitrary [[delooping]]s $\mathbf{B}^n A$, the $n$th cohomology group of $X$ with coefficients in $A$ is
...
This is essentially the argument of ([ToÃ«n, corollary 2.2.6](#Toen)).
...
By prop. \ref{RCohomologyByO} this is equivalent to 
...
([ToÃ«n, theorem 2.2.9](#Toen)) goes through verbatim: it only uses the general properties of the $(\mathcal{O} \dashv j)$-adjunction that we have established above, as well as the fact that $T Alg^{\Delta}_{proj}$ is a 
[[cofibrantly generated model category]] for $T$ the theory of ordinary
commutative algebras. But by our [result on the model structure on TAlg](#ModelTransferToTAlg) (theorem \ref{ModelTransferToTAlg}) we have that for general $T$ this is the [[transferred model structure]] of the [[model structure on cosimplicial abelian groups]], which is cofibrantly generated. Hence by the general properties of transferred model structures, also $T Alg^\Delta_{proj}$ is.

But more abstractly, we can also simply use the general theory of [[reflective sub-(âˆž,1)-categories]] and their characterization as the reflective [[localization of an (âˆž,1)-category]] at a set of weak equivalences: from the above we know that on the full sub-$(\infty,1)$-category of $((T Alg^\Delta_{proj})^{op})^\circ$ on the objects in $C^{\Delta^{op}} \hookrightarrow (T Alg^\Delta)^{op}$ is a reflective sub-$(\infty,1)$-category 
...
We now discuss [[function algebras]] on $\infty$-stacks more generally in the context of [[derived geometry]], meaning that we we pass in the above from sites inside the opposite of a 1-category of $T$-algebras to an [[(âˆž,1)-site]] inside the opposite of an [[(âˆž,1)-category]] of [[âˆž-algebras over an (âˆž,1)-algebraic theory]].
...
Write $(cdgAlg_k^{op})^\circ$ for the [[(âˆž,1)-category]] that is 
[[presentable (âˆž,1)-category|presented]] by the [[model structure on dg-algebras|model structure on unbounded commutative cochain dg-algebras]] over $k$.
...
for the full [[sub-(âˆž,1)-category]] on cochain dg-algebras concentrated in non-positive degree.

=--

Let $C \subset (cdgAlg_k^{op})^\circ_-$ be a [[small (âˆž,1)-category|small]] full sub-$(\infty,1)$-category equipped with the structure of a [[subcanonical coverage|subcanonical]] [[(âˆž,1)-site]].
...
By the [[(âˆž,1)-co-Yoneda lemma]] we may express any $X \in Sh_{(\infty,1)}(C)$ by an [[(âˆž,1)-colimit]] over representables
...
The functor $\mathcal{O}$ simply evaluates this colimit in $((cdgAlg_k^{op})^\circ$, which is the [[(âˆž,1)-limit]] in the [[opposite (âˆž,1)-category]]
...
By construction $\mathcal{O}$ is a colimit-preserving $(\infty,1)$-functor between [[locally presentable (âˆž,1)-categories]]. Accordingly, by the [[adjoint (âˆž,1)-functor theorem]] is has a [[right adjoint|right]] [[adjoint (âˆž,1)-functor]].
...
* $T$ the theory of ordinary commutative algebras over a field $k$ and $J$ the [[fpqc topology]].
  ...
  In this case the adjunction is that considered in ([ToÃ«n](#Toen)).
...
$T$ the Lawvere theory of $\mathbb{Q}$-algebras. Then $(\mathcal{O} \dashv j)$ reproduces the setup discussed at [[rational homotopy theory in an (âˆž,1)-topos]].
...
The **$(\infty,1)$-Cahiers-topos** is the [[(âˆž,1)-category of (âˆž,1)-sheaves]] on [[ThCartSp]] with respect to the good open cover coverage.
...
By the discussioin at <a href="http://ncatlab.org/nlab/show/model+structure+on+simplicial+presheaves#LocalizationAtCoverage">Cech localization of simplicial presheaves at a coverage</a>, the analogous statement holds true for the [[(âˆž,1)-topos]]es over these sites. 
...
The objects of the $(\infty,1)$-Cahiers topos we call **synthetic differential** [[âˆž-Lie groupoid]]s.
...
we call **[[âˆž-Lie algebroid]]**s. 

A [[connected]] $\infty$-Lie algebroid we call an **[[âˆž-Lie algebra]]**.
...
Passing along the embedding $\mathbf{L} \hookrightarrow \mathbf{H}$ we may compute [[âˆž-Lie algebra cohomology]] in $\mathbf{H}$.
...
* [[geometric stack]], [[geometric âˆž-stack]]

## References

The Quillen adjunction over abelian $T$-algebras that we consider generalizes that discussed in 

* {#Toen}[[nLab:Bertrand ToÃ«n]], _Champs affine_ ([arXiv:math/0012219](http://arxiv.org/abs/math/0012219))
...
[[!redirects function algebras on âˆž-stacks]]

[[!redirects rational homotopy theory in an (âˆž,1)-topos]]
