...
the equivalence relation â€œ$x$ is infinitesimally close to $y$â€, which is
...
-   The process of taking points of hyperimaginaries in models
    ...
    infinitesimally close together, itâ€™s easy to see that
...
This is actually an excellent example of why we shouldnâ€™t expect in
...
depend on $\varphi$. They arenâ€™t uniform, and thereâ€™s possibly no set
$P'$ in the ultrafilter (the intersection of all the $P$â€™s might be
...
these two procedures should fail precisely when the $P$â€™s on the left
...
hyperimaginaries in $^*\mathbb{R}$, since for each condition â€œx is less
than $1/n$ away from yâ€, these two sequences agree cofinitely many times
