...
Extending BÃ©nabou's observation that a [[small category]] is a monad in the bicategory of spans of sets, [[Burroni]] defined  $T$-multicategories as _monads in the bicategory_ $\mathcal{E}_{(T)}$ from above. When $T$ is the [[identity monad]] on $\mathrm{Set}$, $T$-multicategories reduce to small categories, and when $T$ is the free monoid monad on $\mathrm{Set}$, $T$-multicatories are exactly ordinary small multicategories.

As an indication of how this theory is useful as a language for higher categories, take $T$ to be the free [[strict Ï‰-category]] monad on the category of [[globular set]]s. Then $T$-multicategories with exactly one object are called [[globular operad]]s, and Leinster defines one such globular operad (the initial "globular operad with contraction") for which the algebras are [[weak Ï‰-categories]].
...
* [[Albert Burroni]], _$T$-catÃ©gories (catÃ©gories dans un triple)_, Cahiers de Topologie et GÃ©omÃ©trie DiffÃ©rentielle CatÃ©goriques, 12 no. 3 (1971), p. 215-321 ([numdam](http://www.numdam.org/item?id=CTGDC_1971__12_3_215_0))
