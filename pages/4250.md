...
This is a sub-entry of [[homotopy groups in an (âˆž,1)-topos]]. 

For the other notion of homotopy groups see [[geometric homotopy groups in an (âˆž,1)-topos]].
...
Recall that since an [[(âˆž,1)-topos]] $\mathbf{H}$ has all [[limits in a quasi-category|limits]],  it is naturally
[powered over âˆžGrpd](http://ncatlab.org/nlab/show/limit+in+a+quasi-category#Tensoring):
...
Let $S^n = \partial \Delta[n+1]$ (or  $S^n := Ex^\infty \partial \Delta[n+1]$) be the ([[Kan fibrant replacement]]) of the [[boundary of a simplex|boundary of the (n+1)-simplex]], i.e. the model in [[âˆžGrpd]] of the [[pointed object|pointed]] $n$-[[sphere]].

Then for $X \in \mathbf{H}$ an object, the power object $X^{S^n} \in \mathbf{H}$ plays the role of the space of of maps from the $n$-sphere into $X$, as in the definition of [[simplicial homotopy groups]], to which this reduces in the case that $\mathbf{H} = $ [[âˆžGrpd]]. 
...
which is restriction to the basepoint. This morphism may be regarded as an object of the [[over quasi-category|over]] [[(âˆž,1)-topos]] $\mathbf{H}_{/X}$.
...
For $f : X \to Y$ a [[morphism]] in an [[(âˆž,1)-topos]] $\mathbf{H}$, its _homotopy groups_ are the homotopy groups in the above sense of $f$ regarded as an object of the [[over quasi-category|over (âˆž,1)-category]] $\mathbf{H}_{/Y}$.

=--

So the homotopy sheaf $\pi_n(f)$ of a morphism $f$ is an object of the [[over quasi-category|over (âˆž,1)-category]] $Disc((\mathbf{H}_{/Y})_{/f}) \simeq Disc(\mathbf{H}_{/f})$. This in turn is equivalent to $\cdots \simeq \mathbf{H}_{/X}$ by the map that sends an object
...
* The object $X$ is **$n$-[[truncated]]** if it is a [[n-truncated object in an (âˆž,1)-category|k-truncated object]] for some $k \gt n$ and if all its categorical homotopy groups above degree $n$ vanish.
...
When the [[(âˆž,1)-topos]] $\mathbf{H}$ is [[presentable (âˆž,1)-category|presented]] by a [[model structure on simplicial presheaves]] $[C^{op}, sSet]_{loc}$, then since this is an [[sSet]]-[[enriched model category]] structure the [[power]]ing by $\infty Grpd$ is modeled, as described at, <a href="http://ncatlab.org/nlab/show/limit+in+a+quasi-category#ModelsForTensoring">$(\infty,1)$-limit -- Tensoring -- Models</a> by the ordinary powering 
...
[[!redirects categorical homotopy groups in an (âˆž,1)-topos]]
