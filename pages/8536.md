...
In [[combinatory logic]], in the [[Î»-calculus]], or more generally in [[type theory]], a *looping [[combinator]]* is closely akin to a [[fixed-point combinator]], but rather than producing a true [[fixed point]], it produces a sequence of points each of which is the [[image]] of the next.
...
[[Per Martin-LÃ¶f]]'s original [[dependent type theory]], which had the additional rule $\vdash Type:Type$, was shown to be [[inconsistency|inconsistent]] by [[Girard's paradox]].  In the 1980's, Meyer, Reinhold, and Howe (see references) showed that this paradox could be modified to construct a looping combinator.
