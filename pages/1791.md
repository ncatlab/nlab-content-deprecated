...
The [[model category|model structure]] $sPSh(C)$ on [[sSet]]-[[enriched functor category|enriched presheaves]] is supposed to be a [[presentable (infinity,1)-category|presentation]] of the [[(âˆž,1)-category of (âˆž,1)-sheaves]] on an [[sSet-site]] $C$.
...
This means that in as far as the [[model structure on simplicial presheaves]] models [[âˆž-stacks]], the model structure on [[sSet]]-[[enriched category|enriched categories]] model [[derived stack]]s.
...
This appears on ([ToÃ«nVezzosi, page 14](#ToenVezzosi)).
...
For an [[sSet-site]] $C$ regarded as an [[(âˆž,1)-site]], the local model structure on $[C^{op}, sSet]$ is a [[presentable (âˆž,1)-category|presentation]] of the [[(âˆž,1)-category of (âˆž,1)-sheaves]] on $C$, in that there is an [[equivalence of (âˆž,1)-categories]]
...
Where a [[topos]] or [[(âˆž,1)-topos]] over an ordinary [[site]] encodes [[higher geometry]], over a genuine [[sSet-site]] one speaks of [[derived geometry]]. An [[âˆž-stack]] on such a higher site is also called a [[derived stack]]. 

Therefore the model structure on $sSet$-presheaves serves to model contexts of derived geometry. For instance over the [[etale (âˆž,1)-site]].
...
* [[Bertrand ToÃ«n]], [[Gabriele Vezzosi]], _Topos Theory_ ([arXiv:math/0207028](http://arxiv.org/abs/math/0207028))
{#ToenVezzosi}

* [[Bertrand ToÃ«n]], [[Gabriele Vezzosi]], _Segal topoi and stacks over Segal categories_ Proceedings of the Program _Stacks, Intersection theory and Non-abelian Hodge Theory_ , MSRI, Berkeley, January-May
2002 ([arXiv:0212330](http://arxiv.org/abs/math/0212330))

The relation to intrinsically defined [[(âˆž,1)-topos theory]] is around remark 6.5.2.15 of
