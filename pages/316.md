...
This definition generalizes to [[higher category theory]]. In particular in [[(âˆž,1)-category theory]] a cone over an [[âˆž-groupoid]] is essentially a cone in the sense of [[homotopy theory]].
...
For $F : D \to C$ a [[diagram]] of [[(âˆž,1)-categories]], i.e. an [[(âˆž,1)-functor]], the $(\infty,1)$-category of $(\infty,1)$-cones over $F$ is the [[over quasi-category]] denoted $C_{/F}$. Its objects are cones over $F$. Its [[k-morphism]]s are $k$-homotopies between cones. The [[limit in a quasi-category|(âˆž,1)-categorical limit]] over $F$ is, if it exists, the [[initial object]] in $C_{/F}$.
