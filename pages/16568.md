...
This means that for $\mathbf{H}$ a [[cohesive (âˆž,1)-topos]] of geometric spaces of sorts, then the corresponding $\mathbb{G}$-prequantum geometry is that encoded by the [[slice (âˆž,1)-topos]] $\mathbf{H}_{/\mathbf{B}\mathbb{G}_{conn}}$. Hence prequantum geometry is the geometry in slices over [[higher moduli stacks]] for [[differential cohomology]].
...
If we regard $\mathbb{R}^{2n} \simeq T^\ast \mathbb{R}^n$ as the [[cotangent bundle]] of the [[Cartesian space]] $\mathbb{R}^n$, then this is what is known as the [[Liouville-PoincarÃ© 1-form]].
...
Recall the definition of [[L-âˆž algebras]].
...
define an [[L-âˆž algebra]] $\mathfrak{poiss}(X,\omega)$, to be called the _[[Poisson bracket Lie (p+1)-algebra]]_ as follows.
...
Definition \ref{PoissonBracketLienAlgebra} indeed gives an [[L-âˆž algebra]] in that the higher [[Jacobi identity]] is satisfied.
...
The **ÄŒech cochain complex** $C^\bullet((X,\{U_i\}),A_\bullet)$ 
...
Let $\overline{\theta}$ be any [[Cech cohomology|Cech]]-[[Deligne cohomology|Deligne]]-[[cocycle]] relative to an [[open cover]] $\mathcal{U}$ of $X$, which gives a [[prequantum n-bundle]] for $\omega$. The [[L-âˆž algebra]] 
...
There is an [[equivalence]] in the [[model structure for L-âˆž algebras|homotopy theory of L-âˆž algebras]]
...
for the truncated [[de Rham complex]] regarded as an abelian [[L-âˆž algebra]].
...
$\mathfrak{poiss}(X,\omega)$, def. \ref{PoissonBracketLienAlgebra}, is an [[L-âˆž extension]] of the [[Hamiltonian vector fields]] by the truncated de Rham complex,
...
This is the proper [[moduli stack]] of $\mathbb{G}$-[[principal âˆž-connections]] on $X$ in that a family $U \longrightarrow \mathbb{G}\mathbf{Conn}(X)$ is a _vertical_ $\mathbb{G}$-principal $\infty$-connection on $U \times X\to U$.

+-- {: .num_example }
###### Example

For $\mathbf{H} = $[[SmoothâˆžGrpd]] or =[[FormalSmoothâˆžGrpd]], for $\mathbb{G} = \mathbf{B}^p U(1)$ the [[circle n-group|circle (p+1)-group]] with its standard Hodge filtration as in example \ref{StandardChoiceOfBGconn}, then for $X$ any [[smooth manifold]] or [[formal smooth manifold]], $(\mathbf{B}^p U(1))\mathbf{Conn}(X)$ is presented via the [[Dold-Kan correspondence]] by the sheaf $U \mapsto Ch_\bullet$ of [[vertical differential form|vertical]] [[Deligne complexes]] on $U \times X$ over $U$. 
...
For $\mathbb{G} \simeq \mathbf{B}\mathbb{G}'$ then the [[loop space object]] of the moduli stack of $\mathbb{G}$-principal $\infty$-connections on $X$ is the moduli stack of [[flat âˆž-connections]] with gauge group $\Omega \mathbb{G}$
...
The canonical [[precomposition action|precomposition]] [[âˆž-action]] of the [[automorphism âˆž-group]] $\mathbf{Aut}(X)$ on $[X,\mathbf{B}\mathbb{G}_{conn}]$ passes along $conc$ to an [[âˆž-action]] on $\mathbb{G}\mathbf{Conn}(X)$.
...
1. The **[[Hamiltonian symplectomorphism âˆž-group]]** 
...
1. A **[[Hamiltonian action]]** of an [[âˆž-group]] $G$ on $(X,\nabla)$ is an [[âˆž-group]] homomorphism 
...
1. An **[[âˆž-moment map]]**  is an $\infty$-group homomorphism 

   $$
     G \longrightarrow \mathbf{QuantMorph}(X,\nabla)
   $$

1. The **[[Heisenberg âˆž-group]]** for a given Hamiltonian $G$-action $\rho$ is the [[homotopy pullback]] 
...
exhibiting the [[quantomorphism âˆž-group]] as an [[âˆž-group extension]] of the [[Hamiltonian symplectomorphism âˆž-group]] by the [[moduli stack]] of $\Omega \mathbb{G}-$[[flat âˆž-connections]], classified by a [[cocycle]] $\mathbf{KS}$.
...
In $\mathbf{H} = $ [[SmoothâˆžGrpd]], let $\mathbb{G} = \mathbf{B}^p U(1)$ be the [[circle n-group|circle (p+1)-group]] and let $X \in SmoothMfd \hookrightarrow Smooth \infty Grpd$ be [[n-connected topological space|p-connected]], then $(\Omega\mathbf{B}^p U(1))\mathbf{FlatConn}(X)\simeq \mathbf{B}^{p}U(1)$. Hence here prop. \ref{TheQuantomorphismGroupExtension} gives
...
exhibiting the [[Heisenberg âˆž-group]] as an [[âˆž-group extension]] of the $G$ by the [[moduli stack]] of $\Omega \mathbb{G}-$[[flat âˆž-connections]], classified by a [[cocycle]] $\mathbf{KS}(\rho)$.


The class of the [[cocycle]] $\mathbf{KS}(\rho)$ is the [[obstruction]] to prequantizing $\rho$ to a [[moment map]] (the _[[classical anomaly]]_ of $\rho$); and the the [[Heisenberg âˆž-group]] [[âˆž-group extension|extension]] of $G$ is the universal cancellation of this anomaly.
...
* {#AGIT89} [[JosÃ© de AzcÃ¡rraga]], [[Jerome Gauntlett]], J.M. Izquierdo, [[Paul Townsend]], _Topological Extensions of the Supersymmetry Algebra for Extended Objects_, Phys. Rev. Lett. 63 (1989) 2443 ([spire](http://inspirehep.net/record/26393?ln=en))
...
* {#FRS13b} [[Domenico Fiorenza]], [[Chris Rogers]], [[Urs Schreiber]], _[[schreiber:L-âˆž algebras of local observables from higher prequantum bundles]]_, Homology, Homotopy and Applications, Volume 16 (2014) Number 2, p. 107 â€“ 142 ([arXiv:1304.6292](http://arxiv.org/abs/1304.6292))
