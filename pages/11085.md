...
1. [âˆž-Chern-Simons theory as the general example](#InfinityCSAsGeneralExample)
...
* A **[[field (physics)|field]]** theory identifies the basic configurations of a physical system with generalized functions on spacetime, such as [[tensor fields]], and more generally with [[sections]] $\phi$ of some [[fiber bundle]] over spacetime
  ...
  (This is the traditional formulation. Below we discuss that the concept of _field [[fiber bundle]]_ needs to be refined to that of field _[[fiber âˆž-bundle]]_ in order to accuratly capture all aspects of field theory:   _[The traditional idea of field bundles and its problems](#IdeaOfFieldBundlesAndItsProblems)_.)
...
The higher refinement of groupoids which we need are _[[âˆž-groupoids]]_. The collection of these is equivalently that of [[simplicial sets]] or (compactly generated weakly Hausdorff) [[topological spaces]] with the [[weak homotopy equivalences]] universally turned into actual [[homotopy equivalences]]:

$$
  \infty Grpd \simeq L_{whe} sSet \simeq L_{whe} Top
$$

The combination of these two aspects, the [[geometry]] and the [[homotopy theory]], is that of [[(âˆž,1)-sheaves]]/[[âˆž-stacks]], here we write $\mathbf{H}$ for the collection of [[presheaves]] of [[simplicial sets]] ([[simplicial presheaves]]) with the local ([[stalk]]-wise) [[weak homotopy equivalences]] universally turned into actual homotopy equivalences:
...
This $\mathbf{H}$ is the _[[(âˆž,1)-topos]]_ over $\mathcal{C}$. This is the kind of context for the [[higher geometry]] encoded by $\mathcal{C}$ in which all of our constructions take place.
...
if $\mathcal{C}$ satisfies some properties that make it an an [[infinity-cohesive site]], the the [[global section geometric morphism]] of $\mathbf{H}$ extended to an [[adjoint quadruple]] of [[(âˆž,1)-functors]]:
...
with some mild extra condition on $\Pi$ and $coDisc$. This is the case in particular for $\mathcal{C} = $ [[SmthMfd]] in which case we say that $\mathbf{H}$ is the [[cohesive (âˆž,1)-topos]] of _[[smooth âˆž-groupoids]]_.

The defining [[adjoint quadruple]] of [[(âˆž,1)-functors]] induces an [[adjoint triple]] of [[comonad|co]][[(âˆž,1)-monads]]:

| [[shape modality]] | $\dashv$ | [[flat modality]] | $\dashv$ |[[sharp modality]] |
|--|--|--|--|--|
| $\mathbf{\Pi} \coloneqq Disc \circ \Pi$ | | $flat \coloneqq Disc \circ \Gamma$ |   | $\sharp \coloneqq coDisc \circ \Gamma$  |

For $G \in Grp(\mathbf{H})$ a [[group object in an (âˆž,1)-category|group object]] in $\mathbf{H}$, hence a geometric [[âˆž-group]], we have by these axioms of [[cohesion]] a [[pasting diagram]] of [[(âˆž,1)-pullbacks]]
...
Here the [[delooping]] $\mathbf{B}G$ is the moduli of geometric $G$-[[principal âˆž-bundles]],  $\flat_{dR} \mathbf{B}G$ is that of flat [[infinity-Lie algebroid-valued differential form
|L-âˆž algebra valued differential forms]] and $\theta_G$ is the _[[Maurer-Cartan form]]_ on the [[smooth âˆž-group]] $G$.

If $\mathbb{G}$ is "slightly abelian" in that it is equipped with the structure of a [[braided âˆž-group]], then we write
...
for the [[Maurer-Cartan form]] of its [[delooping]] [[âˆž-group]] and call this the _universal [[curvature]] characteristic_.
...
is the moduli for [[principal âˆž-connections]].
...
In particular if, in the context of [[smooth âˆž-groupoids]], we consider $\mathbb{G} = \mathbf{B}^{n-1}U(1)$ the [[circle n-group]] then 
...
is given as the [[âˆž-stackification]] of the image under the [[Dold-Kan correspondence]] of the _[[Deligne complex]]_ for [[ordinary differential cohomology]].

In this case there is [[fiber integration in ordinary differential cohomology]] refined to moduli $\infty$-stacks: for $\Sigma_k$ an oriented [[closed manifold]] of [[dimension]] $k$ we have a map of [[moduli âˆž-stacks]]
...
Under [[geometric realization of cohesive âˆž-groupoids|geometric realization]] as just [recalled](#IntermezzoElementsOfHigherCohesiveGeometry) this has an essentially unique lift to _smooth cohomology_, hence to a morphism of [[moduli âˆž-stacks]]
...
The nontrivial part is that, as one can see, the  [[cup product]] on [[differential cohomology]] refines to [[moduli âˆž-stacks]] of [[circle n-bundle with connection]] as the [[âˆž-stackification]] of what is called the [[Beilinson-Deligne cup product]], and so we have 
...
| [[schreiber:âˆž-Chern-Simons theory]] | $\leftarrow$[[holographic principle]] $\rightarrow$| [[schreiber:âˆž-Wess-Zumino-Witten theory ]] | [[Kaluza-Klein reduction]] $\to$ |   |
|--|--|--|--|--|
...
Here the lower factorization of this map shows canonically that this 6-bundle is [[equivariant]] with respect to the [[adjoint action|adjoint]] [[âˆž-action]] of the [[string 2-group]] on itself, the analog of the ordinary adjoint equivariance of the ordinary WZW 2-bundle.
...
In the examples of [[2d Chern-Simons theory]] and [[7d Chern-Simons theory]] that we have discussed above, the fully extended Lagrangian is equivalently the [[prequantum n-bundle]] on the universal [[moduli âˆž-stack]] of [[field (physics)|fields]], for $n = 3$ and $n = 7$, respectively.

But in fact this is the general situation for fully extended Lagrangians: 

if $\mathbf{Fields} \in \mathbf{H}$ is the object that serves as the [[moduli âˆž-stack]] of [[field (physics)|fields]] for some $n$-dimensional [[theory (physics)|theory]] in that for any [[spacetime]]/[[worldvolume]] $\Sigma_n$ of [[dimension]] $n$ the space of fields is the [[mapping stack]] $[\Sigma_n, \mathbf{Fields}]$, then the corresponding [[prequantum n-bundle]] has to be over this object $\mathbf{Fields}$ and hence to be modulated by a map of moduli $\infty$-stacks of the form
...
Above we considered the case that $\mathbf{Fields} \simeq \mathbf{B}G_{conn}$ for a [[Lie group]] $G$ or $\mathbf{Fields} \simeq \mathbf{B}String_{conn}$ for the [[string 2-group]]. In general for $G \in Grpd(\mathbf{H})$ a [[smooth âˆž-group]] an extended Lagrangian as above
encodes an [[extended prequantum field theory|extended prequantum]] _[[higher gauge theory]] of Chern-Simons type_: 

the [[curvature]] $(n+1)$-form of $\mathbf{L}$ (i.e. the [[n-plectic geometry|pre n-plectic form]] of the [[n-plectic geometry|n-plectic]] [[âˆž-stack]] $\mathbf{Fields}$ )
...
is -- by [[âˆž-Chern-Weil theory]] -- an [[invariant polynomial]] of the [[âˆž-Lie algebra]] $\mathfrak{g}$ of $G$. Accordingly, the [[action functional]]
...
|L-âˆž algebra valued differential form]] $A$ (the [[higher gauge fields]]) to a [[Lagrangian]] $CS(A)$ which locally is a differential form with the property that $ d CS(A) = F_A$.
This means that 
$\exp\left( 2 \pi i \int_{\Sigma_n} [\Sigma_n, \mathbf{L}] \right)$ is a [[higher gauge theory]] _of Chern-Simons type_. We say that it is an _[[schreiber:âˆž-Chern-Simons theory]]_.
...
|L-âˆž algebroid valued differential form]].
...
The remaining question then is how accurately [[extended prequantum field theory]] constrains the space of possible [[local Lagrangians]]. In other words: which theories of interest can not be thought of as [[schreiber:âˆž-Chern-Simons theories]] or [[schreiber:âˆž-Wess-Zumino-Witten theory ]]?

As a hint to the answer of this question, here is a list of names examples of [[schreiber:âˆž-Chern-Simons theories]]. We discuss the items of the list in detail in the following.
