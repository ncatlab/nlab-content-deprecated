...
[[over-(âˆž,1)-category]] $C^\circ / X$: we have an [[equivalence of (âˆž,1)-categories]]
...
It is clear that we have an [[essentially surjective (âˆž,1)-functor]] $C^\circ/X \to (C/X)^\circ$. What has to be shown is that this is a [[full and faithful (âˆž,1)-functor]] in that it is an [[equivalence in an (âˆž,1)-category|equivalence]] on all [[hom-object|hom]]-[[âˆž-groupoid]]s $C^\circ/X(a,b) \simeq (C/X)^\circ(a,b)$.

To see this, notice that the hom-space in an [[over-(âˆž,1)-category]] $C^\circ/X$ between objects $a : A \to X$ and $b : B \to X$ is given (as discussed there) by the 
[[(âˆž,1)-pullback]]
