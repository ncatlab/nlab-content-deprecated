...
Thus, for $A$ a commutative [[âˆž-algebra]], its _Hochschild homology complex_ is its [[(âˆž,1)-colimit|(âˆž,1)-tensoring]] $S^1 \cdot A$ with the [[âˆž-groupoid]] incarnation of the circle. More generally, for $S$ any $\infty$-groupoid/simplicial set, $S \cdot A$ is the corresponding _higher order Hochschild homology_ of $A$.

In the presence of [[function algebras on âˆž-stacks]] it may happen that $A = \mathcal{O}(X)$ is the algebra of functions on some [[âˆž-stack]] $X$ and that $\mathcal{O}(-)$ sends [[powering]]s of  $X$ to tensorings of $\mathcal{O}(X)$. In that case it follows that the Hochschild homology complex of $\mathcal{O}(X)$ is the function complex $\mathcal{O}(\mathcal{L}(X))$ on the [[derived loop space]] $\mathcal{L}X$ of $X$.
...
Then still a little later, it was understood that this is just the ordinary tensor product in the [[symmetric monoidal (âˆž,1)-category]] of [[chain complex]]es. If this
...
This, generally, is the definition of the Hochschild homology object of any bimodule over an [[monoid in a monoidal (âˆž,1)-category|monoid]] in a symmetric monoidal $(\infty,1)$-category (symmetry is needed to make sense of $A^{op}$).  Dually, the Hochschild *cohomology* object is 
...
Let $\mathbf{H}$ be an [[(âˆž,1)-topos]] that admits [[function algebras on âˆž-stacks]] (see there for details)
...
In particular the [[(âˆž,1)-category]] of [[âˆž-algebra]]s $Alg^{op}$ is [[(âˆž,1)-colimit|(âˆž,1)-tensored]] over [[âˆžGrpd]]. Then for $A \in Alg$ and $K \in \infty Grpd$ we say that
...
We say  a full [[sub-(âˆž,1)-category]] of $\mathbf{H}$ consists of **$\mathcal{O}$-perfect objects** if on these $\mathcal{O}$ commutes with [[(âˆž,1)-limit]]s.
...
For $\mathcal{O}$ the functor that forms [[symmetric monoidal (âˆž,1)-category|symmetric monoidal (âˆž,1)-categories]] of [[quasicoherent sheaf|quasicoherent âˆž-stacks of modules]] over [[âˆž-stack]]s over an [[(âˆž,1)-site]] of [[âˆž-algebra over an (âˆž,1)-algebraic theory|âˆž-algebras]] for the ordinary theory of commutative $k$-algebras this has setup been considered in detail in ([Ben-ZviFrancisNadler](#Ben-ZviFrancisNadler)).
...
Let $T$ be an [[(âˆž,1)-algebraic theory]] and $T Alg_\infty$ its [[(âˆž,1)-category]] of $\infty$-algebras. Let $C$ with $T \hookrightarrow C \hookrightarrow T Alg_\infty^{op}$ be a [[small (âˆž,1)-category|small]] full [[sub-(âˆž,1)-category]] of $T Alg_\infty^{op}$ which is closed under [[(âˆž,1)-limit]]s in $T Alg$ and equipped with the structure of a [[subcanonical coverage|subcanonical]] [[(âˆž,1)-site]]. 

Take $\mathbf{H} := Sh(C)$ the [[(âˆž,1)-category of (âˆž,1)-sheaves]] on $C$. This is an [[(âˆž,1)-topos]] for [[derived geometry]] modeled on $T Alg_\infty$.  Write $C \hookrightarrow \mathbf{H}$ for the [[(âˆž,1)-Yoneda embedding]].
...
where on the right we have the <a href="http://ncatlab.org/nlab/show/limit+in+a+quasi-category#Tensoring">(âˆž,1)-tensoring</a> of $T Alg_\infty$ over $\infty Grpd$, which is the [[(âˆž,1)-colimit]] over the [[diagram]] $S^1$ of the [[(âˆž,1)-functor]] constant on $\mathcal{O}(X)$
...
Because the [[(âˆž,1)-Yoneda embedding]] preserves [[(âˆž,1)-limits]] the limit $X^K$ may be computed in $C$. By assumption $C$ is closed under limits in $T Alg_\infty^{op}$. The limit $X^K$ in $T Alg^{op}$ is the colimit $K \cdot \mathcal{O}(X)$ in the [[opposite (âˆž,1)-category]] of $\infty$-algebras.

=--

This definition of general higher order Hochschild homology by $(\infty,1)$-copowering is 

* explicit in [ToÃ«nVezzosi](#ToenVezzosi), for ordinary Hochschild homology, hence $K = S^1$,
...
* explicit in [Ben-Zvi/Francis/Nadler, corollary 4.12](#Ben-ZviFrancisNadler) for HH with values in [[quasicoherent sheaf|quasicoherent âˆž-stacks]] and over perfect $\infty$-stacks (see there for details).
...
This generalizes to $\infty$-colimits of functors constant on an algebra, but over a genuine [[(âˆž,1)-category]] diagram.

Specifically let $X$ be framed $n$-manifold, $A$ an [[little k-cubes operad|En-algebra]] and $D_X$ the [[(âˆž,1)-category]] whose objects are framed embeddings of disjoint unions of open discs into $X$ and morphisms are inclusions of these. Let $F_A$ be the functor that assigns $A^{k}$ to an object corresponding to $k$ discs in $X$, and iterated products/units to morphisms 

Then the [[(âˆž,1)-colimit]]
...
Let $T$ be a [[Lawvere theory]] regarded as a 0-[[truncated]] [[(âˆž,1)-algebraic theory]]. 

Consider a [[model structure on simplicial T-algebras]]/on [[homotopy T-algebras]] [[presentable (âˆž,1)-category|presenting]] $T Alg_\infty$ such that 
...
Then we look in detail at higher order Hochschild homology in the $(\infty,1)$-topos over an [[(âˆž,1)-site]] of formal duals of [[dg-algebra]]s. In this context the classical [theorem by Jones](#JonesTheorem) on Hochschild homology and loop space cohomology is a natural consequence of the general machinery.

### Gradings and conventions {#GradingsAndConventions}

In [[derived geometry]] two categorical gradings interact: a [[cohesive (âˆž,1)-topos|cohesive]] $\infty$-groupoid $X$ has a space of [[k-morphism]]s $X_k$ for all non-negative $k$, and each such has itself a _[[simplicial T-algebra]]_ of functions with a component in each non-positive degree. But the directions of the face maps are opposite. We recall the grading situation from [[function algebras on âˆž-stacks]].
...
Taking this together, for $X_\bullet$ a general [[âˆž-stack]], its function algebra is generally an _unbounded_ cochain dg-algebra with mixed contributions as above, the simplicial degrees contributing in the positive direction, and the homological resolution degrees in the negative direction:
...
#### Identification with KÃ¤hler differential forms {#IdWithKÃ¤hlerForOrdAlgebra}

We spell out in detail how in degree 0 and 1 the [[homology]] of the Hochschild complex of $A$ is that of its [[KÃ¤hler differential form]]s. Under mild conditions on $A$ this is also true in higher degrees, which is the statement of the [Hochschild-Kostant-Rosenberg theorem](#HochschildKostantRosenberg).


+-- {: .num_prop #KÃ¤hler1Forms}
###### Proposition

The [[homology]] of the Hochschild complex $S^1 \cdot A$ in degree 1 is the  [[KÃ¤hler differential form]]s of $A$
...
The last term on the right is precisely the term by which one has to quotient out the module of formal expressions $f \wedge d g$ to get the module of [[KÃ¤hler differential]]s: setting it to 0 is the [[derivation]] property of $d$
...
Under the identification of $HH_\bullet(A,A) = H_\bullet(S^1 \cdot A)$ with KÃ¤hler differential forms, the [[cup product]] on homology identifies with the [[exterior algebra|wedge product]] of differential 0- and 1-forms.
...
We describe the canonical [[action]] of the [[automorphism 2-group]] of the circle $S^1$ on $S^1 \cdot A$ and how its degree-1 part induces under the [above](#IdWithKÃ¤hlerForOrdAlgebra) identification $H_\bullet(S^1 \cdot A) \simeq \Omega^\bullet_K(A)$ the action of the de Rham differential.
...
Under the above identification of the homology of $\mathbf{B}\mathbb{Z} \cdot A$ with KÃ¤hler forms, this is on elements the map
...
So let now $T$ be the [[Lawvere theory]] of ordiary commutative [[associative algebra]]s over a [[field]] $k$, regard as a 0-[[truncated]] [[(âˆž,1)-algebraic theory]]. 

+-- {: .num_defn}
###### Proposition

The [[(âˆž,1)-category]] $CAlg(k)_\infty$ of [[algebras over a Lawvere theory|âˆž-algebras]] over $T$ is [[presentable (âˆž,1)-category|presented]] by the [[model structure on simplicial T-algebras|model structure on simplicial commutative k-algebras]] $(CAlg_k^{\Delta^{op}})_{proj}$.
...
The first statement is discussed at [[(âˆž,1)-algebraic theory]] and [[homotopy T-algebra]]. The second statement is discussed at [[monoidal Dold-Kan correspondence]].
...
be a [[subcanonical coverage|subcanonical]] [[(âˆž,1)-site]] that is a full 
[[sub-(âˆž,1)-category]] of formal duals of $\infty$-$T$-algebras, closed under [[(âˆž,1)-limit]]s in $T Alg_\infty^{op}$. 
...
be the [[(âˆž,1)-sheaf (âˆž,1)-topos]] over $C$. 

Following the notation at [[Isbell duality]] and [[function algebras on âˆž-stacks]] we write $\mathcal{O}(X) \in T Alg_\infty$ for an object that under the [[(âˆž,1)-Yoneda embedding]] $C \hookrightarrow T Alg_\infty^{op} \to \mathbf{H}$ maps to an object called $X$ in $\mathbf{H}$.
...
The term _derived_ is just to emphasize that we do not form the [[free loop space object]] in an [[(âˆž,1)-topos]] of $(\infty,1)$-sheaves over a 1-[[site]] inside the 1-category $Alg_k^{op}$. These "underived" (not embedded into [[(âˆž,1)-category theory]]) free loop space objects would just be equivalent to $X$. The [[derived loop space]] instead has rich interesting structure.

But if the ambient context of [[higher geometry]] over the genuine [[(âˆž,1)-site]] of formal duals to $\infty$-algebras is clear, we can just speakk of _free loop space objects_ . They are canonically given.
...
We have that $\mathcal{O}(\mathcal{L}X)$ is given by the [[(âˆž,1)-pushout]] in $CAlg_\infty$ 
...
Since [[âˆž-stackification]] $L : PSh_{(\infty,1)}(C) \to \mathbf{H}$ is a [[left exact (âˆž,1)-functor]] and hence preserves finite [[(âˆž,1)-limit]]s, we have that the defining pullback for $\mathcal{L}X$ may be computed in the [[(âˆž,1)-category of (âˆž,1)-presheaves]] $PSh_{(\infty,1)}(C)$. Since the [[(âˆž,1)-Yoneda embedding]] preserves all [[(âˆž,1)-limit]]s this in turn may be computed in the [[(âˆž,1)-site]] $C$, hence by assumption in $T Alg_\infty$. The relevant $(\infty,1)$-pullback there is the claimed $(\infty,1)$-pushout in the [[opposite (âˆž,1)-category]] $T Alg_\infty$. 
...
We discuss details of Hochschild homology in the context of [[dg-geometry]]:  the [[(âˆž,1)-topos]] over an [[(âˆž,1)-site]] of formal duals of commutative [[dg-algebra]]s over a field, [[presentable (âˆž,1)-category|presented]] by the [[model structure on dg-algebras]].


Fix a [[field]] $k$ of [[characteristic]] 0. We consider now the context of [[dg-geometry]] with its [[function algebras on âˆž-stacks]] taking values in unbounded dg-algebras, exhibited by the [[adjoint (âˆž,1)-functors]]
...
[Pirashvili's copowering definition](#PirashviliHigherOrder) of higher order Hochschild homology holds true in dg-geometry. For $X$ a [[manifold]] regarded as a [[topological space]] and then as a [[constant âˆž-stack]] in $\mathbf{H}$ we have for any $A \in cdgAlg_k$
...
[[Jones' theorem]] asserts that the Hochschild homology of the [[dgc-algebra]] of [[differential forms]] on a [[smooth manifold]] computes the [[ordinary cohomology]] of the corresponding [[free loop space]]. We discuss now how this result follows using derived loop spaces of [[constant âˆž-stacks]]
...
Set $k = \mathbb{R}$. Write $LConst X \in \mathbf{H}$ for the [[constant âˆž-stack]] on the [[homotopy type]] of $X$, regarded as a [[topological space]] $\simeq$ [[âˆžGrpd]]. Then
...
Since $LConst$ is a [[left exact (âˆž,1)-functor]] it commutes with forming [[free loop space objects]] and therefore
...
Consider as before the categorical [[circle]] $S^1$ as the corresponding [[constant âˆž-stack]] in $\mathbf{H}$. We describe the function $\infty$-algebra on $S^1$. Below this will serve to explain the nature of the canonical circle action on the Hochschild complex of a cdg-algebra.
...
Every [[âˆž-groupoid]] is the [[(âˆž,1)-colimit]] over itself (as described there) of the [[(âˆž,1)-functor]] constant on the point. This [[(âˆž,1)-colimit]] is preserved by the left [[adjoint (âˆž,1)-functor]] $LConst : \infty Grpd \to \mathbf{H}$, so that we have

$$
  S^1 \simeq {\lim_{\to}}_{S^1} *
$$

in $\mathcal{H}$. The [[(âˆž,1)-functor]] $\mathcal{O}$ is also left adjoint, so that 
...
* Hochschild homology of $\mathcal{O}(X)$ encodes [[KÃ¤hler differential form]]s on $X$;
...
The **[[Hochschild-Kostant-Rosenberg theorem]]** states that under suitable conditions, the Hochschild homology of an algebra (with coefficients in itself) computes the wedge powers of its [[KÃ¤hler differential]]s.

Let $A$ be an [[associative algebra]] over $k$. Recall the natural [identification](#KÃ¤hler1Forms)

$$
  HH_1(A,A) \simeq \Omega^1(A)
$$

of the first Hochschild homology of $A$ with coefficients in itself and degree-1 [[KÃ¤hler differential form]]s of $A$.



Write $\Omega^0(R/k) := R \simeq HH_0(R,R)$.

For $n \geq 2$ Write $\Omega^n(R/k) = \wedge^n_R \Omega(R/k)$ for the $n$-fold [[exterior algebra|wedge product]] of $\Omega(R/k)$ with itself: the degree $n$-KÃ¤hler-differentials.
...
in [[âˆžGrpd]] and under $LConst : \infty Grpd \to \mathbf{H}$ then also in our [[(âˆž,1)-topos]].

Applying the <a href="http://ncatlab.org/nlab/show/(infinity,1)-topos#ClosedMonoidalStructure">(âˆž,1)-topos internal hom</a> $[-,X]$ or equivalent the <a href="http://ncatlab.org/nlab/show/limit+in+a+quasi-category#Tensoring">(âˆž,1)-powering</a> $X^{(-)}$ into a given object $X \in \mathbf{H}$ to this [[cospan]] produces the [[span]]
...
Let $T$ be the [[algebraic theory]] of ordinary [[associative algebra]]s over a [[field]] $k$, regarded as an [[(âˆž,1)-algebraic theory]] and let $\mathbf{H}$ be the [[(âˆž,1)-topos]] of $(\infty,1)$-sheaves over a small site in $T Alg_\infty^{op}$.

Under the [[inverse image]] of the [[global section]] [[(âˆž,1)-geometric morphism]] and the [[homotopy hypothesis]]-equivalence
...
we may regard every [[topological space]] $X$ as a [[constant âˆž-stack]] $LConst X$, an object in $\mathbf{H}$.
...
This has maybe been first made explicit by [[Bertrand ToÃ«n]]. Details are at [[function algebras on âˆž-stacks]].
...
* [[Teimuraz Pirashvili]], _Hodge decomposition for higher order Hochschild homology_ Annales Scientifiques de lâ€™Ã‰cole Normale SupÃ©rieure Volume 33, Issue 2, March 2000, Pages 151-179 ([ps](http://www.mathematik.uni-bielefeld.de/sfb343/preprints/pr98058.ps.gz))
{#Pirashvili}

A survey of traditional higher order Hochschild (co)homology and further developments and results are described in

* [[GrÃ©gory Ginot]], _Higher order Hochschild cohomology_ 
...
* [[GrÃ©gory Ginot]], Thomas Tradler, [[Mahmoud Zeinalian]], _Derived higher Hochschild homology, topological chiral homology and factorization algebras_, [arxiv/1011.6483](http://arxiv.org/abs/1011.6483)
...
* [[Bertrand ToÃ«n]] [[Gabriele Vezzosi]], _$S^1$-Equivariant simplicial algebras and de Rham theory_ ([arXiv:0904.3256](http://arxiv.org/abs/0904.3256))

 
General homotopy-theoretic setups and results for contexts in which this makes sense are discussed in

* [[Bertrand ToÃ«n]], [[Gabriele Vezzosi]], _HAG II, geometric stacks and applicatons_ ([arXiv:math/0404373v4](http://arxiv4.library.cornell.edu/abs/math/0404373v4))
{#ToenVezzosiStacks}
...
* Geoffroy Horel, _Factorization homology and calculus Ã  la Kontsevich Soibelman_ ([arXiv:1307.0322](http://arxiv.org/abs/1307.0322))
