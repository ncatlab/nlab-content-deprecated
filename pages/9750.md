...
Let $\mathbf{H}$ be any  [[(âˆž,1)-topos]] and let $G$ be an [[âˆž-group]] in $\mathbf{H}$. Then the right [[base change]]/[[dependent product]] along the canonical point inclusion $\ast \to \mathbf{B}G$ into the [[delooping]] of $G$ takes the following form: There is a pair of [[adjoint âˆž-functors]] of the form
...
* $[G,-]/G$ denotes the [[homotopy quotient]] by the [[conjugation action|conjugation]] [[âˆž-action]] for $G$ equipped with its canonical [[âˆž-action]] by left multiplication and the argument
...
* $\hat X \to X$ a $G$ [[principal âˆž-bundle]]
...
the [[(âˆž,1)-category]] of $G$-[[âˆž-actions]] $Act_G(\mathbf{H})$.
Under the [[equivalence of (âˆž,1)-categories]]

$$
  Act_G(\mathbf{H}) \simeq \mathbf{H}_{/\mathbf{B}G}
$$

(from [NSS 12](geometry+of+physics+-+fundamental+super+p-branes#NSS12)) then $G$ with its canonical [[âˆž-action]] is $(\ast \to \mathbf{B}G)$
...
But now since the [[slice (âˆž,1)-topos]] $\mathbf{H}_{/\mathbf{B}G}$ is itself [[cartesian closed (infinity,1)-category|cartesian closed]], via
