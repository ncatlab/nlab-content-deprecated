...
Let $C$ be an [[(âˆž,1)-category]] and $S$ a collection of morphisms in $C$. Then an [[object]] $c \in C$ is **$S$-local** if the [[hom-functor]]
$$
...
A useful reference with direct ties to the [[(âˆž,1)-category]] story in the background is section A.3.7 of
...
* a [[reflective subcategory]] as well as a [[reflective (âˆž,1)-subcategory]] can be realized as the full ($(\infty,1)$-)subcategory on $S$-local objects, where $S$ is the collection of morphisms sent by the corresponding [[localization of an (âˆž,1)-category]] to equivalences. For details on this see the discussion at [[geometric embedding]].
