...
The tools of [[geometric quantization]] mainly apply to [[quantum mechanics]] and only partially to [[quantum field theory]]. In particular in the context of _[[extended prequantum field theory]]_ in [[dimension]] $n$ a [[prequantum bundle]] over the ([[phase space|phase]]-)space of [[field (physics)|fields]] is to be refined (de-[[transgression|transgressed]]) to a _[[prequantum n-bundle]]_ over the [[moduli âˆž-stack]] of [[field (physics)|fields]]. Therefore in order to apply [[geometric quantization]] to [[extended prequantum field theory]] to obtain [[extended quantum field theory]] we first need extended/higher prequantum geometry.
...
Important construction principle for [[(âˆž,1)-categories]]: [[simplicial localization]]. For $\mathcal{C}$ a [[category]] with some subset of morphisms $W \hookrightarrow Mor(\mathcal{C})$ declared to be "[[weak equivalences]]", the [[simplicial localization]] 
...
is called the _[[(âˆž,1)-topos]]_ of [[(âˆž,1)-sheaves]]/[[âˆž-stacks]] on $C$.

An [[A-âˆž algebra]]-object $G$ in such an $(\infty,1)$-topos such that $\pi_0(G)$ is a [[group]] is called an [[âˆž-group]] "with geometric structure as encoded by the test spaces $C$". The canonical source of $\infty$-groups are the [[homotopy fiber products]] of point inclusions $* \to X$ of any object X, the [[loop space object]]
...
In fact this are _all_ the [[âˆž-groups]] that there are, up to equivalence: forimg [[loop space objects]] is an [[equivalence of (âˆž,1)-categories]]
...
between [[âˆž-groups]] and [[pointed object|pointed]] [[connected object in an (âˆž,1)-topos|connected]] objects. The inverse equivalence $\mathbf{B}$ is the _[[delooping]]_ operation.

We say that such an $(\infty,1)$-topos $\mathbf{H}$ is _[[cohesive]]_ if it is equipped with an [[adjoint triple]] of [[idempotent monad|idempotent]] (co)/[[(âˆž,1)-monads]]
...
This implies (strictly speaking we need [[differential cohesion]] for that, coming from another adjoint triple of (co)monads) that for every [[braided âˆž-group]] $\mathbb{G} \in Grp(\mathbf{H})$ there is a canonical object $\mathbf{B}\mathbb{G}_{conn}$ which modulats $\mathbb{G}$-[[principal âˆž-connections]].
...
The way all these notions and theorems work is by considering [[automorphism âˆž-groups]] of the classifying (or rather: modulating) maps $\nabla \colon X \to \mathbf{B}\mathbb{G}_{conn}$ of a [[prequantum âˆž-bundle]] in the [[slice (âˆž,1)-topos]] over the domain. For instance
...
The [[Lie differentiation]] of the [[âˆž-group extension]] sequence of prop. \ref{QuantomorphismExtension} is a [[homotopy fiber sequence]] of [[L-âˆž algebras]] 
...
* $\mathbf{H}(X, \flat (\mathbf{B}^{n-1})\mathbb{R})$ is the [[chain complex]] for flat [[de Rham cohomology]] in the given degree, regarded as an abelian [[L-âˆž algebra]].
