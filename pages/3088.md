...
One very general notion of internal $\infty$-groupoid is given by taking some standard definition of $\infty$-groupoid and writing it down internally to an [[(âˆž,1)-category]].  This is described below in the section

* [âˆž-groupoids internal to an (âˆž,1)-category](#KanInoo1Cat).

For various applications somewhat stricter or at least more rigidified models for this may be useful. Notably one may wish to speak of $\infty$-groupoids internal to an ordinary category.  In terms of [[Kan complexes]] as a model for ordinary [[âˆž-groupoids]], this is discussed below in the section

* [Kan complexes internal to an ordinary category](#KanIn1Cat).

Notice from the discussion there that these two aspects may and often do interplay: simplicial objects in [[Grothendieck topos|sheaf topos]]es may be used to [[presentable (infinity,1)-category|present]] [[(âˆž,1)-category|(âˆž,1)-categories]] inside of which one may be interested in $\infty$-groupoid objects in the first sense. For instance a Lie $\infty$-group object is usefully thought of as a _group object_ internal to the [[(âˆž,1)-topos]] presented by [[model structure on simplicial presheaves|a model of simplicial sets in]] the [[category of sheaves]] on [[Diff]].

Finally, a simplified version of [[âˆž-groupoid]] is often sufficient and useful: that given by [[strict omega-groupoid|strict âˆž-groupoid]]s, equivalently [[crossed complexes]]. These may be straightforwardly [[internalization|internalized]] in any ordinary category with [[pullback]]s. This is discussed in
...
The special case of internal *1-groupoids* in an (âˆž,1)-category, which are still only associative and unital up to higher homotopy, but which do not include "higher cells" as additional data, is discussed in detail at:
...
The [[Kan complex]] definition of [[âˆž-groupoid]] may be [[internalization|internalized]] to more general categories. Namely:
...
* Another classical example consists of the [[âˆž-Lie groupoids]].
...
Models for [[âˆž-stacks]]/[[(âˆž,1)-presheaves]] in [[higher geometry]] by local Kan complexes of objects in a given site (for instance locally Kan [[simplicial manifolds]] for [[higher differential geometry]]) are discussed in 
...
[[!redirects Internal âˆž-groupoid]]
[[!redirects internal âˆž-groupoid]]
[[!redirects infinity-groupoid object]]
[[!redirects âˆž-groupoid object]]
