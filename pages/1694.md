...
Currently, if you open [[(infinity,1)-category]] for editing and look at the bottom, you will see
    ...
    [[!redirects (âˆž,1)-category]]
    [[!redirects (âˆž,1)-categories]]
    [[!redirects (infinity,1)-categories]]

This means that we can now simply insert links like [[(âˆž,1)-categories]] and be redirected to [[(infinity,1)-category]].  The only downside here (compared to the previous alternative) is that math material is in the text font.  (Some of us even write `$2$-[[2-category|category]]` for this reason.)

##Caveat##

The `âˆž` symbol included in the link is not obtained via the itex syntax `$\infty$`. To actually include non-ASCII symbols in links, you either need to be an expert at unicode *or* simply find the 'âˆž' symbol already displayed somewhere, copy it, and paste the symbol into the edit box. To see what I mean, you can edit this page to look at the source code for the link: [[(âˆž,1)-categories]].
