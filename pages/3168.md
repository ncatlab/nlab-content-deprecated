...
Some sources use â€œ$X$ is inhabitedâ€ in the global sense, while others use the term "inhabited" only internally.  Regardless, a [[pointed object]] always means one _equipped with_ a global element $1\to X$ (which is the same whether interpreted internally or externally), and the terms "global element" and "well-supported" are always unambiguous.
