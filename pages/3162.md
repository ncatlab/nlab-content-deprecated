...
From the [[nPOV]], a [[cohomology]] set $H(X,A)$ of cohomology classes on an [[object]] $X$ with coefficients in an [[object]] $A$ in an [[(âˆž,1)-topos]] is the [[decategorification]] of an [[derived hom space|hom âˆž-groupoid]] $\mathbf{H}(X,A)$. The objects in this $\infty$-groupoid are called **cocycles** . The morphisms are called _coboundaries_ .
...
The objects in the [[derived hom space|hom âˆž-groupoid]] $\mathbf{H}(X,A)$ are often expressed in terms of various 1-categorical models for $\mathbf{H}$, such as a [[homotopical category]] $C$ equipped with 
...
This homotopy fiber may be thought of as the internal [[principal âˆž-bundle]] in $\mathbf{H}$ with classifying map $\omega$.
