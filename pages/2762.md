...
In  the context of [[elliptic cohomology]] one assigns to every [[elliptic curve]] $\phi$ over a [[ring]] $R$ a [[cohomology theory]] [[Brown representability theorem|represented]] by an [[E-âˆž ring]] [[spectrum]] $E_\phi$.
...
looks like an [[E-âˆž ring]] valued [[structure sheaf]] on $\mathcal{M}_{1,1}$.
...
The [[moduli stack]] $\mathcal{M}_{1,1}$ of [[elliptic curve]]s equipped with the [[E-âˆž ring]]-valued [[structure sheaf]] $\mathcal{O}^{Der}$ may be regarded as the _derived moduli stack_ of [[derived elliptic curve]]s in that for any [[E-âˆž ring]] $R$ the space of derived stack morphisms
...
The special case of the general theory that is needed here, where the coefficient objects of structure sheaves are [[E-âˆž ring]]s, is described in

* [[Jacob Lurie]], [[Spectral Schemes]],

while the general theory of [[E-âˆž ring]]s themselves, in the [[(âˆž,1)-category]] theory context needed here, is developed in
...
The **central ingredient** which we choose at the beginning to get a theory of [[higher geometry]] going is an [[(âˆž,1)-category]] $\mathcal{G}$ whose objects we think of as **model spaces** : the simplest objects exhibiting the geometric structures that we mean to consider.
...
* without smooth structure
  ...
  * $\mathcal{G} = (E_\infty Ring^{fin})^{op}$, the formal dual of [[E-âˆž ring]]s: the category of (finitely generated) algebraic derived [[affine scheme]]s.

=--


These [[(âˆž,1)-category|(âˆž,1)-categories]] $\mathcal{G}$ are naturally equipped with the structure of a [[site]] (and a bit more, which we won't make explicit for the present purpose). Following [[Jacob Lurie]] we call such a $\mathcal{G}$ a **[[geometry (for structured (infinity,1)-toposes)|geometry]]** . 
...
of an [[âˆž-groupoid]] of possible ways to probe $X$ by $U$, for each possible $U$, natural in $U$. More precisely, this should define an object in the [[(âˆž,1)-category of (âˆž,1)-presheaves]] on $\mathcal{G}$
...
Such objects are called [[âˆž-stack]]s on $\mathcal{G}$. The [[(âˆž,1)-category]] $Sh(\mathcal{G})$ is called an [[âˆž-stack]] [[(âˆž,1)-topos]].

A supposedly pedagogical discussion of the general philosophy of [[âˆž-stacks]] as probebable spaces is at [[motivation for sheaves, cohomology and higher stacks]].

The [[âˆž-stack]]s on $\mathcal{G}$ that are used in the following are those that satisfy [[descent]] on [[ÄŒech cover]]s. But we will see [[(âˆž,1)-topos]]es of [[âˆž-stack]]s that may satisfy different descent conditions, in particular with respect to [[hypercover]]s. Every [[âˆž-stack]] [[(âˆž,1)-topos]] has a [[hypercompletion]] to one of this form.

For concretely working with [[hypercomplete (âˆž,1)-topos]]es it is often useful to use [[models for âˆž-stack (âˆž,1)-toposes]] in terms of the [[model structure on simplicial presheaves]].
...
$$
  ...
  {\hookrightarrow}&
     PSh_{(\infty,1)}(C)
    && \text{abstract nonsense def of (âˆž,1)-topos}
    \\
    \uparrow^{\simeq} &&
    \uparrow^{\simeq}
    && \text{Lurie's theorem}
...
**(Counter-)Example** For $\mathcal{G} = $ [[Diff]], for every $n \in \mathbb{N}$ we have the [[âˆž-stack]] $\Omega_{cl}^n(-)$ (which happens to be an ordinary [[sheaf]]) that assigns to each manifold $U$ the set of closed [[differential form|n-form]]s on $U$. This is important as a generalized space: it is something like the rational version of the [[Eilenberg-MacLane space]] $K(\mathbb{Z}, n)$. But at the same time this is a "wild" space that has exotic properties: for instance for $n=3$ this space has just a single point, just a single curve in it, just a single surface in it, but has many nontrivial probes by 3-dimensional manifolds.

In the classical theory for instance of [[ringed space]]s or [[diffeological space]]s a _concrete underlying space_ is taken to be a [[topological space]]. But this in turn is a bit _too_ restrictive for general purposes: a topological space is the same as a [[localic topos]]: a [[category of sheaves]] on a [[category of open subsets]] of a [[topological space]]. The obvious generalization of this to [[higher geometry]] is: an [[n-localic (âˆž,1)-topos]] $\mathcal{X}$.

This makes us want to say and make precise the statement that

+-- {: .standout}

An **concrete [[âˆž-stack]]** $X$ is one which has an _underlying_ [[(âˆž,1)-topos]] $\mathcal{X}$:

the collection of $U$-probes of $X$ is a [[subobject]] of the collection of [[(âˆž,1)-topos]]-morphisms from $U$ to $\mathcal{X}$:
...
We think of $\mathcal{X}$ as the [[(âˆž,1)-topos]] of [[âˆž-stack]]s on a category of open subsets of a would-be space $X$, only that this would be space $X$ might not have an independent existence as a space apart from $\mathcal{X}$. The available entity closest to it is the [[terminal object]] ${*}_{\mathcal{X}} \in \mathcal{X}$. 

To say that $\mathcal{X}$ is _modeled on $\mathcal{G}$_ means that among all the [[âˆž-stack]]s on the would-be space a [[structure sheaf]] of functions with values in objects of $\mathcal{G}$ is singled out: for each object $V \in \mathcal{G}$ there is a [[structure sheaf]] $\mathcal{O}(-,V) \in \mathcal{X}$, naturally in $V$.

This yields an [[(âˆž,1)-functor]]
...
An [[(âˆž,1)-functor]] $\mathcal{O} : \mathcal{G} \to \mathcal{X}$  is a **$\mathcal{G}$-valued structure sheaf** on the [[(âˆž,1)-topos]] if

* it preserves finite [[limit]]s 

* and sends covering coproducts $(\coprod_i U_i) \to U$ to [[effective epimorphism]]s.

A pair $(\mathcal{X}, \mathcal{O})$ of an [[(âˆž,1)-topos]] $\mathcal{X}$ equipped with $\mathcal{G}$-valued [[structure sheaf]] $\mathcal{O} : \mathcal{G} \to \mathcal{X}$ we call a [[structured (âˆž,1)-topos]].
...
A **concrete [[âˆž-stack]] $X$ modeled on $\mathcal{G}$** is 

* an [[(âˆž,1)-topos]] $\mathcal{X}$ ("of $\infty$-stacks on $X$")
...
The fundamental **example** for [[structured (âˆž,1)-topos]]es
...
then the obvious [[(âˆž,1)-functor]] $f^* : \mathcal{L}Top(\mathcal{G}) \to \mathcal{L}Top(\mathcal{G}')$ admits a [[left adjoint]]
...
the archetypical [[(âˆž,1)-topos]] [[âˆžGrpd]], being finite [[limit]]-preserving functors $\mathcal{G}_{disc}^{op} \to \infty Grpd$
...
For every $U \in \mathcal{G}$ there is naturally induced a [[coverage|topology]] on the [[over category]] $Pro(\mathcal{G})/U$. Define the [[(âˆž,1)-topos]]
...
naturally to be thought of as the [[(âˆž,1)-topos]] of [[âˆž-stack]]s _on $U$_ .

This is canonically equipped with a [[(âˆž,1)-functor]]
...
For every $U \in \mathcal{G}$ the pair $(Spec U, \mathcal{O}_{Spec U})$ is indeed a [[structured (âˆž,1)-topos]] and is indeed equivalent to the $Spec^{\mathcal{G}} U$ defined more abstractly above.
...
We have seen that $\mathcal{G}$-[[structured (âˆž,1)-topos]]es are those general spaces modeled on $\mathcal{G}$ that are well-behaved in that at least they do have an "underlying topological structure" in the form of an underlying [[(âˆž,1)-topos]]. But such concrete spaces may still be very different from the model objects in $\mathcal{G}$.
...
But in many situations one would like to regard $\mathcal{G}$-[[structured (âˆž,1)-topos]]es that are not globally but _locally_ equivalent to objects in $\mathcal{G}$. This is supposed to be captured by the following definition.

+-- {: .un_def}
###### Definition
**[[Structured Spaces|StSp, def. 2.3.9]]

A [[structured (âˆž,1)-topos]] $(\mathcal{X}, \mathcal{O})$ is a **$\mathcal{G}$-generalized scheme** if
...
> **warning** the following statements really pertain to pregeometries, not geometries. for the moment this here is glossing over the difference between the two. See [[geometry (for structured (âˆž,1)-toposes)]] for the details.
...
* ordinary [[schemes]] are those $(CRing^{fin})^{op}$-[[generalized scheme]]s whose underlying [[(âˆž,1)-topos]] is
...
* Finally, to make contact with the application to the derived moduli stack of derived elliptic curves, it seems that in [[Spectral Schemes]] a derived Deligne-Mumford stack (with derived in the sense of having replaced ordinary commutative rings by [[E-âˆž ring]]s) is gonna be a 1-localic  $(E_\infty Ring^{fin})^{op}$-[[generalized scheme]].
