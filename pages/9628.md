...
In order to disentangle conceptual from technical aspects, we first discuss [[discrete âˆž-groupoid|geometrically discrete higher groupoids]]. The results of this discussion then in particular help to suggest what the right definition of "higher Lie groupoid" in the ontext of higher convolution algebras should be in the first place.
...
This iterated internalization on the codomain of the groupoid convolution algebra functor has a natural analog on its domain: a 2-groupoid we may present by a [[double groupoid]], namely a [[groupoid object in an (âˆž,1)-category]] in [[Grpd]] which is 3-[[coskeleton|coskeletal]] as a [[simplicial object]] in [[Grpd]].

+-- {: .num_remark}
###### Remark

Given a [[groupoid object in an (âˆž,1)-category|groupoid object]] $\mathcal{G}_\bullet$ in the [[(2,1)-topos]] [[Grpd]] hence a [[double groupoid]], applying the groupoid convolution algebra $(2,1)$-functor $C$ to the corresponding [[simplicial object]] $\mathcal{G}_\bullet \in Grpd^{\Delta^{op}}$ yields:
...
Let $R$ be an [[E-âˆž ring]] and let $A$ an [[E-âˆž algebra]] over $R$.
...
1. $A^R_\bullet(X)$ is a left $A^R_\bullet(A)$-module for every $R$-[[âˆž-module]] $X$.
...
* [[Georges Maltsiniotis]], _GroupoÃ¯des quantiques_, Comptes R
Rendus Acad. Sci. Paris __314__, pp. 249-252 (1992) [ps](http://people.math.jussieu.fr/~maltsin/ps/GRN-1.PS)
* G. Maltsiniotis, _Quasi-groupoÃ¯des quantiques_ (travail en commun avec A. BruguiÃ¨res), C.R. Acad. Sci. Paris __319__, pp. 933-936 (1994) [ps](http://people.math.jussieu.fr/~maltsin/ps/N-QSBIG.PS)
