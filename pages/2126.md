...
The **singular cohomology** (also **Betti cohomology**) of a [[topological space]] $X$ is the [[cohomology]] in [[âˆžGrpd]] of its [[fundamental âˆž-groupoid]] $\Pi(X)$:
...
With $\infty Grpd$ [[presentable (âˆž,1)-category|presented]] by the category [[sSet]] of [[simplicial set]]s, the fundamental $\infty$-groupoid $\Pi(X)$ is modeled by the [[Kan complex]] 
...
(meaning: any open subset $U\subset X$ has an open cover $W$ by open subsets $W_i\subset U$ that are contractible inÂ $U$),
then the [[sheaf cohomology]] ofÂ $X$ is isomorphic
to the singular cohomology ofÂ $X$ for any abelian
