...
The notion of adjunction between two [[(âˆž,1)-functors]] generalizes the notion of [[adjoint functors]] from [[category theory]] to [[(infinity,1)-category|(âˆž,1)-category theory]].
...
* One definition of ordinary adjoint functors says that a pair of functors $C \stackrel{\overset{L}{\leftarrow}}{\underset{R}{\to}} D$ is an adjunction if there is a [[natural transformation|natural isomorphism]]
  ...
  The analog of this definition makes sense very generally in [[(âˆž,1)-category theory]], where $Hom_C(-,-) : C^{op} \times C \to \infty Grpd$ is the $(\infty,1)$-categorical hom-object.
...
Using the [[(âˆž,1)-Grothendieck construction]] the notion of cograph of a functor has an evident generalization to $(\infty,1)$-categories.
...
$\epsilon : Id_D \to R \circ L$ -- a morphism in the [[(âˆž,1)-category of (âˆž,1)-functors]] $Func(D,D)$ -- such that for all $d \in D$ and $c \in C$ the induced morphism
...
is an [[equivalence of âˆž-groupoids]].
...
We make use here of the explicit realization of the [[(âˆž,1)-Grothendieck construction]] in its incarnation for [[quasi-categories]]: here an [[(âˆž,1)-functors]] $L : D \to C$ may be regarded as a map $\Delta[1]^{op} \to $ [[(âˆž,1)Cat]], which corresponds under the Grothendieck construction to a [[Cartesian fibration]] of [[simplicial sets]] $coGraph(L) \to \Delta[1]$. 
...
Two [[(âˆž,1)-functors]] $L : C \to D$ and $R : D \to C$ are called **adjoint** -- with $L$ _left adjoint_ to $R$ and $R$ _right adjoint_ to $L$ if
...
Moreover, even the choice of homotopy is unique, up to ever higher homotopy, i.e. the collection of all right adjoints to $L$ forms a [[contractible]] [[âˆž-groupoid]], in the following sense:

Let $Func^L(C,D), Func^R(C,D) \subset Func(C,D)$ be the full sub-quasi-categories on the [[(âˆž,1)-category of (âˆž,1)-functors]] between $C$ and $D$ on those functors that are left adjoint and those that are right adjoints, respectively. Then there is a canonical [[equivalence of quasi-categories]] 
$$
...
* $R$ is a [[full and faithful (âˆž,1)-functor]] precisely is the counit $L R \stackrel{}{\to} Id$ is an [[equivalence of quasi-categories|equivalence]] of [[(âˆž,1)-functor]]

  In this case $C$ is a [[reflective (âˆž,1)-subcategory]] of $D$.

* $L$ is a [[full and faithful (âˆž,1)-functor]] precisely is the unit $Id \to L R$ is an [[equivalence of quasi-categories|equivalence]] of [[(âˆž,1)-functors]].
...
be a pair of adjoint $(\infty,1)$-functors where the $(\infty,1)$-category $C$ has all [[(âˆž,1)-pullback]]s. 

Then for every object $X \in C$ there is induced a pair of adjoint $(\infty,1)$-functors between the [[over-(âˆž,1)-categories]]
...
* $R/X$ is the composite
  ...
  of the evident functor induced by $R$ with the [[(âˆž,1)-pullback]] along the $(L \dashv R)$-[[unit of an adjunction|unit]] at $X$.
...
A pair of adjoint $(\infty,1)$-functors $(L \dashv R) : C \stackrel{\leftarrow}{\hookrightarrow} D$ where $R$ is a [[full and faithful (âˆž,1)-functor]] exhibits $C$ as a [[reflective (âˆž,1)-subcategory]] of $D$. This subcategory and the composite $R \circ L : D \to D$ are a [[localization of an (âˆž,1)-category|localization]] of $D$.
...
The proof that a [[Quillen adjunction]] of [[model categories]] induces an adjunction of [[(âˆž,1)-categories]] is recorded in
...
[[!redirects adjoint (âˆž,1)-functor]]
[[!redirects adjoint (âˆž,1)-functors]]

[[!redirects adjoint âˆž-functor]]
[[!redirects adjoint âˆž-functors]]


[[!redirects (âˆž,1)-adjunction]]
[[!redirects (âˆž,1)-adjunctions]]

[[!redirects adjunction of (infinity,1)-categories]]
[[!redirects adjunctions of (infinity,1)-categories]]

[[!redirects adjunction of (âˆž,1)-categories]]
[[!redirects adjunctions of (âˆž,1)-categories]]
