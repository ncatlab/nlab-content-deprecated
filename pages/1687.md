...
Given an [[(âˆž,1)-topos]] $\mathcal{X}$, a **gerbe** in $\mathcal{X}$ is an [[object]] $\mathcal{G} \in \mathcal{X}$ that is
...
The first condition says that a gerbe is an object in the [[(2,1)-topos]] $\tau_{\leq 1 } \mathcal{X} \hookrightarrow \mathcal{X}$ inside $\mathcal{X}$. This means that for $C$ any [[(âˆž,1)-site]] of definition for $\mathcal{X}$, a gerbe is a [[(2,1)-sheaf]] on $C$, $\mathcal{G} \in Sh_{(2,1)}(C)$: a [[stack]] on $C$.
...
Equivalent to this is the [[over-(âˆž,1)-topos|over-(2,1)-topos]] $\tau_{\leq 1} \mathcal{H}/j(X)$, where $\tau_{\leq 1}\mathcal{H} := Sh_{(2,1)}(C)$ is the [[big topos|big]] [[(2,1)-topos]] over $C$ (and $j$ denotes its [[(âˆž,1)-Yoneda embedding|(2,1)-Yoneda embedding]]).

Since this $\mathcal{H}$ is a [[cohesive (âˆž,1)-topos]] we may think of its objects a general [[topological âˆž-groupoid|continuous âˆž-groupoid]]s or [[âˆž-Lie groupoid|smooth âˆž-groupoid]]s. In large parts of the literature coming after [Giraud](#Giraud) gerbes, or related structures equivalent to them, are described this way in terms of [[topological groupoid]]s and [[Lie groupoid]]s. This perspective is associated with the notion of a _[[bundle gerbe]]_ .


### $G$-Gerbes

We discuss gerbes that have a "strucure group" $G$ akin to a [[principal bundle]]. Indeed, while not the same concept, these _$G$-gerbes_ are [[equivalence in an (âˆž,1)-category|equivalent]] to $AUT(G)$-[[principal 2-bundle]]s, for $AUT(G)$ the [[automorphism 2-group]] of $G$.
...
any [[point of a topos|topos point]], the [[stalk]] functor $x^*$, being an [[inverse image]] is [[left exact functor|left exact]] and hence preserves [[categorical homotopy groups in an (âˆž,1)-topos|homotopy sheaves]] and [[terminal object]]s. It follows that the 0th homotopy sheaf is trivial
...
More generally, by the discussion at [[looping and delooping]] we have in an [[equivalence of (âˆž,1)-categories]]
...
between the [[âˆž-group]] objects in the ambient [[(âˆž,1)-topos]] $\mathcal{X}$ and the [[pointed object|pointed]] [[connected]] objects.

It follows that for a gerbe $P$ that admits a global section $* \to P$ the above relation holds not only [[stalk]]-wise, but globally: it is the [[delooping]] of its own first [[categorical homotopy groups in an (âˆž,1)-topos|sheaf of homotopy groups]]
...
Let $G \in Grp(\mathcal{X})$ be a group object. A gerbe $P \in \mathcal{X}$ is a **$G$-gerbe** if there exists an [[effective epimorphism]] $U \to *$ and an [[equivalence in an (âˆž,1)-category|equivalence]]
...
* $\mathcal{X} = Sh_{(\infty,1)}(Op(X))$ is the [[(âˆž,1)-category of (âˆž,1)-sheaves]] on the [[category of open subsets]] of $X$;
...
Let $\mathcal{X}$ be any ambient [[(âˆž,1)-topos]].

Let $G \in Grp(\mathcal{X}) \subset \infty Grpd(\mathcal{X})$ be a [[group object]] (a [[0-truncated]] [[âˆž-group]]).
...
for the [[core]] of the full [[sub-(âˆž,1)-category]] on $G$-gerbes in $\mathcal{X}$.
...
In the general perspective of [[(âˆž,1)-topos theory]] this appears as  ([JardineLuo, theorem 23](#JardineLuo)). 
...
Instead, under the above equivalence a gerbe is identified with the [[associated âˆž-bundle]] with fibers $\mathbf{B}G$ 
...
* [[principal âˆž-bundle]] / [[associated âˆž-bundle]]  / [[âˆž-gerbe]]
...
* [[J. Giraud]], _Cohomologie non abÃ©lienne_ , Springer  (1971)
...
A discussion from the point of view of [[(âˆž,1)-topos theory]] is in 

* [[Rick Jardine]], Z. Luo, _Higher order principal bundles_ , K-theory (2004) ([web](http://www.math.uiuc.edu/K-theory/0681/))
 {#JardineLuo}

The definition for $n$-gerbes as $n$-truncated and $n$-connected objects (see [[âˆž-gerbe]]) is in
