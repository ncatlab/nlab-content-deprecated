...
The notion of [[limit]] and [[colimit]] generalize from [[category theory]] to [[(âˆž,1)-category theory]]. One model for [[(âˆž,1)-categories]] are [[quasi-categories]]. This entry discusses limits and colimits in quasi-categories.
...
C$ an [[(âˆž,1)-functor]] (a morphism of the underlying [[simplicial set]]s) , the **limit** over $F$ is, if it exists, the [[terminal object in a quasi-category|quasi-categorical terminal object]] in the [[over quasi-category]] $C_{/F}$:
...
$f \colon K \to \mathcal{C}$ be a [[diagram]] with $(\infty,1)$-colimiting [[cocone]] $\tilde f \colon K \star \Delta^0 \to \mathcal{C}$. Then the induced map of [[slice (âˆž,1)-categories|slice quasi-categories]]
...
For $F \colon \mathcal{K} \to \mathcal{C}$ a diagram in an $(\infty,1)$-category and $\underset{\leftarrow}{\lim} F$ its limit, there is a natural [[equivalence of (âˆž,1)-categories]] 

$$
  \mathcal{C}_{/F} \simeq \mathcal{C}_{/\underset{\leftarrow}{\lim} F}
$$

between the [[slice (âˆž,1)-categories]] over $F$ (the $(\infty,1)$-category of $\infty$-[[cones]] over $F$) and over just $\underset{\leftarrow}{\lim}F$.
...
The right functor is an equivalence by prop. \ref{SlicingOverLimitingCone}. The left functor is induced by restriction along an op-[[final (âˆž,1)-functor]] (by the Examples discussed there) and hence is an equivalence by the discussion at _[[slice (âˆž,1)-category]]_ ([Lurie, prop. 4.1.1.8](#Lurie)).
...
By the discussion at [[adjoint (âˆž,1)-functor]] ([[Higher Topos Theory|HTT, prop. 5.2.2.8]]) this requires exhibiitng a morphism $\eta : Id_{Func(K,C)} \to const colim$ in $Func(Func(K,C),Func(K,C))$ such that for every $f \in Func(K,C)$ and $Y \in C$ 
...
Let $\kappa$ be a [[regular cardinal]]. An [[(âˆž,1)-category]] $C$ has all $\kappa$-small limits precisely if it has [[equalizer]]s and $\kappa$-small [[product]]s.
...
A central ingredient in the proof is the fact, discused at [[(âˆž,1)-category of (âˆž,1)-functors]] and at [[model structure on functors]], that [[sSet]]-[[enriched functor]]s do model [[(âˆž,1)-functor]]s, in that for $A$ a [[combinatorial simplicial model category]], $S$ a [[quasi-category]] and $\tau(S)$ the corresponding $sSet$-category under the left adjoint of the [[homotopy coherent nerve]], we have an [[equivalence of quasi-categories]]
...
The quasi-category $N(A^\circ)$ [[presentable (âˆž,1)-category|presented]] by a [[combinatorial simplicial model category]] $A$ has all small quasi-categorical limits and colimits.
...
Since $(\infty,1)$-categories equivalent to those of the form $N(A^\circ)$ for $A$ a [[combinatorial simplicial model category]] are precisely the [[locally presentable (âˆž,1)-categories]], it follows from this in particular that every locally presentable $(\infty,1)$-category has all limits and colimits.
...
We expect for $S, T \in $ [[âˆžGrpd]] and for $const_T : S \to \infty Grpd$ the constant diagram, that 
...
Correspondingly, for $C$ an $(\infty,1)$-category with colimits, it is [[copower|tensored]] over [[âˆžGrpd]] by setting
...
Every [[âˆž-groupoid]] $S$ is the $(\infty,1)$-colimit in [[âˆžGrpd]] of the constant diagram on the [[point]] over itself:
...
$(\infty,1)$-tensoring and $(\infty,1)$-cotensoring of $C$ over [[âˆžGrpd]]
...
For $C$ an $(\infty,1)$-category, $X : D \to C$ a diagram, $C/X$ the [[over-(âˆž,1)-category]] and $F : K \to C/X$ a diagram in the over-$(\infty,1)$-category, then the [[(âˆž,1)-limit]] $\lim_{\leftarrow} F$ in $C/X$ coincides with the $(\infty,1)$-limit $\lim_{\leftarrow} F/X$ in $C$. 
...
Limits and colimits over a [[(âˆž,1)-functor]] with values in the [[(âˆž,1)-category]] [[âˆž-Grpd]] of [[âˆž-groupoids]] may be reformulated in terms of the  [[universal fibration of (âˆž,1)-categories]], hence in terms of the [[(âˆž,1)-Grothendieck construction]].

Let [[âˆžGrpd]] be the [[(âˆž,1)-category]] of [[âˆž-groupoid]]s. Let the [[(âˆž,1)-functor]] $Z|_{Grpd} \to \infty Grpd^{op}$ be the [[universal fibration of (infinity,1)-categories|universal âˆž-groupoid fibration]] whose fiber over the object denoting some $\infty$-groupoid is that very $\infty$-groupoid.
...
an [[(âˆž,1)-functor]]. Recall that the [[Cartesian fibration|coCartesian fibration]] $E_F \to X$ classified by $F$ is the pullback of the [[universal fibration of âˆž-groupoids]] $Z|_{Grpd}$ along F:
...
* The $\infty$-colimit of $F$ is equivalent to the [[(âˆž,1)-Grothendieck construction]] $E_F$:

  $$
    \underset{\longrightarrow}{\lim} F \simeq E_F 
  $$

* The $\infty$-limit of $F$ is equivalent to the [[âˆž-groupoid of sections]] of $E_F \to X$
...
The form of the statement in prop. \ref{InfinityGroupoidalCoLimitsViaIntegrationAndSlicing} is the special case of the general form of [[internal (co-)limits]], here internal to the [[(âˆž,1)-topos]] [[âˆžGrpd]] with $Core(\inftyGrpd_{small})$ its small [[object classifier]]. See at _[internal (co-)limit -- Groupoidal homotopy (co-)limits](internal+%28co-%29limit#ExamplesInfinityGroupoidal)_ for more on this.
...
For $F : D \to $ [[(âˆž,1)Cat]] an [[(âˆž,1)-functor]], its $\infty$-colimit is given by forming the [[(âˆž,1)-Grothendieck construction]] $\int F$ of $F$ and then inverting the [[Cartesian morphism]]s.
...
For $C$ an ordinary [[category]] that admits small [[limit]]s and [[colimit]]s, and for $K$ a [[small category]], the [[functor category]] $Func(D,C)$ has all small limits and colimits, and these are computed objectwise. See [[limits and colimits by example]]. The analogous statement is true for an [[(âˆž,1)-category of (âˆž,1)-functors]].
...
* The [[(âˆž,1)-category of (âˆž,1)-functors]] $Func(D,C)$ has all $K$-indexed colimits;
...
* **$(\infty,1)$-limit
  ...
  * [[finite (âˆž,1)-limit]], [[relative (âˆž,1)-limit]]
...
* [[AndrÃ© Joyal]], _Quasi-categories and Kan complexes_ Journal of Pure and Applied Algebra 175 (2002), 207-222.

A brief survey is on page 159 of

* [[AndrÃ© Joyal]], _The theory of quasicategories and its applications_ lectures at [Simplicial Methods in Higher Categories](http://www.crm.es/HigherCategories/), ([pdf](http://www.crm.cat/HigherCategories/hc2.pdf))
...
[[!redirects (âˆž,1)-limit]]
[[!redirects (âˆž,1)-colimit]]
[[!redirects (âˆž,1)-limits]]
[[!redirects (âˆž,1)-colimits]]

[[!redirects (âˆž,1)-product]]
[[!redirects (âˆž,1)-coproduct]]
[[!redirects (âˆž,1)-products]]
[[!redirects (âˆž,1)-coproducts]]

[[!redirects (âˆž,1)-pushout]]
[[!redirects (âˆž,1)-pushouts]]
...
[[!redirects limit in an (âˆž,1)-category]]
[[!redirects limits in an (âˆž,1)-category]]
[[!redirects limits in (âˆž,1)-categories]]
...
[[!redirects colimit in an (âˆž,1)-category]]
[[!redirects colimits in an (âˆž,1)-category]]
[[!redirects colimits in (âˆž,1)-categories]]

[[!redirects (âˆž,1)-powering]]
[[!redirects (âˆž,1)-tensoring]]
[[!redirects (âˆž,1)-copower]]