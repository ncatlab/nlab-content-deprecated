...
The notion of _principal $\infty$-bundle_ is a [[vertical categorification|categorification]] of [[principal bundle]] from [[groups]] and [[groupoids]] to [[âˆž-groupoids]], or rather from parameterized groupoids (generalized spaces called [[stacks]]) to parameterized $\infty$-groupoids (generalized spaces called [[âˆž-stacks]]).
...
* [[universal principal âˆž-bundle]]
...
We define $G$-principal $\infty$-bundles in the general context of an [[âˆž-stack]] [[(âˆž,1)-topos]] $\mathbf{H}$, with $G$ a [[groupoid object in an (âˆž,1)-category|group object in the (âˆž,1)-topos]].
...
Conversely, for $G \in \mathbf{H}$ we say an object $\mathbf{B}G$ is a [[delooping]] of $G$ if it has an essentially unique point and if $G \simeq \Omega \mathbf{B}G$. We call $G$ an **[[âˆž-group]]**. More in detail, its structure as a [[groupoid object in an (âˆž,1)-category|group object in an (âˆž,1)-category]] is exhibited by the [[ÄŒech nerve]] 
...
To every cocycle $g : X \to \mathbf{B}G$ is canonically associated its [[homotopy fiber]] $P \to X$, the [[(âˆž,1)-pullback]]
...
We discuss now that $P$ canonically has the structure of a $G$-[[principal âˆž-bundle]] and that $\mathbf{B}G$ is the [[fine moduli space]] for $G$-principal $\infty$-bundles.
...
Let $G$ be a [[groupoid object in an (âˆž,1)-category|group object in the (âˆž,1)-topos]] $\mathbf{H}$. A **principal [[action]]** of $G$ on a morphism $(P \to X) \in \mathbf{H}$ is 
a [[groupoid object in an (âˆž,1)-category|groupoid object]] $P//G$ that sits over $*//G$ in that we have a 
...
and such that $P \to X$ exhibits the [[(âˆž,1)-colimit]]
...
This principality condition asserts that the groupoid object $P//G$ is [[groupoid object in an (infinity,1)-category|effective]]. By [[(âˆž,1)-topos|Giraud's axioms characterizing (âˆž,1)-toposes]], every groupoid object in $\mathbf{H}$ is effective.
...
The defining [[(âˆž,1)-pullback]] square for $P \times_X P$ is 
...
of which the one on the right is the defining one for $G$ and the remaining one on the left is just an [[(âˆž,1)-product]].  
...
$G$-[[principal âˆž-bundle]] if the colimit over $P//G \to *//G$
...
We have an [[equivalence of (âˆž,1)-categories]]
...
For some comments on the generalization of the notion of [[connection on a bundle]] to principal $\infty$-bundles see [[schreiber:differential cohomology in an (âˆž,1)-topos -- survey]].

## Concrete realizations 

We discuss realizations of the general definition in various
[[(âˆž,1)-toposes]] $\mathbf{H}$.

### In topological spaces 

The following general construction was originally due to
Quillen and defines principal _groupoid_ $\infty$-bundles in the [[(âˆž,1)-topos]] [[Top]] 
...
be the [[(âˆž,1)-topos]]  of [[âˆž-stacks]]
...
For $C$ a [[site]] of test [[space]], -- for instance duals of [[algebras over a Lawvere theory]] as described at [[function algebras on infinity-stacks]] -- let $\mathbf{H} = Sh_{(\infty,1)}(C)$ be the [[(âˆž,1)-topos]]  of [[âˆž-stacks]] on $C$. This is a [[gros topos]].

#### Smooth principal $\infty$-bundles

Smooth principal $\infty$-bundles are realized in the $\infty$-[[Cahiers topos]] as described in some detail at [[âˆž-Lie groupoid]].

In this context there is a notion of [[connection on a principal âˆž-bundle]].

## Examples

### Ordinary principal bundles

For $G$ an ordinary [[Lie group]], a $G$-principal bundle in the $(\infty,1)$-topos $\mathbf{H} = $ [[âˆžLieGrpd]] is an ordinary $G$-[[principal bundle]].

### Circle $n$-bundles

For $G = \mathbf{B}^{n-1} U(1) \in $ [[âˆžLieGrpd]], the <a href="http://ncatlab.org/nlab/show/Lie+infinity-groupoid#BnU1">circle Lie n-group</a>, a $G$-principal $\infty$-bundle is a **circle $n$-bundle**.
...
A principal $\infty$-bundle over a [[0-connected]] object / [[delooping]] object $\mathf{B}K$ is a _[[normal morphism of âˆž-groups]]_. See there for more details.
...
* **principal $\infty$-bundle** / [[associated âˆž-bundle]] / [[âˆž-gerbe]], [[twisted âˆž-bundle]], [[groupoid-principal âˆž-bundle]]
...
on [[associated âˆž-bundle]]s.

The fully general abstract formalization in [[(âˆž,1)-topos theory]] as discussed here was first indicated in 
...
* {#NSS12} [[Thomas Nikolaus]], [[Urs Schreiber]], [[Danny Stevenson]], _[[schreiber:Principal âˆž-bundles -- models and general theory]]_
...
* E. Dror, [[William Dwyer]], and [[Daniel Kan]], _Equivariant maps which are self homotopy equivalences_, Proc. Amer. Math. Soc. 80 (1980), no. 4, 670â€“672 ([JSTOR](http://www.jstor.org/stable/2043448))
...
[[!redirects principal âˆž-bundle]]
[[!redirects principal âˆž-bundles]]