...
In the [[(âˆž,1)-topos]] [[Top]] the construction of a [[loop space]] of a given [[topological space]] is familiar.

This construction may be generalized to any other [[(âˆž,1)-topos]] and in fact to any other [[(âˆž,1)-category]] with [[homotopy pullbacks]].


## Definition

Loop space objects are defined in any [[(âˆž,1)-category]] $\mathbf{C}$ with [[homotopy pullbacks]]: for $X$ any [[pointed object]] of $\mathbf{C}$ with point ${*} \to X$, its [[loop space object]] is [[generalized the|the]] [[homotopy pullback]] $\Omega X$ of this point along itself:
...
Usually the [[(âˆž,1)-category]] in question is [[presentable (infinity,1)-category|presented]] by concrete 1-categorical data, such as that of a [[model category]]. In that case the above [[homotopy pullback]] has various realizations as an ordinary [[pullback]].
