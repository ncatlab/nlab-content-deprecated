...
An [[(âˆž,1)-category]] $C$ has [[finite limit|finite]] [[(âˆž,1)-limits]] if it has a [[terminal object in a quasi-category|terminal object]] and for every [[object]] $x \in X$, the [[over-(âˆž,1)-category]] $C_{/x}$ has finite [[products]]. 

We say that such a $C$ is **locally cartesian closed** if moreover $C_{/x}$ is a [[cartesian closed (âˆž,1)-category]] for every object $x$.  This is equivalent to asking that the pullback functor $f^*\colon C_{\y} \to C_{\x}$, for any $f\colon x\to y$ in $C$, has a right adjoint $\Pi_f$.

## Examples

* Every (Grothendieck) [[(âˆž,1)-topos]] is locally cartesian closed.  This follows from the [[universal colimit|universality of colimits]] and the [[adjoint functor theorem]].
...
The following theorem should be compared with the fact that every [[locally presentable (âˆž,1)-category]] admits a presentation by a [[Cisinski model category]], indeed by a [[left Bousfield localization]] of a [[global model structure on simplicial presheaves]].
...
1. [[(âˆž,1)-colimit|(âˆž,1)-Colimits]] in $C$ are [[universal colimits|stable under pullback]].
...
###### Theorem
**(Cisinski, Shulman)** Assuming the initiality of the syntactic 1-category, every locally presentable locally cartesian closed $(\infty,1)$-category admits an interpretation of [[Martin-LÃ¶f type theory]] with [[dependent sum types]], [[dependent product types]], and [[identity types]] ("[[homotopy type theory]]").
...
This includes in particular all ([[âˆž-stack]]-) [[(âˆž,1)-toposes]].  The latter should in addition possess [[univalence|univalent universes]], but that is at present known only weakly except for a restricted class.  See also _[[internal logic of an (âˆž,1)-topos]]_.
...
A step in the latter direction was conjectured in ([Joyal 2011](#Joyal11)), and established in [[Chris Kapulkin]]â€™s PhD thesis (see [Kapulkin 14](#Kapulkin14), [Kapulkin 15](#Kapulkin15)): 
...
However, this â€œsyntactic $(\infty,1)$-categoryâ€ will not in general be locally presentable, lacking appropriate colimits.  There is thus a mismatch between the two best statements we have at present, for the two directions of the internal language correspondence.
...
Characterizations of locally presentable locally cartesian closed $(\infty,1)$-categories (as [[locally presentable (âˆž,1)-categories]] with [[universal colimits]]) are discussed in section 6.1 of
...
* {#GepnerKock12} [[David Gepner]], [[Joachim Kock]], "Univalence in locally cartesian closed âˆž-categories", [arXiv](http://arxiv.org/abs/1208.1749)


[[!redirects locally cartesian closed (âˆž,1)-category]]
[[!redirects locally cartesian closed (infinity,1)-categories]]
[[!redirects locally cartesian closed (âˆž,1)-categories]]
