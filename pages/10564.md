...
of $G$ regarded as an [[âˆž-group]], see at _[[âˆž-action]]_ for how that encodes actions on structures above it):
...
Based on this one may want to consider an $E$-[[(âˆž,1)-module bundle|âˆž-line bundle]] over the full space $M_{S^1}$ and take the genuine $E$-equivariant cohomology to be the [[global sections]] of that. (Specifically in [[elliptic cohomology]] that space $M_{S^1}$ is equivalent to the [[elliptic curve]] $C$ that gives the theory its name, but in some sense discussed [below](#InterpretationInQuantumFieldTheory) the spaces $M_{S^1}$ and $C$ arise conceptually differently and it is a fairly deep coincidence that they are in fact equivalent, which one may want to remember.)
...
Let $E \in CRing_\infty$ be an [[elliptic spectrum|elliptic]] [[E-âˆž ring]] [[spectrum]] with [[elliptic curve]] $A \to Spec E$. 
...
By [[Elmendorf's theorem]] we have a [[equivalence of (âˆž,1)-categories]]
...
Let the [[global points]] of the [[elliptic curve]] $A$ over $Spec E$ be equipped with an _orientation_ in the sense of a non-degenerate [[âˆž-group]] homomorphism of the form
...
to the [[slice (âˆž,1)-topos]] over $A_G$. 
...
be the [[left adjoint]] to the [[(âˆž,1)-Yoneda embedding]] as discussed at _[[function algebras on âˆž-stacks]]_.
...
Regard $\mathbf{B}G$ in [[SmoothâˆžGrpd]] = $Sh_\infty(SmthMfd)$. Then by the discussion at [[Lie group cohomology]] we have
...
The [[âˆž-group extension]] classified by $k \in \mathbb{Z} \in \pi_0\mathbf{H}(\mathbf{B}G, \mathbf{B}\mathbb{C}^\times)$ is the [[string 2-group]] at level $k$
...
To that end, pass to the larger [[(âˆž,1)-topos]] of [[smooth E-âˆž groupoids]] over the [[complex numbers]].

Let $\mathbb{G}_m$ there denote the object which to a pair consisting of a [[smooth manifold]] $U$ and an [[E-âˆž ring]] $R$ assigns

$$
  \mathbb{G}_m \;\colon\; (U, R) \mapsto GL_1(R) \otimes C^\infty(U,\mathbb{C}^\times) 
$$ 

hence the [[tensor product]] of the [[âˆž-group of units]] of $R$ with the underlying [[abelian group]] of [[smooth functions]] on $X$ with values in $\mathbb{C}^\times$. 

Let then $A \in CAlg_\infty$ be an [[E-âˆž ring]], and take now $B = Spec(A)$. Write 
...
are equivalently elements in the [[âˆž-group of units]] of $A$. So we should get an $A$-[[(âˆž,1)-module bundle]] modulated by
...
consider the [[string 2-group]] [[âˆž-group extension]]
...
The corresponding [[higher moduli stacks]] of [[flat âˆž-connections]] on an [[elliptic curve]] $T$ form the [[âˆž-group extension]]
...
bundle on the moduli space has the structure of a $K((q))$-[[âˆž-module]], hence of an actual [[spectrum]] ([Lurie, below remark 5.4](#Lurie)).
...
Given an [[E-âˆž ring]] $A$ with an oriented [[derived elliptic curve]] $\Sigma \to Spec(A)$ there are a priori two different $A$-[[âˆž-line bundles]] on $B Spin$.
...
where $\mathbb{S}$ is the [[sphere spectrum]], $GL_1(-)$ the [[âˆž-group of units]]-construction and $J$ the [[J-homomorphism]].  (This is what appears as $\mathcal{A}_s$ in [Lurie, middle of p.38](#Lurie)). Notice that by ([Ando-Blumberg-Gepner 10, section 8](#AndoBlumbergGepner10)), for the case $A = $ [[tmf]] this is equivalently the $A$-[[âˆž-line bundle]] [[associated âˆž-bundle|associated]] to the [[universal Chern-Simons line 3-bundle]]
...
(where in ([Lurie](#Lurie)) $Loc_{Spin}(\Sigma)$ is denoted $M_{Spin}$). Evaluating this bundle on [[global points]] yields the $A$-[[âˆž-line bundle]]
...
* {#Grojnowski94} I. Grojnowski, _Delocalised equivariant elliptic cohomology_ (1994), in _Elliptic cohomology_, volume 342 of London Math. Soc. Lecture Note Ser., pages 114â€“121. Cambridge Univ. Press, Cambridge, 2007 ([pdf](http://hopf.math.purdue.edu/Grojnowski/deloc.pdf))
...
elliptic cohomology_ . Geom. Topol., 7:91â€“153,
...
* {#NS} [[Joost Nuiten]], [[Urs Schreiber]], some notes 
