...
A _homotopy fiber sequence_ is a "long left-[[exact sequence]]" in an [[(âˆž,1)-category]]. (The dual concept is that of _[[cofiber sequence]]_.)

Traditionally fiber sequences have been considered in the context of [[homotopical category|homotopical categories]] such as [[model category|model categories]] and [[category of fibrant objects|Brown category of fibrant objects]] which [[presentable (infinity,1)-category|present]] the [[(âˆž,1)-category]] in question. In particular, classically this was considered for [[Top]] itself. In these cases they are obtained in terms of [[homotopy pullbacks]]. Since, as discussed there, the homotopy fiber of a morphism may be computed as the ordinary 1-categorical [[fiber]] of any [[fibration]] [[resolution]] of this morphism, one often also speaks of _fibration sequences_.
...
Usually, when looking at fibration sequences in 1-categorical contexts of the [[homotopy category of an (âˆž,1)-category]], one doesn't see these long fibration squences directly, but only "in cohomology".

This can be usefully understood as follows:

recall from [[cohomology]] that for $X$ and $A$ objects in an [[(âˆž,1)-category]] $C$ that is an [[(âˆž,1)-topos]], the $\infty$-groupoid of $A$-valued cocycle on $X$ is just $Hom_C(X,A)$, so that the corresponding cohomology classes are
...
where $Ho_C$ is the corresponding [[homotopy category of an (âˆž,1)-category]].
...
is again homotopy pullback diagram (of [[âˆž-groupoids]]); in particular the morphism $Hom_C(X,A \times_K B)\to Hom_C(X,A) \times_{Hom_{C}(X,K)} Hom_C(X,B)$ induced by the universal property of homotopy pullback is an equivalence. 
...
A morphism $f : B \to C$ in [[âˆžGrpd]] is an equivalence precisely if all its homotopy fibers over every point of $C$ are [[contractible]], i.e. are equivalent to $*$.

More generally, a morphism $f : B \to C$ in any [[(âˆž,1)-category]] $\mathcal{C}$ is an equivalence if for all objects $X \in \mathcal{C}$ all homotopy fibers of the morphism $\mathcal{C}(X,f) : \mathcal{C}(X,B) \to \mathcal{C}(X,C)$ are contractible.

=--

For more on this see [[n-truncated object of an (âˆž,1)-category]]. Also [[Higher Topos Theory|HTT, around example 5.5.6.13]]. For more on homotopy fibers of hom-spaces see [the section below](#OfFuncCats).
...
in [[âˆžGrpd]], where the point of $Hom_{\mathcal{C}}(X,C)$ is $X \to * \to C$ with $* \to C$ the point of $C$, so that $Hom_{\mathcal{C}}(X,A)$ is the homotopy fiber over this point of the morphism given by postcomposition with $B \to C$.
...
in the [[over quasi-category|over (âˆž,1)-category]] $\mathcal{C}_{/C}$.
...
Model all [[(âˆž,1)-categories]] as [[quasi-categories]]. 
...
Let $G$ be a [[group]] and let $\mathbf{B}G$ denote the corresponding one-object [[groupoid]] (in the world of [[âˆž-groupoids]]) or else the [[classifying space]] $\mathcal{B}G$.
...
in the ordinary 1-category of $n$-groupoids or spaces, using a replacement $\hat X \stackrel{\simeq}{\twoheadrightarrow} X$ of $X$ by an acyclic fibration (called "[[hypercover]]" in this context) (for instance the ÄŒech groupoid associated with an open cover of $X$). 
...
The same logic -- even the same diagrams -- work for [[principal 2-bundles]] and generally for [[principal âˆž-bundles]].

### Action groupoids {#ActionGroupoid}

Let $G$ be an [[âˆž-group]] in that $\mathbf{B}G$ is an [[âˆž-groupoid]] with a single object. An action of $G$ on an [[(âˆž,1)-category]] is an [[(âˆž,1)-functor]]

$$
  \rho : \mathbf{B}G \to (\infty,1)Cat
$$

to [[(âˆž,1)Cat]]. This takes the single object of $\mathbf{B}G$ to some $(\infty,1)$-category  $V$.

The _[[action groupoid]]_ $V//G$ is the [[limit in a quasi-category|(âˆž,1)-categorical colimit]] over the action:
...
classified by $\rho$ under the [[(âˆž,1)-Grothendieck construction]]. We obtain a [[fiber sequence]] to the left by adjoining the $(\infty,1)$-categorical pullback along the point inclusion $* \to \mathbf{B}G$
...
in $\mathbf{H} = $ [[âˆžGrpd]]/[[Top]], where $\mathbb{R}$ is the abelian group (under addition) of [[real number]], $\mathbb{Z}$ the abelian group of [[integer]]s, and where $\mathbb{R}/\mathbb{Z} = S^1 = U(1)$ is their [[quotient]] group, the circle group or [[unitary group]] $U(n)$ for $n=1$.
...
be its cohomology with coefficients in $A$, computed in [[âˆžGrpd]] which we may [[presentable (âˆž,1)-category|present]] by [[sSet]], where the [[fundamental âˆž-groupoid]] $\Pi(X)$ is the singular simplicial complex $Sing X$ and we have
...
in $\mathbf{H} = $ [[âˆžGrpd]]/[[Top]] is given (which modeled in [[sSet]] or in terms of [[CW-complex]]es in [[Top]] may be modeled by an ordinary pushout), and where $A \in \infty Grpd$ is some coefficient group object. Then applying the $(\infty,1)$-categorical hom $\mathbf{H}(-, A) : \mathbf{H}^{op} \to \infty Grpd$ yields the $\infty$-pullback diagram
...
A discussion of fiber sequences in terms of [[associated âˆž-bundles]] is in
...
[[!redirects (âˆž,1)-fiber]]
[[!redirects (âˆž,1)-fibers]]
[[!redirects (âˆž,1)-fibre]]
[[!redirects (âˆž,1)-fibres]]
