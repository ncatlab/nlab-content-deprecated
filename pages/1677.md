...
+-- {: .query}
If $A$ were a ring, then I\'d call $M$ an â€˜$A$-algebra'.  Unfortunately, that term can also be used for an $A$-model.  Also, even in ring theory, that term is usually only used when $A$ is commutative.  One might, following 'bimodule' (and 'bimodel') say 'bialgebra' in that case, but that also has another meaning.  So let\'s give up in that direction.

But it seems OK to me to call it an â€˜$A$-monad'.  ---Toby
