...
>  This is a subsection of the entry [[cohesive (âˆž,1)-topos]]. See there for background and context.
...
A [[cohesive (âˆž,1)-topos]] is a context of [[âˆž-groupoid]]s that are equipped with a [[geometry|geometric]] notion of _cohesion_ on their collections of [[object]]s and [[k-morphism]]s, for instance [[Euclidean-topological âˆž-groupoid|topological cohesion]] or [[smooth âˆž-groupoid|smooth cohesion]].
...
Here we consider one extra axiom on a [[cohesive (âˆž,1)-topos]] that does imply a good intrinsic notion of synthetic differential extension, compatible with the given notion of cohesion. We speak of *differential cohesion*.
...
We discuss [[extra structure]] on a [[cohesive (âˆž,1)-topos]]
...
equipped with an  [[adjoint quadruple]] of [[adjoint (âˆž,1)-functors]] of the form
...
where $i_!$ is  a [[full and faithful (âˆž,1)-functor|full and faithful]] and preserves [[finite products]].
...
This implies that also $i_*$ is a [[full and faithful (âˆž,1)-functor]].
...
By the characterizaton of full and faithful [[adjoint (âˆž,1)-functor]]s
the condition on $i_!$ is equivalent to $i^* i_! \simeq Id$. Since $(i^* i_! \dashv i^* i_*)$ it follows by essential uniqueness of [[adjoint (âˆž,1)-functor]]s that also $i^* i_* \simeq Id$.
...
This definition captures the characterization of an [[infinitesimal object]] as having a single [[global element|global point]] surrounded by an infinitesimal neighbourhood: as we shall see in more detail [below](#InfinitesimalPathsAndReduction), the [[(âˆž,1)-functor]] $i^*$ may be thought of as contracting away any infinitesimal extension of an object. Thus $X$ being an [infinitesimal object](#InfinitesimalObject) amounts to  $i^* X \simeq *$, and the [[adjoint (âˆž,1)-functor|(âˆž,1)-adjunction]] $(i_! \dashv i^*)$ then indeed guarantees that $X$ has only a single global point, since
...
a morphism of [[(âˆž,1)-topos]]es over [[âˆžGrpd]].
...
Moreover $i_*$ is necessarily a [[full and faithful (âˆž,1)-functor]].
...
We also consider the [[(âˆž,1)-monads]]/comonads induced from these reflections:
...
Let $C$ be an [[âˆž-cohesive site]]. We say a [[site]] $C_{th}$
...
Let $C$ be an [[âˆž-cohesive site]] and  $(i \dashv p) : C \stackrel{\overset{i}{\hookrightarrow}}{\underset{p}{\leftarrow}} C_{th}$ an [infinitesimal neighbourhood site](#InfinitesimalNeighBourhoodSite).

Then the [[(âˆž,1)-category of (âˆž,1)-sheaves]] on $C_{th}$ is a cohesive $(\infty,1)$-topos and the restriction $i^*$ along $i$ exhibits it as an [infinitesimal neighbourhood](#InfinitesimalNeighbourhoodIsOverInfGroupoid) of the cohesive $(\infty,1)$-topos over $C$.
...
Moreover, $i_!$ restricts on representables to the [[(âˆž,1)-Yoneda embedding]] factoring through $i$:
...
We [[presentable (âˆž,1)-category|present]] the [[(âˆž,1)-sheaf (âˆž,1)-category]] $Sh_{(\infty,1)}(C_{th})$ by the projective [[model structure on simplicial presheaves]] [[Bousfield localization of model categories|left Bousfield localized]] at the [[covering]] [[sieve]] inclusions
...
(as discussed at [[models for âˆž-stack (âˆž,1)-toposes|models for (âˆž,1)-sheaf (âˆž,1)-toposes]]).
...
This establishes the quadruple of [[adjoint (âˆž,1)-functor]]s as claimed.
...
Conversely this implies that $Sh_{(\infty,1)}(C_{th})$ is an [[âˆž-connected (âˆž,1)-topos]] over [[SmoothâˆžGrpd]], exhibited by the triple of adjunctions
...
This happens notably for the model of [[formal smooth âˆž-groupoids]] and all its variants such as formal [[complex analytic âˆž-groupoids]] etc. But in this case $(\flat^{rel} \dashv \sharp^{rel})$ does not provide  [[Aufhebung]] for $(\flat \dashv \sharp)$.
...
The [[counit of a comonad|counit]] of the relative flat modality is a [[formally Ã©tale morphism]].
...
a cohesive $(\infty,1)$-topos equipped with differential cohesion. These structures parallel the [structures in a general cohesive (âˆž,1)-topos](#Structures).
...
In the presence of [differential cohesion](#InfinitesimalCohesiveInfTopos) there is an infinitesimal analog of the [geometric paths âˆž-groupoids](#Paths).
...
Define the [[adjoint triple]] of [[adjoint (âˆž,1)-functor]]s corresponding to the [[adjoint quadruple]] $(i_! \dashv i^* \dashv i_* \dashv i^!)$:
...
* $\Im(X)$ is the **[[infinitesimal path âˆž-groupoid]]**
  of $X$;
...
* $\Re(X)$ is the **[[reduced cohesive âˆž-groupoid]]** underlying
  $X$.
...
Notice that for $f : X \to Y$ any [[morphism]] in any [[(âˆž,1)-topos]] $\mathbf{H}$, there is the corresponding [[base change geometric morphism]] between the [[over-(âˆž,1)-toposes]]
...
##### Formally smooth/Ã©tale/unramified morphisms
...
is an [[effective epimorphism in an (âˆž,1)-category|effective epimorphism]].
...
(induced from the [[adjoint quadruple]] $(i_! \dashv i^* \dashv i_* \dashv i^!)$, see there) is an [[effective epimorphism in an (âˆž,1)-category|effective epimorphism]].
...
By the condition that $i_!$ is a [[full and faithful (âˆž,1)-functor]] the second morphism here in an [[equivalence in an (âˆž,1)-category|equivalence]], as indicated, and hence the component of the composite on $X$ being an effective epimorphism is equivalent to the component $i_! X \to \mathbf{\Pi} i_! X$ being an effective epimorphism.
...
1. $f$ is a **[[formally Ã©tale morphism]]** if this morphism is an [[equivalence in an (âˆž,1)-category|equivalence]], hence if
...
An order-(-2) formally unramified morphism is equivalently a [[formally Ã©tale morphism]].

Only for [[0-truncated]] $X$ does formal smoothness together with formal unramifiedness imply formal Ã©taleness.
...
A morphism $f \colon X \to Y$ in $\mathbf{H}_{th}$ is **[[formally etale morphism|formally Ã©tale]]** if it is $\Im$-[[Pi-closed morphism|closed]], hence if its $\Im$-unit naturality square
...
The collection of [[formally Ã©tale morphisms]] in $\mathbf{H}$, def. \ref{FormalRelativeSmoothnessByCanonicalMorphism}, is closed under the following operations.

1. Every [[equivalence in an (âˆž,1)-category|equivalence]] is formally Ã©tale.

1. The composite of two formally Ã©tale morphisms is itself formally Ã©tale.
...
1. If
   ...
   is a [[diagram]] such that $g$ and $h$ are formally Ã©tale, then also $f$ is formally Ã©tale.

1. Any [[retract]] of a formally Ã©tale morphisms is itself formally Ã©tale.

1. The [[(âˆž,1)-pullback]] of a formally Ã©tale morphisms is formally Ã©tale if the pullback is preserved by $i_!$.
...
The second two statements follow by the [[pasting law]] for [[(âˆž,1)-pullback]]s: let $f : X \to Y$ and $g : Y \to Z$ be two
...
If $f$ and $g$ are formally Ã©tale then both small squares are pullback squares. Then the pasting law says that so is the outer rectangle and hence $g \circ f$ is formally Ã©tale. Similarly, if $g$ and $g \circ f$ are formally Ã©tale then the right square and the total reactangle are pullbacks, so the pasting law says that also the left square is a pullback and so also $f$ is formally Ã©tale.

For the fourth claim, let $Id \simeq (g \to f \to g)$ be a [[retract]] in the [[arrow category|arrow (âˆž,1)-category]] $\mathbf{H}^I$. By applying the natural transformation $\phi : i_! \to I_*$ we obtain a retract

$$
  Id \simeq ((i_! g \to i_*g) \to  (i_! f \to i_*f) \to (i_! g \to i_*g))
$$

in the category of squares $\mathbf{H}^{\Box}$. We claim that generally, if the middle piece in a retract in $\mathbf{H}^\Box$ is an [[(âˆž,1)-pullback]] square, then so is its retract sqare. This implies the fourth claim.

To see this, we use that

1. [[(âˆž,1)-limit]]s are computed by [[homotopy limit]]s in any [[presentable (âˆž,1)-category]] $C$ presenting $\mathbf{H}$;
...
For the last claim, consider an [[(âˆž,1)-pullback]] diagram
...
* the second square is a pullback, since $f$ is formally Ã©tale.
...
* therefore finally the third square is a pullback, by the other clause of the [[pasting law]]. Hence  $p$ is formally Ã©tale.
...
The properties listed in prop. \ref{PropertiesOfFormallyEtaleMorphisms} correspond to the axioms on the _[[open map]]s_ ("admissible maps") in a [[geometry (for structured (âˆž,1)-toposes)]]  ([Lurie, def. 1.2.1](#LurieStSp)). This means that a notion of formally Ã©tale morphisms induces a notion of [[locally algebra-ed topos|locally algebra-ed (âˆž,1)toposes]]/[[structured (âˆž,1)-toposes]] in a cohesive context. This is discuss in

* [[cohesive (âˆž,1)-topos -- structure âˆž-sheaves]].
...
$i_!$ is a [[full and faithful (âˆž,1)-functor]]. It follows that
...
is an [[equivalence in an (âˆž,1)-category|equivalence]]. As such it is in particular an [[effective epimorphism in an (âˆž,1)-category|effective epimorphism]].
...
We discuss how in differential cohesion $\mathbf{H}_{th}$ every object $X$ canonically induces its [[Ã©tale (âˆž,1)-topos]] $Sh_{\mathbf{H}_{th}}(X)$.


For $X \in \mathbf{H}_{th}$ any object in a differential cohesive $\infty$-topos, we formulate

1. the [[(âˆž,1)-topos]] denoted $\mathcal{X}$ or $Sh_\infty(X)$ of [[(âˆž,1)-sheaves]] over $X$, or rather of formally Ã©tale maps into $X$;

1. the [[structure (âˆž,1)-sheaf]] $\mathcal{O}_{X}$ of $X$.

The resulting structure is essentially that discussed ([Lurie, Structured Spaces](#Lurie)) if we regard $\mathbf{H}_{th}$ equipped with its formally Ã©tale morphisms, def. \ref{FormallyEtaleInHTh}, as a ([[large category|large]]) [[geometry for structured (âˆž,1)-toposes]].

One way to motivate this is to consider structure sheaves of flat differential forms. To that end, let $G \in Grp(\mathbf{H}_{th})$ a differential cohesive [[âˆž-group]] with [de Rham coefficient object](cohesive+%28infinity,1%29-topos+--+structures#deRhamCohomology) $\flat_{dR}\mathbf{B}G$ and for $X \in \mathbf{H}_{th}$ any differential homotopy type, the product projection
...
for the full [[sub-(âˆž,1)-category]] of the [[slice (âˆž,1)-topos]] over $X$ on those maps into $X$ which are formally Ã©tale, def. \ref{FormallyEtaleInHTh}.
...
The inclusion $\iota$ of def. \ref{EtaleSlice} is both [[reflective sub-(âˆž,1)-category|reflective]] as well as [[coreflective subcategory|coreflective]], hence it fits into an [[adjoint triple]] of the form
...
Therefore $(\mathbf{H}_{th})_{/X}^{fet}$, being a reflective subcategory of a [[locally presentable (âˆž,1)-category]], is (as discussed there) itself locally presentable. Hence by the [[adjoint (âˆž,1)-functor theorem]] it is now sufficient to show that the inclusion preserves all small [[(âˆž,1)-colimits]] in order to conclude that it also has a right [[adjoint (âˆž,1)-functor]].

So consider any [[diagram]] [[(âˆž,1)-functor]] $I \to (\mathbf{H}_{th})_{/X}^{fet}$ out of a [[small (âˆž,1)-category]]. Since the inclusion of $(\mathbf{H}_{th})_{/X}^{fet}$ is full, it is sufficient to show that the $(\infty,1)$-colimit over this diagram taken in $(\mathbf{H}_{th})_{/X}$ lands again in $(\mathbf{H}_{th})_{/X}^{fet}$ in order to have that $(\infty,1)$-colimits are preserved by the inclusion. Moreover, colimits in a slice of $\mathbf{H}_{th}$ are computed in $\mathbf{H}_{th}$ itself (this is discussed at _[slice category - Colimits](overcategory#LimitsAndColimits)_).
...
is an [[(âˆž,1)-pullback]] square. But since $\Im$ is a [[left adjoint]] it commutes with the $(\infty,1)$-colimit on objects and hence this diagram is equivalent to
...
This diagram is now indeed an [[(âˆž,1)-pullback]] by the fact that we have [[universal colimits]] in the [[(âˆž,1)-topos]] $\mathbf{H}_{th}$, hence that on the left the component $Y_i$ for each $i \in I$ is the [[(âˆž,1)-pullback]] of $\Im(Y_i) \to \Im(X)$, by assumption that we are taking an $(\infty,1)$-colimit over formally Ã©tale morphisms.
...
For the case that $X \simeq \ast$ in prop. \ref{EtalificationIsCoreflection}, then the proof there shows that the Ã©talification operation over the point is just ${\&} $:
...
Indeed, for any $X$ then ${\&} X \to \ast$ is a [[formally Ã©tale morphism]] since
...
The $\infty$-category $(\mathbf{H}_{th})_{/X}^{fet}$ is an [[(âˆž,1)-topos]] and the canonical inclusion into $(\mathbf{H}_{th})_{/X}$ is a [[geometric embedding]].
...
By prop. \ref{EtalificationIsCoreflection} the inclusion $(\mathbf{H}_{th})_{/X}^{fet} \hookrightarrow (\mathbf{H}_{th})_{/X}$ is [[reflective sub-(infinity,1)-category|reflective]] with reflector given by the $(\Im-equivalences , \Im-closed)$ factorization system. Since $\Im$ is a [[right adjoint]] and hence in particular preserves [[(âˆž,1)-pullbacks]], the $\Im$-equivalences are stable under pullbacks. By the discussion at _[[stable factorization system]]_ this is the case precisely if the corresponding reflector preserves [[finite (âˆž,1)-limits]]. Hence the embedding is a [[geometric embedding]] which exhibits a [[sub-(âˆž,1)-topos]] inclusion.
...
also as the ([[petit (âˆž,1)-topos|petit]]) [[(âˆž,1)-topos]] of $X$
or the _[[Ã©tale (âˆž,1)-topos]]_ of $X$.
...
for the composite [[(âˆž,1)-functor]] that sends any $A \in \mathbf{H}_{th}$ to the etalification, prop. \ref{EtalificationIsCoreflection}, of the projection $A \times X \to X$.
...
For $X, A \in \mathbf{H}_{th}$ and for $U \to X$ a [[formally Ã©tale morphism]] in $\mathbf{H}_{th}$ (hence like an [[open subset]] of $X$), we have that
...
where we used the [[adjoint (âˆž,1)-functor|âˆž-adjunction]] $(\iota \dashv Et)$ of prop. \ref{EtalificationIsCoreflection} and the [[(âˆž,1)-Yoneda lemma]].
...
indeed an $\mathbf{H}_{th}$-[[structure sheaf]] in the sense of [[structured (âˆž,1)-toposes]], for $\mathbf{H}_{th}$ regarded as a (large) [[geometry (for structured (âˆž,1)-toposes)]] with the [[formally Ã©tale morphisms]] being the "admissible morphisms".
...
We need to check that $\mathcal{O}_{X}$ preserves [[finite (âˆž,1)-limits]] and [[formally Ã©tale morphism|formally Ã©tale]] [[covers]] (where covers here in the [[canonical topology]] on the given toposes are [[1-epimorphisms]]). The first statement follows since $\mathcal{O}_{X}$ is [[right adjoint]] to the forgetful functor
...
with $U \to X$ formally Ã©tale, this can be completed to a diagram of the form
...
with $\widehat{U} \to U$ a formally Ã©tale 1-epimorphism. But since both [[1-epimorphisms]] as well as [[formally Ã©tale morphisms]] are stable under [[(âˆž,1)-pullback]] we can take $\widehat U \coloneqq \widehat{Y} \times_Y U$.
...
In the case that $\mathbf{H}_{th}$ happens to have an [[(âˆž,1)-site]] of definition whose [[covers]] are ([[Grothendieck pretopology|generated]]) from [[formally Ã©tale morphisms]] (a small [[geometry (for structured (âˆž,1)-toposes)]]), then $\mathbf{H}_{th}$ is the [[classifying topos|classifying (âˆž,1)-topos]] for [[structure sheaves]] and $\mathcal{O}_X$ in prop. \ref{StructureSheafIsIndeedStructureSheaf} may be regarded as the [[inverse image]] of the classifying [[geometric morphism]]
...
Let $G \in Grp(\mathbf{H}_{th})$ be an [[âˆž-group]] and write $\flat_{dR} \mathbf{B}G \in \mathbf{H}_{th}$ for the corresponding de Rham coefficient object.
Then
...
For $U \in \mathbf{H}_{th}$ a test object (say an object in a [[(âˆž,1)-site]] of definition, under the [[Yoneda embedding]]) a formally Ã©tale morphism $U \to X$ is like an [[open map]]/open embedding. Regarded as an object in $(\mathbf{H}_{th})_{/X}^{fet}$ we may consider the sections over $U$ of the cotangent bundle as defined above, which in $\mathbf{H}_{th}$ are diagrams
...
where we are now simply including on the left the formally Ã©tale map $(U \to X)$ along $(\mathbf{H}_{th})^{fet}_{/X} \hookrightarrow (\mathbf{H}_{th})_{/X}$.

In other words, the sections of the $G$-valued flat cotangent sheaf $\mathcal{O}_X(\flat_{dR}\mathbf{B}G)$ are just the sections of $X \times \flat_{dR}\mathbf{B}G \to X$ itself, only that the _domain_ of the section is constrained to be a formally Ã©tale patch of $X$.
...
is [[locally âˆž-connected (âˆž,1)-topos|locally âˆž-connected]].
...
preserves [[(âˆž,1)-limits]], so that it has a further
...
By the discussion at [slice (âˆž,1)-category -- Limits and colimits](slice+infinity-category#LimitsAndColimits) an [[(âˆž,1)-limit]] in the slice $(\mathbf{H}_{th})_{/X}$ is computed as an [[(âˆž,1)-limit]] in $\mathbf{H}$ of the [[diagram]] with the slice [[cocone]] adjoined. By [[right adjoint|right adjointness]] of the inclusion $Sh_{\mathbf{H}}(X) \hookrightarrow (\mathbf{H}_{th})_{/X}$ the same is then true for $Sh_{\mathbf{H}}(X) \coloneqq (\mathbf{H}_{th})_{/X}^{et}$.
...
Let $f \;\colon\; Y \longrightarrow X$ be a [[formally Ã©tale morphism]] in a differentially cohesive $\infty$-topos $\mathbf{H}$. Then pullback $f^\ast$ is the [[inverse image]] of an [[Ã©tale morphism]] of [[structured (âˆž,1)-toposes]] between the corresponding [[Ã©tale toposes]], def. \ref{TheStructureSheafOfX}, hence
there is an [[Ã©tale geometric morphism]]
...
Since the inclusion of the point into the interval is an op-[[final (âˆž,1)-functor]] we have (by [this proposition](over-%28infinity%2C1%29-category#FinalFunctorsInduceEquivalentSlices)) an [[equivalence]] of [[over-(âˆž,1)-categories]]
...
Since $f$ is formally Ã©tale by assumption and since [[formally Ã©tale morphisms]] are closed under [[composition]], this restricts to an [[equivalence]]
...
This $\theta_X(A)$ we call the **Liouville-PoincarÃ© $A$-cocycle** on $\underset{X}{\sum} \iota \mathcal{O}_X(A)$.
...
Consider the model of differential cohesion given by $\mathbf{H}_{th} =$ [[SynthDiffâˆžGrpd]]. Write $\Omega^1 \in \mathbf{H }\stackrel{i_!}{\hookrightarrow} \mathbf{H}_{th}$ for the abstract [[sheaf]] of [[differential 1-forms]].
...
So the Liouville-PoincarÃ© $\Omega^1$-cocycle according to \ref{TheLiouvillePoincareCocycle} is a [[differential 1-form]]
...
hence the original $\sigma$. This is the defining property which identifies $\that$ as the traditional [[Liouville-PoincarÃ© 1-form]].


=--


#### Manifolds and Ã©tale groupoids
 {#EtaleObjects}
 {#CohesivemanifoldsSeparated}

An ordinary [[topological groupoid|topological]]/[[Lie groupoid|Lie]] [[Ã©tale groupoid]] is one whose source/target map is an
[[Ã©tale map]]. We consider now a notion that can be formulated in the presence of infinitesimal cohesion which generalizes this.

+-- {: .num_defn #FormalEtaleGroupoid}
###### Definition

A [[groupoid object in an (âˆž,1)-category|groupoid object]] $\mathcal{G}_\bullet$ is an  **[[Ã©tale âˆž-groupoid]]** if the equivalent (via the higher [[Giraud theorem]]) [[effective epimorphism]] (the [[atlas]]) $\mathcal{G}_0 \longrightarrow \mathcal{G}$ is a [[formally Ã©tale morphism]].
...
such that both morphisms are [[formally Ã©tale morphisms]] and such that $U \to X$ is in addition an [[effective epimorphism]].
...
If $\iota \colon U \to X$ is a [[formally Ã©tale morphism]], def. \ref{FormallyEtaleInHTh}, then
...
By the definition of formal Ã©taleness and using the [[pasting law]] we have an equivalence of [[pasting diagrams]] of [[homotopy pullbacks]] of the following form:
...
By the discussion at _[[Mayer-Vietoris sequence]]_ in the section _[Over an âˆž-group](Mayer-Vietoris%20sequence#OverAGroupObject)_ and using that the [[infinitesimal shape modality]] preserves group structure, the defining [[homotopy pullback]] of $T_{inf} G$ is equivalent to the pasting of pullback diagrams of the form
...
By [this discussion](fiber+infinity-bundle#Properties) this fiber [[fiber âˆž-bundle]] is the [[associated âˆž-bundle]] of an essentially uniquely determined $\mathbf{Aut}(\mathbb{D}^V)$-[[principal âˆž-bundle]].
...
Given a $V$-manifold $X$, def. \ref{Manifold}, for framed $V$, def. \ref{Framing}, then its _[[frame bundle]]_ $Fr(X)$ is the $GL(V)$-[[principal âˆž-bundle]] given by prop. \ref{FormalDiskBundleOfRegularManifoldsTrivializesOverCover} via remark \ref{ModulatingMapOfFormalDiskBundle}.
...
Let $V$ be framed, def. \ref{Framing}, let $G$ be an [[âˆž-group]] and $G \to GL(V)$ a homomorphism to the general linear group of $V$, def. \ref{GeneralLinearGroup}, hence
...
in the [[slice (âˆž,1)-topos]].

=--

In fact $G\mathbf{Struc}\in \mathbf{H}_{/\mathbf{B}GL(n)}$ is the [[moduli âˆž-stack]] of such $G$-structures.

The double [[slice (âˆž,1)-topos|slice]] $(\mathbf{H}_{/\mathbf{B}GL(n)})_{/G\mathbf{Struc}}$ is the [[(âˆž,1)-category]] of such $G$-structures.
...
of the [[relative flat modality]], def. \ref{InducedRelativeShapeAndFlat} (using that by prop. \ref{CounitOfFlatRelIsFormallyEtale} this is also formally Ã©tale and hence induces map of frame bundles):
...
In traditional contexts this is also called _[[crystalline cohomology]]_ or just _[[de Rham cohomology]]_ . Since we already have an <a href="http://nlab.mathforge.org/nlab/show/cohesive+%28infinity%2C1%29-topos#deRhamCohomology">intrinsic notion of de Rham cohomology</a> in any [[cohesive (âˆž,1)-topos]], which is similar to but may slightly differ from infinitesimal flat differential cohomology, we shall say **[[synthetic differential geometry|synthetic]] de Rham cohomology** for the notion of def. \ref{InfinitesimalFlatCohomology} if we wish to honor traditional terminology. In this case we shall write
...
The objects on the left are **[[principal âˆž-bundle]]s equipped with flat [[connection on an âˆž-bundle|âˆž-connection]]** . The first morphism forgets their [[higher parallel transport]] along finite volumes and just remembers the parallel transport along infinitesimal volumes. The last morphism finally forgets also this connection information.
...
For $A \in \mathbf{H}_{th}$ an abelian [[âˆž-group]] object we say that the **[[de Rham theorem]]** for $A$-coefficients holds in $\mathbf{H}_{th}$ if for all $X \in \mathbf{H}_{th}$ the
...
The discussion at _[synthetic differential âˆž-groupoid -- Lie differentiation](synthetic+differential+infinity-groupoid#LieDifferentiation)_ immediately generalizes to produce a concept of Lie differentiation in any differentially cohesive context. This Lie differentiation is just the flat modality of the differential cohesion but regarded as cohesive over its induced [[infinitesimal cohesion]]. As such, there is a left adjoint to Lie differentiation, given by the corresponding shape modality. However, the substance of Lie theory here will be to restrict this adjunction to [[geometric âˆž-stacks]]. On the geometric $\infty$-stacks the Lie differentiation via passage to inffinitesimal cohesion will yield actual $L_\infty$-algebras, but some structure is required to make the formal [[Lie integration]] of these lang indeed in [[geometric âˆž-stacks]].
...
For $C^{op}$ any [[(âˆž,1)-site]] the construction of the
[[tangent (âˆž,1)-category]] $T_{C} \to C$ provides a canonical infinitesimal thickening of $C$:
...
$cod$-[[relative (âˆž,1)-adjunction]]. The composite $L \circ i$ is the [[cotangent complex]] functor for $C$ and $\Omega^\infty$ is fiberwise the canonical map out of the [[stabilization]].

The image of $i$ is contained in that of $\Omega^\infty$. Therefore we may restrict the $(cod \dashv i)$-adjunction on the right to the [[full sub-(âˆž,1)-category]] $\tilde T_C$ of $C^{\Delta[1]}$ on thise objects in the image of $\Omega^\infty$. This yields an infinitesimal neighbourhood of [[(âˆž,1)-sites]]
...
For references on the general notion of _[[cohesive (âˆž,1)-topos]]_, see there.

The following literature is related to or subsumes by the discussion here.

Something analogous to the notion of [[infinity-connected (infinity,1)-site|âˆž-connected site]] and the
[[fundamental âˆž-groupoid in a locally âˆž-connected (âˆž,1)-topos]] is the content of section 2.16.  of
...
The [infinitesimal path âˆž-groupoid adjunction](#LieTheory) $(\Re \dashv \Im \dashv \&)$ is essentially discussed in section 3 there.
...
The notion of forming [[petit topos|petit]] $(\infty,1)$-toposes of Ã©tale objects over a given object appears in

* {#Lurie} [[Jacob Lurie]], _[[Structured Spaces]]_
 

* {#Carchedi} [[David Carchedi]], _The Meaning of Ã‰tale Stacks_ ([arXiv:1212.2282](http://arxiv.org/abs/1212.2282))
 



[[!redirects differential cohesion]]
[[!redirects differential cohesive (âˆž,1)-topos]]
[[!redirects differential cohesive âˆž-topos]]
[[!redirects differential cohesive (âˆž,1)-toposes]]
[[!redirects differential cohesive âˆž-toposes]]
