...
where $v$ is a countably infinite set of variables.  (That is, a term is either a given variable, a given term applied to a given term, or the $\lambda$-abstraction of a given term as a function of a given variable.)  There are three equivalences on terms, $\alpha$ (for handling dummy variables), $\beta$ (substitution of terms), and $\eta$ (extensionality).  The corresponding category has 
...
* six function symbols
  ...
  * $A:T \times T \to T$. (Application) This takes $f$ and $g$ and produces $f(g)$.
...
It is fun to note that \[3â€“5\] all appeared (independently) in the same conference in 1999. Semantics of variable binders was clearly a pressing problem that year.
...
In the presence of variables, a term with $n$ variables is represented as $1 \to V^{*n} \times T$ (in your notation). However, for reasons of substitution, one would still like this hom-set to be in 1-1 correspondence with $T^n \to T$.  Somehow this is what the papers \[3â€“6\] manage to do, each in their own way.

I hope these references will be useful. It would be great if you had a more abstract monoidal framework of which the particular constructions in \[3â€“6\] are concrete examples. I have always wondered about the precise connection between \[3â€“6\], and whether there is a bigger picture.


[[!redirects sharing graphs]]