...
Write [[Grpd]] for the [[(2,1)-category]] of [[groupoids]], the [[full sub-(infinity,1)-category]] of [[âˆžGrpd]] on the [[1-truncated objects]].
...
We now consider any [[(âˆž,1)-topos]] $\mathbf{H}$ and formulate the group actions and their associated bundles in [[general abstract|general abstractly]] here.

By the discussion at _[[geometry of physics -- groups]]_, [[group objects in an (âˆž,1)-category]] are equivalently pointed connected objects which we write $(\ast\to\mathbf{B}G)\in \mathbf{H}^{\ast/}$, the group itself being the [[loop space object]]
...
has an immediate generalization to [[âˆž-groups]] in any [[(âˆž,1)-topos]]

+-- {: .num_defn #InfinityAction}
###### Definition

Given an [[âˆž-group]] $G \in \mathbf{H}$ and any object $V \in \mathbf{H}$, then an _[[âˆž-action]]_ $\rho$ of $G$ on $V$ is a [[homotopy fiber sequence]] in $\mathbf{H}$ of the form
...
We had seen above in prop. \ref{OrdinaryStabilizer} that the traditional concept of stabilizer groups of group actions is equivalent to groups of loops in the [[action groupoid]] of the given action. This equivalent reformulation has an immediate generalization to [[âˆž-actions]], def. \ref{InfinityAction}.

For $\mathbf{H}$ an [[(âˆž,1)-topos]],  $G\in \mathbf{H}$ an object equipped with [[âˆž-group]] structure, hence with a [[delooping]] $\mathbf{B}$G, and for $\rho$ an [[âˆž-action]] of $G$ on some $V$, exhibited by a [[homotopy fiber sequence]] of the form
