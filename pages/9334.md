...
Write $BMod^\otimes$ for the [[(âˆž,1)-category of operators]] of the [[(âˆž,1)-operad]] [[operad for bimodules]]. Write 
...
For $p \colon \mathcal{C}^\otimes \to BMod^\otimes$ a [[fibration of (âˆž,1)-operads]], write
...
exhibit these as two [[planar (âˆž,1)-operads]].
...
for the [[(âˆž,1)-category]] over the object labeled $\mathfrak{n}$.
...
For $\mathcal{C}^\otimes \to Assoc^\otimes$ a [[coCartesian fibration of (âˆž,1)-operads]], hence exhibiting $\mathcal{C}^\otimes$ as a [[monoidal (âˆž,1)-category]], [[pullback]] along the canonical map $\phi \colon BMod^\otimes \to Assoc^\otimes$ gives a fibration
...
For $\mathcal{C}^\otimes \to BMod^\otimes$ a [[fibration of (âˆž,1)-operads]] we say that the corresponding [[(âˆž,1)-category]] of [[(âˆž,1)-algebras over an (âˆž,1)-operad]]
...
This notation is naturally suggestive of the existence of the further [[horizontal composition]] by [[tensor product of (âˆž,1)-modules]], which we come to [below](#TensorProductOfBimodules).
...
Write $Tens^\otimes$ for the [[generalized (âˆž,1)-operad]]
discussed at _[[tensor product of âˆž-modules]]_.

For $S \to \Delta^{op}$ an [[(âˆž,1)-functor]] (given as a map of simplicial sets from a [[quasi-category]] $S$ to the [[nerve]] of the [[simplex category]]), write
...
Moreover, for $\mathcal{C}^\otimes \to Tens^\otimes_S$ a [[fibration]] in the [[model structure for quasi-categories]] which exhibits $\mathcal{C}^\otimes$ as an $S$-[[family of (âˆž,1)-operads]], write

$$
  Alg_S(\mathcal{C}) \hookrightarrow Fun_{Tens^\otimes_S}(Step_S, \mathcal{C}^\otimes)
$$

for the full [[sub-(âˆž,1)-category]] on those [[(âˆž,1)-functors]] which send inert morphisms to inert morphisms.
...
We discuss the generalization of the notion of bimodules to [[homotopy theory]], hence the generalization from [[category theory]] to [[(âˆž,1)-category theory]]. ([Lurie, section 4.3](#Lurie)).

Let $\mathcal{C}$ be [[monoidal (âˆž,1)-category]] such that 

1. it admits [[geometric realization]] of [[simplicial objects in an (âˆž,1)-category]] (hence a [[left adjoint|left]] [[adjoint (âˆž,1)-functor]] ${\vert-\vert} \colon \mathcal{C}^{\Delta^{op}} \to \mathcal{C}$ to the constant simplicial object functor), true notably when $\mathcal{C}$ is a [[presentable (âˆž,1)-category]];
...
Then there is an 
[[(âˆž,2)-category]] $Mod(\mathcal{C})$ which given as an [[(âˆž,1)-category object]] internal to [[(âˆž,1)Cat]] has
...
* $(\infty,1)$-category of objects
  ...
  the [[A-âˆž algebras]] and [[âˆž-algebra]] [[homomorphisms]] in $\mathcal{C}$;
...
Morover, the [[horizontal composition]] of bimodules in this [[(âˆž,2)-category]] is indeed the relative 
[[tensor product of âˆž-modules]]
...
we start with a [[generalized (âˆž,1)-operad]] 
$Tens^\otimes \to FinSet_* \times \Delta^{op}$ which is such that 
the [[(âˆž,1)-algebras over an (âˆž,1)-operad]] over its fiber over $[k] \in \Delta^{op}$ is equivalently the collection of $(k+1)$-tuples of 
[[A-âˆž algebras]] in $\mathcal{C}$ together with a string of $k$ $\infty$-bimodules between them. Then we turn that into a [[simplicial object in an (âˆž,1)-category|simplicial object]] in [[(âˆž,1)Cat]]
...
This turns out to be an [[internal (âˆž,1)-category]] object in [[(âˆž,1)Cat]], hence an [[(âˆž,2)-category]] whose object of objects is the category $Alg(\mathcal{C})$ of [[A-âˆž algebras]] and [[homomorphisms]] in $\mathcal{C}$ between them, and whose object of morphisms is the category $BMod(\mathcal{C})$ of $\infty$-bimodules and [[intertwiners]].
...
* on the right we have the [[(âˆž,1)-category]] of [[(âˆž,1)-algebras over an (âˆž,1)-operad]] given by lifts $\mathcal{A}$ in
...
The general theory in terms of [[higher algebra]] of [[(âˆž,1)-operads]] is discussed in section 4.3 of 
...
[[!redirects âˆž-bimodule]]
[[!redirects âˆž-bimodules]]
[[!redirects (âˆž,1)-bimodule]]
[[!redirects (âˆž,1)-bimodules]]

[[!redirects (âˆž,1)-category of (âˆž,1)-bimodules]]
[[!redirects (infinity,1)-category of (âˆž,1)-bimodules]]

[[!redirects (âˆž,1)-categories of (âˆž,1)-bimodules]]
[[!redirects (infinity,1)-categories of (âˆž,1)-bimodules]]
