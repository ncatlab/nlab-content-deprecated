...
An **$n$-truncated [[âˆž-groupoid]]** is an [[n-groupoid]].

An **$n$-truncated [[topological space]]** is a [[homotopy n-type]]: all [[homotopy groups]] above degree $n$ are trivial.

An **$n$-truncated object** in a general [[(âˆž,1)-category]] is an object such that all [[(âˆž,1)-categorical hom-space|hom-âˆž-groupoids]] into it are $n$-truncated.

If an object in an [[(âˆž,1)-topos]]_ is $k$-truncated for any (possibly large) $k$, then it is $n$-truncated precisely if all its [[homotopy groups in an (âˆž,1)-topos|categorical homotopy groups]] above degree $n$ are trivial. 

The complementary notion of $n$-truncated object  is that of an [[n-connected object of an (âˆž,1)-category]].
...
An [[âˆž-groupoid]] $A \in \infty Grpd$ is **$n$-truncated** for $n \in \mathbb{N}$ if it is an [[n-groupoid]]:

Precisely: in the model of [[âˆž-groupoid]]s given by [[Kan complex]]es $A$ is **$n$-truncated** if the [[simplicial homotopy group]]s $\pi_k(A,x)$ are trivial for all $x$ and all $k \gt n$.
...
To generalize this, let now $C$ be an arbitrary [[(âˆž,1)-category]]. For $X,A$ objects in $C$ write $C(X,A) \in $ [[âˆž Grpd]] for the [[(âˆž,1)-categorical hom-space]] (if $C$ is given as a [[simplicially enriched category]] then this is just the [[SSet]]-[[hom-object]] which is guaranteed to be a [[Kan complex]]).
...
An [[âˆž-groupoid]] $A$ is $n$-truncated precisely for all other [[âˆž-groupoid]]s $X$ the hom-$\infty$-groupoid $\infty Grpd(X,A)$ is $n$-truncated.

=--

In categorical terms this just says that [[(n,k)-transformation|(âˆž,k)-transformation]] between $X$ and $A$ whose components a [[k-morphism]]s in $A$ cannot be nontrivial for $k \gt n$ if there are no nontrivial [[k-morphism]]s with $k \gt n$ in $A$.

Using this fact we can transport the notion of $n$-truncation to any [[(âˆž,1)-category]] by testing it on hom-[[âˆž-groupoid]]s:

+-- {: .num_defn #nTruncatedObject}
###### Definition
**($n$-truncated object in an $(\infty,1)$-category)**

An [[object]] $A \in C$ of an [[(âˆž,1)-category]] $C$ is **$n$-truncated**, for $n \in \mathbb{N}$, if for all $X \in C$ the hom-[[âˆž-groupoid]] $C(X,A)$ is $n$-truncated.
...
* A 0-truncated object is also called **discrete** . Notice that this is _categorically discrete_ as in [[discrete category]], not discrete in the sense of [[topological space]]s. An object in an [[(âˆž,1)-topos]] is discrete in this sense if, regarded as an [[âˆž-groupoid]] with extra structure it has only trivial morphisms.
...
A [[morphism]] $f : X \to Y$ of [[âˆž-groupoids]] is $n$-truncated if all of its [[homotopy fibers]] are $n$-truncated by def. \ref{nTruncatedObject}.

A morphism $f : X \to Y$ in an [[(âˆž,1)-category]] $C$ is $n$-truncated if for all $W \in C$ the postcomposition morphism

$$
  C(W,f) : C(W,X) \to C(W,Y)
$$

is $n$-truncated in [[âˆžGrpd]].
...
object of the [[over quasi-category|over (âˆž,1)-category]] $C_{/Y}$.

### In terms of categorical homotopy groups 

At least if the ambient [[(âˆž,1)-category]] is even an [[âˆž-stack]] [[(âˆž,1)-topos]] there is an alternative, more intrinsic, characterization of $n$-truncation in terms of [[categorical homotopy groups in an (âˆž,1)-topos]]:

+-- {: .num_prop #RecognizngnTuncationOnSimplicialHomotopyGroups}
###### Proposition

Suppose that an object $X$ in an [[âˆž-stack]] [[(âˆž,1)-topos]] is $k$-truncated for _some_ $k \in \mathbb{N}$ (possibly very large).

Then for any $n \in \mathbb{N}$ this $X$ is $n$-truncated precisely if all the [[homotopy groups in an (âˆž,1)-topos|categorical homotopy groups]] above degree $n$ are trivial.
...
[[âˆžGrpd]]. Since the [[hom-functor]]s $C(d,-)$ preserve
[[(âˆž,1)-limits]], we have 
in particular that $X \to X \times_Y X$ in $C$ 
is $k$-truncated 
if $C(d,X) \to C(d,X) \times_{C(d,Y)} C(d,X)$ is
$k$-truncated for all $d$ in [[âˆžGrpd]]. Therefore it is 
sufficient to prove the statement for
morphisms in $C =$ [[âˆžGrpd]].

So let now $f : X \to Y$ be a morphism of [[âˆž-groupoid]]s. 
...
For $C$ an $(\infty,1)$-category and $k \geq -2$, the full [[sub-(âˆž,1)-category]] $\tau_{\leq k} C$ is stable under all [[limit in a quasi-category|limits]] in $C$.
...
Let $\mathbf{H}$ be an [[(âˆž,1)-topos]]. For all $(-2) \leq n \leq \infty$ the [[class]] of $n$-truncated morphisms in $\mathbf{H}$ forms the right class in a [[orthogonal factorization system in an (âˆž,1)-category]]. The left class is that of [[n-connected]] morphisms in $\mathbf{H}$.

=--

This appears as a remark in [[Higher Topos Theory|HTT, Example 5.2.8.16]]. A construction of the factorization in terms of a [[model category]] [[presentable (âˆž,1)-category|presentation]] is in ([Rezk, prop. 8.5](#Rezk)). See also [[n-connected/n-truncated factorization system]].


### Model category presentations
 {#ModelCategoryPresentations}

There are [[model structures for homotopy n-types]] that [[presentable (âˆž,1)-category]] present the full [[sub-(âˆž,1)-categories]] of $n$-truncated objects in some ambient $(\infty,1)$-category. See there for more details.
...
For $C$ an [[(âˆž,1)-category]] and $n \geq -2$ in $\mathbb{Z}$ write $\tau_{\leq n} C $ for the [[full subcategory]] of $C$ on its $n$-truncated objects. 

So for instance for $C = $ [[âˆžGrpd]] we have $\tau_{\leq n} \infty Grpd = n Grpd$.


+-- {: .num_prop}
###### Proposition

If $C$ is an [[(âˆž,1)-category]] that is [[presentable (âˆž,1)-category|presentable]] then the canonical inclusion [[(âˆž,1)-functor]]
...
So $n$-truncated objects form a [[reflective sub-(âˆž,1)-category]] 
...
[[locally presentable (âˆž,1)-categories]] $C$ and $D$ commutes with 
truncation:
...
in [[(âˆž,1)Cat]] can be filled by a 2-cell. To see this, notice that the
[[adjoint (âˆž,1)-functor]] of both composite morphisms exists 
(because that of $F$ exists by the [[adjoint (âˆž,1)-functor theorem]]
...
If $C$ is an [[(âˆž,1)-topos]], then truncation $\tau_{\leq n} : C \to C$ preserves [[finite (âˆž,1)-limit|finite]] [[products]].
...
First notice that the statement is true for $C = $ [[âˆžGrpd]]. For instance we can use the example [In âˆžGrpd and Top](#InInfGrpd), model [[âˆž-groupoid]]s by [[Kan complex]]es and notice that then $\tau_{\leq n}$ is given by the truncation functor $tr_{n+1} : sSet \to [\Delta^{op}_{\leq n+1}, sSet]$. This is also a [[right adjoint]] and as such preserves in particular product in $sSet$, which are $(\infty,1)$-products in $\infty Grpd$.

From that we deduce that the statement is true for  $C$ any [[(âˆž,1)-category of (âˆž,1)-presheaves]] $C = PSh_{(\infty,1)}(K) = Func_{(\infty,1)}(K^{op}, \infty Grpd)$ because all relevant operations there are objectwise those in $\infty Grpd$.

So far this shows even that on presheaf $(\infty,1)$-toposes all products (not necessarily finite) are preserved by truncation.

A general [[(âˆž,1)-topos]] $C$ is (by definition) a left exact [[reflective sub-(âˆž,1)-category]] of a presheaf $(\infty,1)$-topos, 
...
is the [[Postnikov tower in an (âˆž,1)-category]] of $A$. See there for more details.
...
In an $(\infty,1)$-topos $C$ there is a notion of categorical [[homotopy groups in an (âˆž,1)-topos]]. For the $(\infty,1)$-topos [[âˆžGrpd]] given by the model of [[Kan complex]]es this coincides with the notion of [[simplicial homotopy groups]]:

+-- {: .num_lemma}
###### Observation

An object $A$ in the [[(âˆž,1)-topos]] [[âˆžGrpd]] is $n$-truncated precisely if its [[homotopy groups in an (âˆž,1)-topos|categorical homotopy groups]] $\pi_k(A)$ vanish for all $k \gt n$.

=--

This simple relation between $n$-truncation and categorical homotopy groups is almost, but not exactly true in an arbitrary [[(âˆž,1)-topos]].

+-- {: .num_prop}
###### Proposition

Let $\mathbf{H}$ be an [[(âˆž,1)-topos]] and $A \in \mathbf{H}$ an $n$-truncated object.

Then

1. for $k \gt n$ we have for the [[homotopy groups in an (âˆž,1)-topos|categorical homotopy groups]] $\pi_k(A) = *$;
...
**Notice.** If $A$ on the other hand is not truncated at all, then all its homotopy groups may be trivial and $A$ may still not be equivalent to the [[terminal object]]. This means that [[Whitehead's theorem]] may fail in a general [[(âˆž,1)-topos]] for untruncated objects. It holds, however, in [[hypercomplete (âˆž,1)-topos]]es.
...
A [[functor]] $f : X \to Y$ between [[groupoids]], is $n$-truncated precisely when regarded as a morphism in [[âˆžGrpd]] it is 
...
$Sh_{(2,1)}(C) \hookrightarrow Sh_{(\infty,1)}(C)$ for the [[(2,1)-topos]] of  [[stacks]]/[[(2,1)-sheaves]] inside the [[(âˆž,1)-sheaf (âˆž,1)-topos]] of all [[âˆž-stacks]]/[[(âˆž,1)-sheaves]].

Write $L_W [C^{op}, Grpd]$ for the [[simplicial localization]] of groupoid valued presheaves in $C$
and write $[C^{op}, sSet]_{proj,loc}$ for the local projective [[model structure on simplicial presheaves]] that [[presentable (âˆž,1)-category|presents]] $Sh_{(\infty,1)}(C)$.
...
$Sh_\infty(A,f)$ is 0-truncated in [[âˆžGrpd]]. We may choose a cofibrant
...
An object in [[âˆžGrpd]] is $n$-truncated precisely if it is an [[n-groupoid]]. To some extent this is so by definition. Equivalently, an object in [[Top]] is $n$-truncated if it is (in the equivalence class of) a [[homotopy n-type]].

So we have for $n \in \mathbb{N}$ a [[reflective sub-(âˆž,1)-category]]
...
If we model the [[(âˆž,1)-category]] [[âˆžGrpd]] as the [[Kan complex]]-[[enriched category]]/fibrant [[simplicial category]] $KanCplx \subset $ [[sSet]] of [[Kan complex]]es, then the truncation adjunction
...
This shows that $KanCplx_{n+1}$ is indeed a full [[sub-(âˆž,1)-category]] of $KanCplx$ on $n$-truncated objects  

Moreover, by the fact discussed at <a href="http://ncatlab.org/nlab/show/adjoint+(infinity%2C1)-functor#SimplicialAndDerived">Simplicial and derived adjunctions</a> at [[adjoint (âˆž,1)-functor]] we have that the [[sSet]]-enriched adjunction $(tr_{n+1} \dashv cosk_{n+1})$ on $KanCplx$ indeed presents a pair of [[adjoint (âˆž,1)-functor]]s on [[âˆžGrpd]]. So $tr_{n+1} : KanCplx \to KanCplx$ indeed presents the [[left adjoint]] $\tau_{\leq} : \infty Grpd \to n Grpd$ to the inclusion $n Grpd \hookrightarrow \infty Grpd$.
...
[[monomorphism]] (as discussed there), precisely if its [[diagonal]] is an [[isomorphism]]. Embedded into [[(âˆž,1)-category]] this becomes 
the special case of prop. \ref{DiagonalCharacterization}
for $n = 0$: a morphism is (-1)-truncated 
(hence a [[monomorphism in an (âˆž,1)-category]]), precisely
if its diagonal is (-2)-truncated (hence an [[equivalence in an (âˆž,1)-category]]).
...
In particular, the diaginal is a [[monomorphism in an (âˆž,1)-category]], 
...
The discussion of categorical [[homotopy groups in an (âˆž,1)-topos]] is in 
section 6.5.1.

A discussion in terms of [[model category]] [[presentable (âˆž,1)-category|presentations]] is in section 7 of
...
[[!redirects n-truncated object of an (âˆž,1)-category]]
[[!redirects n-truncated object in an (infinity,1)-category]]
[[!redirects n-truncated object in an (âˆž,1)-category]]
[[!redirects n-truncated objects in an (infinity,1)-category]]
[[!redirects n-truncated objects in an (âˆž,1)-category]]


[[!redirects truncated object of an (infinity,1)-category]]
[[!redirects truncated object of an (âˆž,1)-category]]
[[!redirects truncated object in an (infinity,1)-category]]
[[!redirects truncated object in an (âˆž,1)-category]]

[[!redirects n-truncated object of an (infinity,1)-topos]]
[[!redirects n-truncated object of an (âˆž,1)-topos]]
[[!redirects n-truncated object in an (infinity,1)-topos]]
[[!redirects n-truncated object in an (âˆž,1)-topos]]

[[!redirects truncated object of an (infinity,1)-topos]]
[[!redirects truncated object of an (âˆž,1)-topos]]
[[!redirects truncated object in an (infinity,1)-topos]]
[[!redirects truncated object in an (âˆž,1)-topos]]
