...
The _[[objects]]_ or _[[types]]_ that it describes are _[[cohesive topos|cohesive]] [[homotopy types]]_, hence [[cohesive âˆž-groupoids]], such as for instance [[smooth âˆž-groupoids]]. See also at _[[motivation for cohesive toposes]]_ for a non-technical discussion.

To the extent that plain [[homotopy type theory]] is a formalization of [[homotopy theory]] in that it is the [[internal language of an (âˆž,1)-topos]], _cohesive_ homotopy type theory is the [[internal logic|internal language]] of a _[[cohesive (âˆž,1)-topos]]_.

One way to arrive at cohesive homotopy type theory is to start with the external axioms for [[cohesive toposes|cohesion]] on a given [[topos]] in terms of an [[adjoint quadruple]] of functors _on_ the given [[topos]] and attempt to formulate them instead in the [[internal logic|internal]] [[Mitchell-BÃ©nabou language|Mitchell-BÃ©nabou]] [[type theory]] language of the topos. This fails, since for formulating the [[reflective subcategories]] of [[discrete objects]] and [[codiscrete objects]] internally one needs instead of the [[type of propositions]] a full [[type of types]], as exists only in [[homotopy type theory]]. Passing to this, the axioms for cohesion can be formulated ([Shulman](#ShulmanInternalizing)) and are automatically that of [[cohesive (âˆž,1)-topos|homotopy cohesion]].


## The Axioms
  {#TheAxioms}

We discuss the formulation in [[homotopy type theory]] of the [internal axioms](http://ncatlab.org/nlab/show/cohesive+%28infinity%2C1%29-topos#InternalDefinition) on a [[cohesive (âˆž,1)-topos]].
...
[[geometric embedding|left-exact]] [[reflective sub-(âˆž,1)-category]],
to be called the [[base (âˆž,1)-topos]] "of [[codiscrete objects]]"._
...
**Axiom B.** There is also a [[coreflective sub-(âˆž,1)-category]] of
_[[discrete objects]]_ such that with the codiscrete reflection
it makes the ambient theory that of a [[local (âˆž,1)-topos]].
...
In other words, one can form the smooth [[fundamental âˆž-groupoid]]
$\Pi(I)$ of the interval and regard _that_ as a [[homotopy type]]
_without_ further geometric structure (a [[discrete âˆž-groupoid]]). This $\Pi(I)$ is an _[[interval type]]_, while $I$ itself is not.
...
[[fundamental infinity-groupoid in a locally infinity-connected (infinity,1)-topos|fundamental âˆž-groupoid reflector]] $\mathbf{\Pi}$.

Specifically, there is a [[model]] for 
[[cohesive (âˆž,1)-topos|homotopy cohesion]], called 
[[SmoothâˆžGrpd]], in which [[smooth manifolds]] 
...
object, but the type $\mathbf{\Pi}(X)$ is a [[discrete âˆž-groupoid]]
...
We discuss implications of the axioms of cohesive homotopy type theory and go through the discussion of the various [[structures in a cohesive (âˆž,1)-topos]].
